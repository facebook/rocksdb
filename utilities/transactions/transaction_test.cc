...
TEST_P(TransactionTest, ValidateSnapshotTest) {
  ...
  Transaction* txn1 = db->BeginTransaction(write_options, TransactionOptions());
  ASSERT_TRUE(txn1);
  ...
  Transaction* txn2 = db->BeginTransaction(write_options, TransactionOptions());
  ASSERT_TRUE(txn2);
  txn2->SetSnapshot();
  ...
  s = pes_txn2->ValidateSnapshot(
      db->DefaultColumnFamily(), "foo", trakced_seq, &dont_care);
