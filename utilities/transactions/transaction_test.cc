...
TEST_P(TransactionTest, RaceInstallFlushResults) {
  ...
  const std::string cf1_name = "cf1";
  {
    ...
    ASSERT_OK(db->CreateColumnFamily(options, cf1_name, &cfh1));
    assert(cfh1);
    delete cfh1;
  }
  ...
  std::vector<ColumnFamilyDescriptor> cf_descs;
  cf_descs.emplace_back(kDefaultColumnFamilyName, options);
  cf_descs.emplace_back(cf1_name, options);
  std::vector<ColumnFamilyHandle*> handles;
  ASSERT_OK(ReOpenNoDelete(cf_descs, &handles));
  assert(2 == handles.size());

  auto* dbimpl = static_cast_with_check<DBImpl>(db->GetRootDB());
  assert(dbimpl);
  {
    dbimpl->TEST_LockMutex();
    ASSERT_LE(2, dbimpl->GetBGJobLimits().max_flushes);
    dbimpl->TEST_UnlockMutex();
  }

  ASSERT_OK(dbimpl->PauseBackgroundWork());

  ASSERT_OK(db->Put(WriteOptions(), handles[1], "foo", "value"));
  ASSERT_OK(db->Put(WriteOptions(), "foo", "value"));

  ASSERT_OK(dbimpl->TEST_FlushMemTable(false, true, handles[1]));

  ASSERT_OK(db->Put(WriteOptions(), "foo", "value"));
  ASSERT_OK(dbimpl->TEST_FlushMemTable(false, true));
  ...
  SyncPoint::GetInstance()->SetCallBack(
      ...
      [&](void* /*arg*/) {
        dbimpl->mutex()->AssertHeld();
        if (!called) {
          called = true;
          SyncPoint::GetInstance()->LoadDependency({
              {"VersionSet::LogAndApply:WriteManifestStart",
               "TransactionTest::RaceInstallFlushResults:BgFlush2"},
              {"TransactionTest::RaceInstallFlushResults:BgFlush2",
               "VersionSet::LogAndApply:WriteManifest"},
          });
        } else {
          TEST_SYNC_POINT("TransactionTest::RaceInstallFlushResults:BgFlush2");
        }
      });
  SyncPoint::GetInstance()->EnableProcessing();

  ASSERT_OK(dbimpl->ContinueBackgroundWork());

  ASSERT_OK(dbimpl->TEST_WaitForFlushMemTable());
  ASSERT_OK(dbimpl->TEST_WaitForFlushMemTable(handles[1]));
  ...
  DB* db1 = nullptr;
  Status s = DB::OpenForReadOnly(options, dbname, &db1);
  ASSERT_OK(s);
  assert(db1);
  delete db1;
}
