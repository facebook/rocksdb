...
// TODO(AR) consider optimising memory use in PartialMergeMulti, replace
// Operations and OperationsIndex with a pointers approach
// TODO(AR) consider implementing Collection Operations for kRemoveAll,
// kReplace, and kReplaceAll
// TODO(AR) implement support for variable sized records per key
// TODO(AR) implement support for variable sized records within a key using
...
CollectionMergeOperator::CollectionMergeOperator(
    ...
    const std::function<std::string(const char* const, size_t, const size_t)>
        trace_record_serializer)
...
bool CollectionMergeOperator::FullMergeV2(
  ...
  if (debug) {
    Log(InfoLogLevel::DEBUG_LEVEL, logger,
        "Starting CollectionMergeOperator::FullMergeV2");
    Log(InfoLogLevel::DEBUG_LEVEL, logger, "key=[%s]",
        merge_in.key.ToString(true).c_str());
    Log(InfoLogLevel::DEBUG_LEVEL, logger, "existing_value: %p",
        merge_in.existing_value);
    if (merge_in.existing_value) {
      Log(InfoLogLevel::DEBUG_LEVEL, logger, "existing_value: [%s]",
          merge_in.existing_value->ToString(true).c_str());
    }
    Log(InfoLogLevel::DEBUG_LEVEL, logger, "operand_list.size: %zu",
        merge_in.operand_list.size());
  }

  // we wrap operand list in MultiOperandList so that _kMulti is transparently
  // expanded for us
  MultiOperandList<const std::vector<Slice>> multi_operand_list(
  ...
  if (merge_in.existing_value != nullptr && !found_clear_operation) {
  ...
  for (auto it_operand = last_clear_operation;
       it_operand != multi_operand_list.end(); ++it_operand) {
    ...
    switch (collection_op) {
      ...
      case CollectionOperation::kAdd:
        if (!fm_add(value, operand->size() - sizeof(CollectionOperation),
                    merge_out->new_value, logger, debug)) {
          if (trace_) {
      ...
      case CollectionOperation::kRemove:
        if (!fm_remove(value, operand->size() - sizeof(CollectionOperation),
                       merge_out->new_value, logger, debug)) {
          if (trace_) {
      ...
      default:
        Log(InfoLogLevel::ERROR_LEVEL, logger,
            "Unknown Collection operation: %x", collection_op);
        if (trace_) {
  ...
  if (debug) {
    Log(InfoLogLevel::DEBUG_LEVEL, logger, "Updated new_value=[%s]",
        toHex(merge_out->new_value.c_str(), merge_out->new_value.length())
            .c_str());
    Log(InfoLogLevel::DEBUG_LEVEL, logger,
        "CollectionMergeOperator::FullMergeV2 OK");
  }
...
bool CollectionMergeOperator::PartialMergeMulti(
  ...
  if (debug) {
    Log(InfoLogLevel::DEBUG_LEVEL, logger,
        "Starting CollectionMergeOperator::PartialMergeMulti");
    Log(InfoLogLevel::DEBUG_LEVEL, logger, "key=[%s]",
  ...
  while (it != multi_operand_list.begin()) {
    ...
    if (collection_op == CollectionOperation::kClear) {
      ...
      pm_clear(operations);
      break;  // NOTE: we can stop processing, don't need to examine anything
              // before this kClear

    } else if (collection_op == CollectionOperation::kAdd) {
      if (!pm_add(operations, operations_index, record_ptr,
                  operand->size() - sizeof(CollectionOperation), logger,
                  debug)) {
        new_value->clear();  // we should leave new_value unchanged, so cleanup
                             // any progress so far
        if (trace_) {
      ...
      if (!pm_remove(operations, operations_index, record_ptr,
                     operand->size() - sizeof(CollectionOperation), logger,
                     debug)) {
        new_value->clear();  // we should leave new_value unchanged, so cleanup
                             // any progress so far
      ...
      Log(InfoLogLevel::ERROR_LEVEL, logger, "Unknown Collection operation: %x",
          collection_op);
      new_value->clear();  // we should leave new_value unchanged, so cleanup
                           // any progress so far
      if (trace_) {
  ...
  if (debug) {
    Log(InfoLogLevel::DEBUG_LEVEL, logger, "Updated new_value=[%s]",
        toHex(new_value->c_str(), new_value->length()).c_str());
    Log(InfoLogLevel::DEBUG_LEVEL, logger,
        "CollectionMergeOperator::PartialMergeMulti OK");
  }
...
void CollectionMergeOperator::pm_contiguous_operations(
  ...
  for (auto it_a = operations.begin(); it_a != operations.end(); it_a++) {
    for (auto it_b = it_a + 1; it_b != operations.end(); it_b++) {
      if (it_a->operation_ == it_b->operation_) {
        ...
        it_a->records_.insert(it_a->records_.end(), it_b->records_.begin(),
                              it_b->records_.end());
...
void CollectionMergeOperator::pm_serialize(Operations& operations,
  ...
  while (!operations.empty()) {
    ...
    if (operation.operation_ != CollectionOperation::kClear) {
      for (auto it = operation.records_.rbegin();
           it != operation.records_.rend(); ++it) {
...
bool CollectionMergeOperator::pm_add(Operations& operations,
                                     OperationsIndex& operations_index,
                                     const char* const records_ptr,
                                     const size_t records_len, Logger* logger,
                                     const bool debug) const {
  // loop through records in reverse
  for (const char* record_ptr = records_ptr + records_len - fixed_record_len_;
       record_ptr >= records_ptr; record_ptr -= fixed_record_len_) {
    ...
    if (operation_idx == operations_index.end()) {
      ...
      push_or_mergeprev_operation_record(operations, operations_index,
                                         operation_idx,
                                         CollectionOperation::kAdd, record);
    } else {
      // yes, previously processed an operation for this record

      const auto it_operation_record_offsets = operation_idx->second.rbegin();
      auto it_prev_operation =
          operations.begin() + it_operation_record_offsets->first;
      ...
      if (it_prev_operation->operation_ == CollectionOperation::kAdd) {
        ...
        switch (unique_constraint_) {
          case UniqueConstraint::kMakeUnique:
            ...
            if (debug) {
              Log(InfoLogLevel::DEBUG_LEVEL, logger,
                  "encountered new record for kAdd which is a duplicate of "
                  "another new record, skipping as "
                  "UniqueConstraint::kMakeUnique is set. (value=%s)",
                  record.ToString(true).c_str());
            }
            break;
          ...
          case UniqueConstraint::kEnforceUnique:
            ...
            Log(InfoLogLevel::ERROR_LEVEL, logger,
                "encountered new record kAdd which is a duplicate of another "
                "new record, but UniqueConstraint::kEnforceUnique is set. "
                "(value=%s)",
                record.ToString(true).c_str());
          ...
          case UniqueConstraint::kNone:
            ...
            push_or_mergeprev_operation_record(
                operations, operations_index, operation_idx,
                CollectionOperation::kAdd, record);
            break;
        }
      } else if (it_prev_operation->operation_ ==
                 CollectionOperation::kRemove) {
        // no, the previously processed (following) operation was kRemove, so we
        // IGNORE this kAdd and drop the following kRemove
        remove_operation_record(operations, operations_index, operation_idx,
                                it_operation_record_offsets, it_prev_operation);

      } else {
        Log(InfoLogLevel::ERROR_LEVEL, logger,
            "Processing of kAdd before op=%" PRIu8 " is not implemented.",
            it_prev_operation->operation_);
        return false;
      }
    }
  }
...
void CollectionMergeOperator::push_or_mergeprev_operation_record(
    Operations& operations, OperationsIndex& operations_index,
  ...
  if (operations.empty() == false &&
      operations.back().operation_ == collection_op) {
  ...
  if (it_operation_idx == operations_index.end()) {
    OperationRecordOffsets operation_record_offsets{
        {operations.size() - 1, {operations.back().records_.size() - 1}}};
    operations_index.emplace(record, operation_record_offsets);
  } else {
    auto it_existing_operation_record_offsets =
        it_operation_idx->second.find(operations.size() - 1);
    if (it_existing_operation_record_offsets ==
        it_operation_idx->second.end()) {
      it_operation_idx->second.emplace(
          operations.size() - 1,
          RecordOffsets{operations.back().records_.size() - 1});
    } else {
      it_existing_operation_record_offsets->second.push_back(
          operations.back().records_.size() - 1);
    }
  }
}

void CollectionMergeOperator::remove_operation_record(
    Operations& operations, OperationsIndex& operations_index,
    const OperationsIndex::iterator& it_operation_idx,
  ...
  it_prev_operation->records_.erase(it_prev_operation->records_.begin() +
                                    record_offset);

  // we need to renumber any records_offset_(s) after
  // operation_record_offset.record_offset_ in operation_idx
  ...
  if (drop_operation) {
    ...
    // update the index - drop the only operation_offset from the index (for
    // this record)
    ...
    // update the index - renumber anything in the index greater than
    // operation_offset
    for (auto it_idx = operations_index.begin();
         it_idx != operations_index.end(); ++it_idx) {
      const OperationRecordOffsets* const cur_op_recs = &(it_idx->second);
      OperationRecordOffsets updated_op_recs;
      for (auto it_op_recs = cur_op_recs->begin();
           it_op_recs != cur_op_recs->end(); ++it_op_recs) {
    ...
    // update the index - for the record, at operation_offset, drop the
    // record_offset
    if (it_operation_record_offsets->second.size() > 1) {
      ...
      if (it_operation_idx->second.empty()) {
        // there are no records left for the offset in the index, so drop the
        // entry
        operations_index.erase(it_operation_idx);
      }
    }

    // update the index - for any records at operation_offset whoose
    // record_offset > record_offset, subtract 1
    for (auto it_idx = operations_index.begin();
         it_idx != operations_index.end(); ++it_idx) {
      auto it_op_rec = it_idx->second.find(operation_offset);
      if (it_op_rec != it_idx->second.end()) {
        for (auto it_rec = it_op_rec->second.begin();
             it_rec != it_op_rec->second.end(); ++it_rec) {
...
bool CollectionMergeOperator::pm_remove(Operations& operations,
  ...
  for (const char* record_ptr = records_ptr + records_len - fixed_record_len_;
    ...
    if (operation_idx == operations_index.end()) {
      ...
      push_or_mergeprev_operation_record(operations, operations_index,
                                         operation_idx,
                                         CollectionOperation::kRemove, record);
    } else {
      // yes, previously processed an operation for this record
      ...
      if (prev_operation.operation_ == CollectionOperation::kRemove) {
        switch (unique_constraint_) {
          case UniqueConstraint::kMakeUnique:
            ...
            if (debug) {
              Log(InfoLogLevel::DEBUG_LEVEL, logger,
                  "encountered new record for kRemove which is a duplicate of "
                  "another new record, skipping as "
                  "UniqueConstraint::kMakeUnique is set. (value=%s)",
                  record.ToString(true).c_str());
            }
          ...
          case UniqueConstraint::kEnforceUnique:
            ...
            Log(InfoLogLevel::ERROR_LEVEL, logger,
                "encountered new record for kRemove which is a duplicate of "
                "another new record, but UniqueConstraint::kEnforceUnique is "
                "set. (value=%s)",
                record.ToString(true).c_str());
            return false;  // PartialMergeMulti failed!
            break;
...
void CollectionMergeOperator::fm_clear(std::string& existing, Logger* logger,
                                       const bool debug) const {
  if (debug) {
...
bool CollectionMergeOperator::fm_add(const char* value, const size_t value_len,
                                     std::string& existing, Logger* logger,
                                     const bool debug) const {
  if (debug) {
    Log(InfoLogLevel::DEBUG_LEVEL, logger,
        "Processing kAdd(existing=[%s], value=[%s])",
        toHex(existing.c_str(), existing.length()).c_str(),
        toHex(value, value_len).c_str());
  }

  // check we have a whole number of records in the new value
  if (value_len % fixed_record_len_ > 0) {
    Log(InfoLogLevel::ERROR_LEVEL, logger,
        "add encountered incomplete records in the new value (value_len=%zu, "
        "fixed_record_len_=%" PRIu16 ")",
        value_len, fixed_record_len_);
    return false;
  }
  ...
  if (existing.size() % fixed_record_len_ > 0) {
    Log(InfoLogLevel::ERROR_LEVEL, logger,
        "add encountered incomplete records in the existing Collection "
        "(existing.size=%zu, fixed_record_len_=%" PRIu16 ")",
        existing.size(), fixed_record_len_);
    return false;
  }
...
bool CollectionMergeOperator::insert(const char* value, const size_t value_len,
                                     std::string& existing, Logger* logger,
                                     const bool debug) const {
  ...
  for (size_t i = 0; i < value_records_len; i++) {
    ...
    // TODO(AR) future optimisation: inform next insert_record based on where
    // the previous insert_record did insertion (optimisation for value that has
    // records pre-ordered by comparator)
...
bool CollectionMergeOperator::insert_record(Slice& value_record,
                                            std::string& existing,
                                            Logger* logger,
                                            const bool debug) const {
  ...
  // TODO(AR) future optimisation: rather than iterating existing records from
  // start to finish, we could do a binary search!
  ...
  for (size_t i = 0; i < existing_records_len; i++) {
    ...
    const int64_t comparison =
        comparator_->Compare(existing_record, value_record);
    ...
    if (comparison > 0) {
      ...
      switch (unique_constraint_) {
        case kMakeUnique:
          ...
          if (debug) {
            Log(InfoLogLevel::DEBUG_LEVEL, logger,
                "insert_record encountered existing record which is the same "
                "as the new record, skipping insert_record as "
                "UniqueConstraint::kMakeUnique is set. (value=%s)",
                toHex(value_record.data(), fixed_record_len_).c_str());
          }
          return true;  // insert_record effectively succeeded!

        case kEnforceUnique:
          // new record is a duplicate of an existing record, this is not
          // allowed!
          Log(InfoLogLevel::ERROR_LEVEL, logger,
              "insert_record encountered existing record which is the same as "
              "the new record, but UniqueConstraint::kEnforceUnique is set. "
              "(value=%s)",
              toHex(value_record.data(), fixed_record_len_).c_str());
        ...
        default:
          // insert before existing_record
          const size_t insert_offset = i * fixed_record_len_;
          existing.insert(insert_offset, value_record.data(),
                          fixed_record_len_);
          return true;
      }
    }
  ...
  // we have reached the end of the existing records, and not yet inserted, so
  // we append the new record
  existing.append(value_record.data(), fixed_record_len_);
  return true;
}

bool CollectionMergeOperator::append(const char* value, const size_t value_len,
                                     std::string& existing, Logger* logger,
                                     const bool debug) const {
    ...
    switch (unique_constraint_) {
      case kMakeUnique:
        if (exists(value_record, existing)) {
          ...
          if (debug) {
            Log(InfoLogLevel::DEBUG_LEVEL, logger,
                "append encountered existing record which is the same as the "
                "new record, skipping insert_record as "
                "UniqueConstraint::kMakeUnique is set. (value=%s)",
                toHex(value_record.data(), fixed_record_len_).c_str());
          }
        } else {
      ...
      case kEnforceUnique:
        if (exists(value_record, existing)) {
          ...
          // allowed!
          Log(InfoLogLevel::ERROR_LEVEL, logger,
              "apppend encountered existing record which is the same as the "
              "new record, but UniqueConstraint::kEnforceUnique is set. "
              "(value=%s)",
              toHex(value_record.data(), fixed_record_len_).c_str());
...
bool CollectionMergeOperator::exists(Slice& value_record,
...
bool CollectionMergeOperator::fm_remove(const char* value,
                                        const size_t value_len,
                                        std::string& existing, Logger* logger,
                                        const bool debug) const {
  if (debug) {
    Log(InfoLogLevel::DEBUG_LEVEL, logger,
        "Processing kRemove(existing=[%s], value=[%s])",
        toHex(existing.c_str(), existing.length()).c_str(),
        toHex(value, value_len).c_str());
  }
  ...
  if (value_len % fixed_record_len_ > 0) {
    Log(InfoLogLevel::ERROR_LEVEL, logger,
        "remove encountered incomplete records in the new value "
        "(value_len=%zu, fixed_record_len_=%" PRIu16 ")",
        value_len, fixed_record_len_);
    return false;
  }
  ...
  if (existing.size() % fixed_record_len_ > 0) {
    Log(InfoLogLevel::ERROR_LEVEL, logger,
        "remove encountered incomplete records in the existing Collection "
        "(existing.size=%zu, fixed_record_len_=%" PRIu16 ")",
        existing.size(), fixed_record_len_);
    return false;
  }
  ...
  for (size_t i = 0; i < value_records_len; i++) {
    ...
    // loop through existing records, remove the first that matches the record
    // to remove
    const char* existing_ptr = existing.data();
...
void CollectionMergeOperator::fm_trace_start(
    const MergeOperationInput& merge_in,
    MergeOperationOutput* merge_out) const {
  ...
  if (merge_in.existing_value == nullptr) {
    ...
    trace_file << "[" << merge_in.existing_value->ToString(true) << "]"
               << std::endl;
  }
  trace_file << "* new_value: ["
             << toHex(merge_out->new_value.c_str(), merge_out->new_value.size())
             << "]" << std::endl;
  ...
  trace_file << "]" << std::endl;
  size_t i = 0;
  trace_file << "* operand_list(size=" << merge_in.operand_list.size() << "): {"
             << std::endl;
...
void CollectionMergeOperator::fm_trace_start_process_op(
    Slice* slice, std::ofstream& trace_file) const {
...
void CollectionMergeOperator::fm_trace_start_records(
    Slice* slice, const size_t len, std::ofstream& trace_file) const {
...
void CollectionMergeOperator::fm_trace_end(
    MergeOperationOutput* merge_out) const {
  std::ofstream trace_file;
  trace_file.open(TRACE_FILE, std::ofstream::out | std::ofstream::app);
  ...
  for (size_t i = 0; i < merge_out->new_value.size(); i += fixed_record_len_) {
    trace_file << trace_record_serializer_(merge_out->new_value.data(), i,
                                           fixed_record_len_);
...
void CollectionMergeOperator::pm_trace_start(
    const Slice& key, const std::deque<Slice>& operand_list,
    std::string* new_value) const {
  std::ofstream trace_file;
  trace_file.open(TRACE_FILE, std::ofstream::out | std::ofstream::app);
  ...
  trace_file << "* new_value: [" << toHex(new_value->c_str(), new_value->size())
             << "]" << std::endl;
  size_t i = 0;
  trace_file << "* operand_list(size=" << operand_list.size() << "): {"
             << std::endl;
  for (auto it = operand_list.begin(); it != operand_list.end(); ++it) {
    ...
    if (slice.size() > 0) {
      ...
      for (size_t j = 0; j < slice.size(); j += fixed_record_len_) {
        trace_file << trace_record_serializer_(slice.data(), j,
                                               fixed_record_len_);
...
void CollectionMergeOperator::pm_trace_end(std::string* new_value) const {
  ...
  if (new_value == nullptr) {
    ...
    trace_file << "* new_value(size=" << new_value->size() << ")=["
               << std::endl;
...
void CollectionMergeOperator::pm_trace_end_process_op(
    std::string* new_value, size_t& i, std::ofstream& trace_file) const {
...
void CollectionMergeOperator::pm_trace_end_records(
    std::string* new_value, size_t& i, std::ofstream& trace_file) const {
  while (i < new_value->size()) {
    trace_file << trace_record_serializer_(new_value->data(), i,
                                           fixed_record_len_);
...
void CollectionMergeOperator::trace_exit(const char* const msg,
                                         const bool success) const {
  std::ofstream trace_file;
  trace_file.open(TRACE_FILE, std::ofstream::out | std::ofstream::app);
  trace_file << "## EXIT(success=" << std::boolalpha << success << "): " << msg
             << std::endl;
  trace_file << std::endl;
  trace_file.close();
}
