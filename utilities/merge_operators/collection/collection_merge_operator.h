...
class CollectionMergeOperator : public MergeOperator {
 public:
   ...
   */
  CollectionMergeOperator(
      const uint16_t fixed_record_len, const Comparator* comparator = nullptr,
      ...
      const std::function<std::string(const char* const, size_t, const size_t)>
          trace_record_serializer =
              [](const char* const str, size_t offset, const size_t len) {
                return CollectionMergeOperator::toHex(str + offset, len);
              });
 ...
 private:
  ...
  const bool trace_;
  const std::function<std::string(const char* const, size_t, const size_t)>
      trace_record_serializer_;

  static constexpr const char* const TRACE_FILE =
      "/tmp/CollectionMergeOperator.trace";
   ...
   */
  struct Operation {
    Operation(CollectionOperation operation,
              std::initializer_list<Slice> records)
        : operation_(operation), records_(records) {}
    CollectionOperation operation_;
    std::vector<Slice> records_;
  };

  // NOTE: the std::vector in Operations and RecordsOffsets is used as a stack
  // with elements pushed to/or popped from the end
  ...
  using OperationsIndex = std::unordered_map<Slice, OperationRecordOffsets,
                                             SliceHasher, SliceComparator>;
  ...
  bool insert(const char* value, const size_t value_len, std::string& existing,
              Logger* logger, const bool debug) const;
  bool insert_record(Slice& value_record, std::string& existing, Logger* logger,
  ...
  bool fm_remove(const char* value, const size_t value_len,
  ...
  bool pm_add(Operations& operations, OperationsIndex& operations_index,
              const char* const records_ptr, const size_t records_len,
              Logger* logger, const bool debug) const;
  bool pm_remove(Operations& operations, OperationsIndex& operations_index,
                 const char* const records_ptr, const size_t records_len,
                 Logger* logger, const bool debug) const;
  void push_or_mergeprev_operation_record(
  ...
  void remove_operation_record(
      Operations& operations, OperationsIndex& operations_index,
      const OperationsIndex::iterator& it_operation_idx,
      const OperationRecordOffsets::reverse_iterator&
          it_operation_record_offsets,
  ...
  void pm_trace_start(const Slice& key, const std::deque<Slice>& operand_list,
