...
class CollectionMergeOperator : public MergeOperator {
 public:
  ...
  /**
   * Converts a uint32_t to a byte string.
   *
   * Uses big endian format.
   *
   * @param uint32_t i the integer to convert
   * @return the byte string
   */
  static std::string uintToStringBE(const uint32_t i) {
    char buf[4];
    buf[3] = ((i >> 0) & 0xff);
    buf[2] = ((i >> 8) & 0xff);
    buf[1] = ((i >> 16) & 0xff);
   ...
   * @return the byte string
   */
  static std::string uintToStringLE(const uint32_t i) {
    char buf[4];
    buf[0] = ((i >> 0) & 0xff);
    buf[1] = ((i >> 8) & 0xff);
    buf[2] = ((i >> 16) & 0xff);
  ...
  /**
   * Converts a byte_string to a uint32_t.
   *
   * Uses big endian format.
   *
   * @param the byte string to convert
   * @return the integer
   */
  static uint32_t uintFromStringBE(const std::string &str, size_t offset) {
    uint32_t i = str[offset + 3] & 0xff;
    i |= (str[offset + 2] & 0xff) << 8;
    i |= (str[offset + 1] & 0xff) << 16;
    i |= (str[offset] & 0xff) << 24;
    offset+=4;
    return i;
  }
   ...
   * @return the integer
   */
  static uint32_t uintFromStringBE(const char* str, size_t offset) {
    uint32_t i = str[offset + 3] & 0xff;
    i |= (str[offset + 2] & 0xff) << 8;
    i |= (str[offset + 1] & 0xff) << 16;
    i |= (str[offset] & 0xff) << 24;
   ...
   * @return the integer
   */
  static uint32_t uintFromStringLE(const std::string &str, size_t offset) {
    uint32_t i = str[offset++] & 0xff;
    i |= (str[offset++] & 0xff) << 8;
    i |= (str[offset++] & 0xff) << 16;
    i |= (str[offset++] & 0xff) << 24;
    return i;
  }
   ...
   * @return the integer
   */
  static uint32_t uintFromStringLE(const char* str, size_t offset) {
    uint32_t i = str[offset++] & 0xff;
    i |= (str[offset++] & 0xff) << 8;
    i |= (str[offset++] & 0xff) << 16;
    i |= (str[offset++] & 0xff) << 24;
