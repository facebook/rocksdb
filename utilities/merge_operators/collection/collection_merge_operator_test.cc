...
bool test_FullMergeV2(
    const std::vector<std::string>& str_operand_list,
  ...
  TestLogger logger;
  MergeOperator::MergeOperationInput merge_in(key, existing_value, operand_list,
                                              &logger);

  // TODO(AR) adjust when we support variable record size
  CollectionMergeOperator collection_merge(DEFAULT_TEST_RECORD_SIZE, comparator,
                                           unique_constraint);
  const bool result =
      collection_merge.FullMergeV2(merge_in, &(owning_merge_out.merge_out));
  return result;
}

bool test_PartialMergeMulti(
    const std::vector<std::string>& str_operand_list, std::string* new_value,
    const Comparator* comparator = nullptr,
    const UniqueConstraint unique_constraint = UniqueConstraint::kNone,
    const Slice& key = Slice("key1", 4)) {
  // input
  ...
  CollectionMergeOperator collection_merge(DEFAULT_TEST_RECORD_SIZE, comparator,
                                           unique_constraint);
  const bool result =
      collection_merge.PartialMergeMulti(key, operand_list, new_value, &logger);
  return result;
}
...
TEST(FullMergeV2, EmptyOperandList) {
  ...
  const bool result =
      test_FullMergeV2(operand_list, merge_out, &existing_value);
...
TEST(FullMergeV2, Clear_ExistingValue_KeepFollowing) {
  auto new_records = {RecordString(DEFAULT_TEST_RECORD_SIZE),
                      RecordString(DEFAULT_TEST_RECORD_SIZE)};
  auto operand_list = {Clear(), Add(new_records)};
...
TEST(FullMergeV2, Add) {
  ...
  auto operand_list = {Add({new_record})};
...
TEST(FullMergeV2, Add_Ordered) {
  auto operand_list = {Add("3000"), Add("1000"), Add("5000"),
                       Add("2000"), Add("7000"), Add("6000")};
  ...
  const bool result =
      test_FullMergeV2(operand_list, merge_out, nullptr, comparator);
...
TEST(FullMergeV2, Add_Ordered_uint32) {
  ...
  if (port::kLittleEndian) {
    ASSERT_EQ(Add({7000, 6000, 5000, 3000, 2000, 1000}).substr(1),
              merge_out.new_value);
  } else {
    ASSERT_EQ(Add({1000, 2000, 3000, 5000, 6000, 7000}).substr(1),
              merge_out.new_value);
  }
  ASSERT_EQ(0, merge_out.existing_operand.size());
}
...
TEST(FullMergeV2, Add_Existing_Ordered) {
  ...
  const bool result =
      test_FullMergeV2(operand_list, merge_out, &existing_value, comparator);
...
TEST(FullMergeV2, Add_Existing_Duplicate_Constraint_EnforceUnique_NoConflict) {
  auto operand_list = {Add("2000")};
  Slice existing_value("1000");

  auto merge_out = EmptyMergeOut();

  const bool result =
      test_FullMergeV2(operand_list, merge_out, &existing_value, nullptr,
                       UniqueConstraint::kEnforceUnique);
...
TEST(FullMergeV2, AddMany) {
  auto operand_list = {Add({"2000", "4000", "6000", "8000", "9000"})};
...
TEST(FullMergeV2, AddMany_Operands) {
  auto operand_list = {Add("2000"), Add("4000"), Add("6000"), Add("8000"),
                       Add("9000")};
...
TEST(FullMergeV2, AddMany_Ordered) {
  auto operand_list = {Add({"9000", "4000", "8000", "6000", "2000"})};
  auto* comparator = ROCKSDB_NAMESPACE::BytewiseComparator();
...
TEST(FullMergeV2, AddMany_Operands_Ordered) {
  auto operand_list = {Add("9000"), Add("4000"), Add("8000"), Add("6000"),
                       Add("2000")};
  auto* comparator = ROCKSDB_NAMESPACE::BytewiseComparator();
...
TEST(FullMergeV2, AddMany_ExistingValue_Ordered) {
  auto operand_list = {Add({"8000", "4000", "2000", "6000", "9000"})};
  Slice existing_value("5000");
  auto* comparator = ROCKSDB_NAMESPACE::BytewiseComparator();
...
TEST(FullMergeV2, AddMany_Operands_ExistingValue_Ordered) {
  auto operand_list = {Add("8000"), Add("4000"), Add("2000"), Add("6000"),
                       Add("9000")};
  Slice existing_value("5000");
  auto* comparator = ROCKSDB_NAMESPACE::BytewiseComparator();
...
TEST(FullMergeV2, AddMany_Constraint_MakeUnique_NoConflict) {
  auto operand_list = {Add({"1000", "2000", "3000"})};

  auto merge_out = EmptyMergeOut();

  const bool result = test_FullMergeV2(operand_list, merge_out, nullptr,
                                       nullptr, UniqueConstraint::kMakeUnique);
...
TEST(FullMergeV2, AddMany_Constraint_MakeUnique_Conflict) {
  auto operand_list = {Add({"1000", "2000", "1000", "3000"})};
...
TEST(FullMergeV2, AddMany_Operands_Constraint_MakeUnique_NoConflict) {
  auto operand_list = {Add("1000"), Add("2000"), Add("1000"), Add("3000")};
...
TEST(FullMergeV2, RemoveMany) {
  auto operand_list = {Remove({"1000", "4000"})};
...
TEST(FullMergeV2, RemoveMany_Operands) {
  auto operand_list = {Remove("1000"), Remove("4000")};
  Slice existing_value("10002000300040005000");
...
TEST(FullMergeV2, RemoveAll) {
  auto operand_list = {Remove({"1000", "2000", "3000"})};
  Slice existing_value("100020003000");
...
TEST(FullMergeV2, RemoveAll_Operands) {
  auto operand_list = {Remove("1000"), Remove("2000"), Remove("3000")};
  Slice existing_value("100020003000");
...
TEST(FullMergeV2, RemoveMany_Unordered) {
  auto operand_list = {Remove({"4000", "1000"})};
  Slice existing_value("10002000300040005000");
...
TEST(FullMergeV2, RemoveMany_Operands_Unordered) {
  auto operand_list = {Remove("4000"), Remove("1000")};
  Slice existing_value("10002000300040005000");
...
TEST(FullMergeV2, RemoveAll_Operands_Unordered) {
  auto operand_list = {Remove("4000"), Remove("1000"), Remove("2000"),
                       Remove("5000"), Remove("3000")};
  Slice existing_value("10002000300040005000");
...
TEST(FullMergeV2, AddAndRemove_SingleRemoveOperand) {
  auto operand_list = {Add("1000"),   Add("2000"), Remove({"4000", "1000"}),
                       Add("3000"),   Add("6000"), Add("7000"),
                       Remove("2000")};
...
TEST(FullMergeV2, AddAndRemove_MultiRemoveOperand) {
  auto operand_list = {Add("1000"),    Add("2000"),   Remove("4000"),
                       Remove("1000"), Add("3000"),   Add("6000"),
                       Add("7000"),    Remove("2000")};
...
TEST(FullMergeV2, Multi_ClearAdd) {
  auto operand_list = {Multi({Clear(), Add("1000")})};
...
TEST(FullMergeV2, Multi_RemoveAdd) {
  auto operand_list = {Multi({Remove("1000"), Add("1000")})};
...
TEST(FullMergeV2, Multi_RemoveRemoveAdd) {
  auto operand_list = {Multi({Remove({"1000", "1000"}), Add("1000")})};
...
TEST(FullMergeV2, Multi_Mixed_1) {
  auto operand_list = {
      Multi({Add({"2000", "3000", "4000"}), Remove("6000"),
             Add({"7000", "8000"}), Remove("9999")}),
  };
...
TEST(FullMergeV2, Add_MultiRemoveAdd) {
  auto operand_list = {Add({"0594", "0593", "0592"}),
                       Multi({
                           Remove({"0592", "0593", "0594"}),
                           Add({"0594", "0592", "0593"}),
                       })};
  auto merge_out = EmptyMergeOut();
...
TEST(FullMergeV2, Add_MultiRemoveAdd_uint32) {
  auto operand_list = {Add({594, 593, 592}), Multi({
                                                 Remove({592, 593, 594}),
                                                 Add({594, 592, 593}),
                                             })};
  auto merge_out = EmptyMergeOut();
...
TEST(FullMergeV2, EmptyOperands) {
  std::string empty1;
  std::string empty2;
  auto operand_list = {empty1, empty2};
...
TEST(FullMergeV2, EmptyOperandsAndAdd) {
  std::string empty1;
  std::string empty2;
  auto operand_list = {empty1, Add({"1000", "2000"}), empty2};
...
TEST(PartialMergeMulti, AddAdd) {
  auto operand_list = {Add({"1000", "2000"})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"1000", "2000"}), new_value);
}

TEST(PartialMergeMulti, AddAdd_Operands) {
  auto operand_list = {Add({"1000", "3000"}), Add({"2000", "4000"})};
  ...
  */
  ASSERT_EQ(Add({"1000", "3000", "2000", "4000"}), new_value);
}
...
TEST(PartialMergeMulti, AddAddAdd) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"1000", "2000", "3000"}), new_value);
}

TEST(PartialMergeMulti, AddAddAdd_Operands) {
  auto operand_list = {Add({"1000", "2000"}), Add("3000"),
                       Add({"4000", "5000"})};
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"1000", "2000", "3000", "4000", "5000"}), new_value);
}

TEST(PartialMergeMulti, AddAdd_Duplicate_Constraint_None) {
  auto operand_list = {Add({"1000", "1000"})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"1000", "1000"}), new_value);
}

TEST(PartialMergeMulti, AddAdd_Duplicate_Operands_Constraint_None) {
  auto operand_list = {Add("1000"), Add("1000")};
...
TEST(PartialMergeMulti, AddAdd_Duplicate_Constraint_EnforceUnique_NoConflict) {
  ...
  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value, nullptr,
                                             UniqueConstraint::kEnforceUnique);
...
TEST(PartialMergeMulti,
     AddAdd_Duplicate_Operands_Constraint_EnforceUnique_NoConflict) {
  auto operand_list = {Add("1000"), Add("2000")};
...
TEST(PartialMergeMulti,
     AddAdd_Duplicate_Operands_Constraint_EnforceUnique_Conflict) {
  auto operand_list = {Add("1000"), Add("1000")};
...
TEST(PartialMergeMulti, AddAdd_Duplicate_Constraint_MakeUnique_Conflict) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"1000"}), new_value);
}

TEST(PartialMergeMulti,
     AddAdd_Duplicate_Operands_Constraint_MakeUnique_NoConflict) {
  auto operand_list = {Add("1000"), Add("2000")};
...
TEST(PartialMergeMulti,
     AddAdd_Duplicate_Operands_Constraint_MakeUnique_Conflict) {
  auto operand_list = {Add("1000"), Add("1000")};
...
TEST(PartialMergeMulti, AddClear) {
  auto operand_list = {Add("1000"), Clear()};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Clear(), new_value);
}

TEST(PartialMergeMulti, ClearAdd) {
  auto operand_list = {Clear(), Add("1000")};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Clear(), Add("1000")}), new_value);
}

TEST(PartialMergeMulti, AddClearAdd) {
  auto operand_list = {Add("1000"), Clear(), Add("3000")};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Clear(), Add("3000")}), new_value);
}

TEST(PartialMergeMulti, ClearAddAdd) {
  auto operand_list = {Clear(), Add("1000"), Add("3000")};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Clear(), Add({"1000", "3000"})}), new_value);
}

TEST(PartialMergeMulti, AddAddClear) {
  auto operand_list = {Add("1000"), Add("3000"), Clear()};
...
TEST(PartialMergeMulti, RemoveRemove) {
  auto operand_list = {Remove({"1000", "2000"})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Remove({"1000", "2000"}), new_value);
}

TEST(PartialMergeMulti, RemoveRemove_Operands) {
  auto operand_list = {Remove("1000"), Remove("2000")};
...
TEST(PartialMergeMulti, RemoveRemoveRemove) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Remove({"1000", "2000", "3000"}), new_value);
}
...
TEST(PartialMergeMulti, RemoveRemoveRemove_Operands) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Remove({"1000", "2000", "3000", "4000", "5000"}), new_value);
}

TEST(PartialMergeMulti, RemoveRemove_Duplicate_Constraint_None) {
  auto operand_list = {Remove({"1000", "1000"})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Remove({"1000", "1000"}), new_value);
}

TEST(PartialMergeMulti, RemoveRemove_Duplicate_Operands_Constraint_None) {
  auto operand_list = {Remove("1000"), Remove("1000")};
...
TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Constraint_EnforceUnique_NoConflict) {
  auto operand_list = {Remove({"1000", "2000"})};
...
TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Constraint_EnforceUnique_Conflict) {
  auto operand_list = {Remove({"1000", "1000"})};
...
TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Operands_Constraint_EnforceUnique_NoConflict) {
  auto operand_list = {Remove("1000"), Remove("2000")};
...
TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Operands_Constraint_EnforceUnique_Conflict) {
  auto operand_list = {Remove("1000"), Remove("1000")};
...
TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Constraint_MakeUnique_NoConflict) {
  auto operand_list = {Remove({"1000", "2000"})};
...
TEST(PartialMergeMulti, RemoveRemove_Duplicate_Constraint_MakeUnique_Conflict) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Remove({"1000"}), new_value);
}

TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Operands_Constraint_MakeUnique_NoConflict) {
  auto operand_list = {Remove("1000"), Remove("2000")};
...
TEST(PartialMergeMulti,
     RemoveRemove_Duplicate_Operands_Constraint_MakeUnique_Conflict) {
  auto operand_list = {Remove("1000"), Remove("1000")};
...
TEST(PartialMergeMulti, AddRemove) {
  auto operand_list = {Add("1000"), Remove("1000")};
...
TEST(PartialMergeMulti, RemoveAdd) {
  auto operand_list = {Remove("1000"), Add("1000")};
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Remove("1000"), Add("1000")}), new_value);
}

// [Add(1000, 1000), Remove(1000)] => Add(1000)
TEST(PartialMergeMulti, AddAddRemove) {
  auto operand_list = {Add({"1000", "1000"}), Remove("1000")};
...
TEST(PartialMergeMulti, AddAddRemove_Operands) {
  auto operand_list = {Add("1000"), Add("1000"), Remove("1000")};
...
TEST(PartialMergeMulti, RemoveRemoveAdd) {
  auto operand_list = {Remove({"1000", "1000"}), Add("1000")};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Remove({"1000", "1000"}), Add("1000")}), new_value);
}

// [Remove(1000), Remove(1000), Add(1000)] => Multi(Remove(1000, 1000),
// Add(1000))
TEST(PartialMergeMulti, RemoveRemoveAdd_Operands) {
  auto operand_list = {Remove("1000"), Remove("1000"), Add("1000")};
...
TEST(PartialMergeMulti, RemoveAddRemove_Operands) {
  auto operand_list = {Remove("1000"), Add("1000"), Remove("1000")};
...
TEST(PartialMergeMulti, AddRemoveAdd_Operands) {
  auto operand_list = {Add("1000"), Remove("1000"), Add("1000")};
...
TEST(PartialMergeMulti, AddRemoveRemove) {
  auto operand_list = {Add("1000"), Remove({"1000", "1000"})};
...
TEST(PartialMergeMulti, AddRemoveRemove_Operands) {
  auto operand_list = {Add("1000"), Remove("1000"), Remove("1000")};
...
TEST(PartialMergeMulti, RemoveAddAdd) {
  auto operand_list = {Remove("1000"), Add({"1000", "1000"})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Remove("1000"), Add({"1000", "1000"})}), new_value);
}

// [Remove(1000), Add(1000), Add(1000)] => Multi(Remove(1000), Add(1000, 1000))
TEST(PartialMergeMulti, RemoveAddAdd_Operands) {
  auto operand_list = {Remove("1000"), Add("1000"), Add("1000")};
...
// [Remove(1000), Add(1000, 1000), Remove(1000)] => Multi(Remove(1000),
// Add(1000))
TEST(PartialMergeMulti, RemoveAddAddRemove) {
  auto operand_list = {Remove("1000"), Add({"1000", "1000"}), Remove("1000")};
...
// [Remove(1000), Add(1000), Add(1000), Remove(1000)] => Multi(Remove(1000),
// Add(1000))
...
TEST(PartialMergeMulti, AddRemoveRemoveAdd) {
  auto operand_list = {Add("1000"), Remove({"1000", "1000"}), Add("1000")};
...
// [Add(1000), Remove(1000), Remove(1000), Add(1000)] => Multi(Remove(1000),
// Add(1000))
...
TEST(PartialMergeMulti, AddAddRemoveRemove) {
  auto operand_list = {Add({"1000", "1000"}), Remove({"1000", "1000"})};
...
// [Remove(1000, 1000), Add(1000, 1000)] => Multi(Remove(1000, 1000), Add(1000,
// 1000))
TEST(PartialMergeMulti, RemoveRemoveAddAdd) {
  auto operand_list = {Remove({"1000", "1000"}), Add({"1000", "1000"})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Remove({"1000", "1000"}), Add({"1000", "1000"})}),
            new_value);
}

// [Remove(1000), Remove(1000), Add(1000), Add(1000)] => Multi(Remove(1000,
// 1000), Add(1000, 1000))
...
TEST(PartialMergeMulti, AddAddRemoveRemove_Mixed_1) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"2000", "3000"}), new_value);
}

// [Add(1000), Add(2000), Add(1000), Add(3000), Remove(1000), Remove(1000)] =>
// [Add(2000, 3000)]
TEST(PartialMergeMulti, AddAddRemoveRemove_Operands_Mixed_1) {
  auto operand_list = {Add("1000"), Add("2000"),    Add("1000"),
                       Add("3000"), Remove("1000"), Remove("1000")};
...
// [Add(1000, 2000, 1000, 3000), Add(4000, 5000), Remove(1000, 1000),
// Remove(4000, 5000)] => [Add(2000, 3000)]
TEST(PartialMergeMulti, AddAddRemoveRemove_Mixed_2) {
  auto operand_list = {Add({"1000", "2000", "1000", "3000"}),
                       Add({"4000", "5000"}), Remove({"1000", "1000"}),
                       Remove({"4000", "5000"})};
...
// [Add(1000), Add(2000), Add(1000), Add(3000), Add(4000), Add(5000),
// Remove(1000), Remove(1000), Remove(4000), Remove(5000)] => [Add(2000, 3000)]
TEST(PartialMergeMulti, AddAddRemoveRemove_Operands_Mixed_2) {
  auto operand_list = {Add("1000"),    Add("2000"),    Add("1000"),
                       Add("3000"),    Add("4000"),    Add("5000"),
                       Remove("1000"), Remove("1000"), Remove("4000"),
                       Remove("5000")};
...
// [Add(1000, 2000, 1000, 3000), Add(4000, 5000), Remove(1000, 1000),
// Remove(5000)] => [Add(2000, 3000, 4000)]
TEST(PartialMergeMulti, AddAddRemoveRemove_Mixed_3) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"2000", "3000", "4000"}), new_value);
}

// [Add(1000), Add(2000), Add(1000), Add(3000), Add(4000), Add(5000),
// Remove(1000), Remove(1000), Remove(5000)] => [Add(2000, 3000, 4000)]
TEST(PartialMergeMulti, AddAddRemoveRemove_Operands_Mixed_3) {
  auto operand_list = {Add("1000"),    Add("2000"),    Add("1000"),
                       Add("3000"),    Add("4000"),    Add("5000"),
                       Remove("1000"), Remove("1000"), Remove("5000")};
...
// [Add(1000, 2000, 1000, 3000), Remove(1000, 1000), Add(4000, 5000),
// Remove(4000, 5000)] => [Add(2000, 3000)]
TEST(PartialMergeMulti, AddRemoveAddRemove_Mixed_1) {
  auto operand_list = {Add({"1000", "2000", "1000", "3000"}),
                       Remove({"1000", "1000"}), Add({"4000", "5000"}),
                       Remove({"4000", "5000"})};
...
// [Add(1000), Add(2000), Add(1000), Add(3000), Remove(1000), Remove(1000),
// Add(4000), Add(5000), Remove(4000, 5000)] => [Add(2000, 3000)]
TEST(PartialMergeMulti, AddRemoveAddRemove_Operands_Mixed_1) {
  auto operand_list = {Add("1000"),    Add("2000"),    Add("1000"), Add("3000"),
                       Remove("1000"), Remove("1000"), Add("4000"), Add("5000"),
                       Remove("4000"), Remove("5000")};
...
// [Add(1000), Add(2000), Add(1000), Add(3000), Remove(1000), Remove(1000),
// Add(4000), Add(5000), Remove(5000)] => [Add(2000, 3000, 4000)]
TEST(PartialMergeMulti, AddRemoveAddRemove_Operands_Mixed_2) {
  auto operand_list = {Add("1000"), Add("2000"),    Add("1000"),
                       Add("3000"), Remove("1000"), Remove("1000"),
                       Add("4000"), Add("5000"),    Remove("5000")};
...
// [Add(1000, 2000, 1000, 3000), Remove(1000, 1000), Add(4000, 5000),
// Remove(5000, 6000), Add(7000, 8000, 9000), Remove(9999, 9000)] =>
// [Multi(Add(2000, 3000, 4000), Remove("6000"), Add("7000", "8000"),
// Remove("9999"))]
TEST(PartialMergeMulti, AddRemoveAddRemove_Mixed_3) {
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Add({"2000", "3000", "4000"}), Remove("6000"),
                   Add({"7000", "8000"}), Remove("9999")}),
            new_value);
}

// [Add(1000), Add(2000), Add(1000), Add(3000), Remove(1000), Remove(1000),
// Add(4000), Add(5000), Remove(5000), Remove(6000), Add(7000, 8000), Add(9000),
// Remove(9999), Remove(9000)] => [Multi(Add(2000, 3000, 4000), Remove("6000"),
// Add("7000", "8000"), Remove("9999"))]
TEST(PartialMergeMulti, AddRemoveAddRemove_Operands_Mixed_3) {
  auto operand_list = {
      Add("1000"),    Add("2000"), Add("1000"), Add("3000"),    Remove("1000"),
      Remove("1000"), Add("4000"), Add("5000"), Remove("5000"), Remove("6000"),
      Add("7000"),    Add("8000"), Add("9000"), Remove("9999"), Remove("9000")};
...
TEST(PartialMergeMulti, Multi_AddAdd) {
  auto operand_list = {
      Multi({Add("1000"), Add("2000")}),
      Multi({Add("3000"), Add("4000")}),
  };
...
TEST(PartialMergeMulti, MultiRemoveAddRemove_1) {
  auto operand_list = {Multi({
                           Remove("1000"),
                       }),
                       Multi({Add("1000")}), Remove("1000")};
...
TEST(PartialMergeMulti, MultiRemoveAddRemove_2) {
  auto operand_list = {Multi({Remove("1000"), Add("1000")}), Remove("1000")};
...
// [Multi(Add(2000), Remove(1000, Add(1000)), Multi(Remove(1000, 2000),
// Add(3000))] => Multi(Remove(1000), Add(3000))
TEST(PartialMergeMulti, MultiComplex1) {
  auto operand_list = {Multi({Add("2000"), Remove("1000"), Add("1000")}),
                       Multi({Remove({"1000", "2000"}), Add("3000")})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Remove("1000"), Add("3000")}), new_value);
}

// [Add(4000), Remove(9000), Add(5000), Multi(Add(2000), Remove(1000,
// Add(1000)), Remove(5000), Multi(Remove(1000, 2000), Add(3000))] =>
// Multi(Remove(1000, 9000), Add(3000, 4000))
TEST(PartialMergeMulti, MultiComplex2) {
  auto operand_list = {
      Add("4000"),    Remove("9000"),
      Add("5000"),    Multi({Add("2000"), Remove("1000"), Add("1000")}),
      Remove("5000"), Multi({Remove({"1000", "2000"}), Add("3000")})};

  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);

  ASSERT_TRUE(result);
  ASSERT_EQ(Multi({Add("4000"), Remove({"9000", "1000"}), Add({"3000"})}),
            new_value);
}

TEST(PartialMergeMulti, Complex1) {
  auto operand_list = {Add({"0594", "0592", "0593"}),
                       Remove("0593"),
                       Remove("0592"),
                       Remove("0594"),
                       Add({"0594", "0593", "0592"}),
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(Add({"0594", "0593", "0592"}), new_value);
}

TEST(PartialMergeMulti, Complex1_uint32) {
  auto operand_list = {
      Add({594, 592, 593}), Remove(593), Remove(592), Remove(594),
      Add({594, 593, 592}), Remove(592), Remove(593), Remove(594),
      Add({594, 592, 593}), Remove(593), Remove(592), Remove(594),
      Add({594, 593, 592}), Remove(592), Remove(593), Remove(594),
      Add({594, 593, 592}), Remove(592), Remove(593), Remove(594),
      Add({594, 592, 593}), Remove(593), Remove(592), Remove(594),
  ...
  std::string new_value;
  const bool result = test_PartialMergeMulti(operand_list, &new_value);
  ASSERT_EQ(Add({594, 593, 592}), new_value);
