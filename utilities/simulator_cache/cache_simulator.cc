...
bool GhostCache::Admit(const Slice& lookup_key) {
  ...
  auto s = sim_cache_->Insert(lookup_key, /*value=*/nullptr,
  ...
  *admitted = true;
  ...
  if (handle != nullptr) {
    ...
    *is_cache_miss = false;
    ...
    auto s = sim_cache_->Insert(key, /*value=*/nullptr, &kNoopCacheItemHelper,
                                value_size,
...
void HybridRowBlockCacheSimulator::Access(const BlockCacheTraceRecord& access) {
  ...
  if (access.caller == TableReaderCaller::kUserGet &&
    ...
    if (access.referenced_data_size > 0 && inserted == InsertResult::ADMITTED) {
      // TODO: Should we check for an error here?
      auto s =
          sim_cache_->Insert(row_key, /*value=*/nullptr, &kNoopCacheItemHelper,
