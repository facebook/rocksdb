...
std::vector<std::shared_ptr<BlobFile>> BlobDBImpl::TEST_GetObsoletedFiles()
    const {
  ReadLock l(&mutex_);
  std::vector<std::shared_ptr<BlobFile>> obsoleted_files;
  for (auto& bfile : obsoleted_files_) {
    obsoleted_files.emplace_back(bfile);
  }
