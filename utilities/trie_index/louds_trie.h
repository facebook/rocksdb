...
class LoudsTrie {
 ...
 private:
  ...
  // Block handles array (one per leaf, delta-varint encoded on disk).
  std::vector<TrieBlockHandle> handles_;
};
...
class LoudsTrieIterator {
 ...
 private:
  ...
  // pushing entries onto path_ and building current_key_. Sets
  ...
  // position at depth i in the trie. The depth equals current_key_.size()
  // when positioned on a child leaf, or current_key_.size() when
  // positioned on a prefix key (in which case the last path_ entry's
  // label is not appended to current_key_ since the node itself is the key).
  //
  ...
  // of the node and current_key_ has the prefix key value.
