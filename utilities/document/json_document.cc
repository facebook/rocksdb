...
void InitJSONDocument(std::unique_ptr<char[]>* data,
  ...
  assert(res);
  uint32_t bytesWritten = f(writer);
...
void InitString(std::unique_ptr<char[]>* data,
  ...
  InitJSONDocument(data, value, std::bind(
      [](fbson::FbsonWriter& writer, const std::string& str) -> uint32_t {
        ...
        assert(res);
        auto bytesWritten = writer.writeString(str.c_str(), str.length());
...
bool Compare(fbson::FbsonValue* left, fbson::FbsonValue* right) {
  ...
  switch (left->type()) {
    ...
    case fbson::FbsonType::T_Array:
    {
      ...
      for (size_t i = 0; i < leftArr->numElem(); ++i) {
...
JSONDocument* JSONDocument::Deserialize(const Slice& src) {
  ...
  auto value = fbson::FbsonDocument::createValue(input.data(), input.size());
  if (value == nullptr) {
    return nullptr;
  }
