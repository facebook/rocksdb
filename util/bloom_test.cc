...
TEST(DraftTest, FilterConstructionChecksum){
  BlockBasedTableOptions table_options;
  table_options.detect_filter_construct_corruption = true;
  for (auto mode : {BloomFilterPolicy::Mode::kFastLocalBloom, BloomFilterPolicy::Mode::kStandard128Ribbon}) {
    table_options.filter_policy.reset(new BloomFilterPolicy(
      FLAGS_bits_per_key, mode));
    FilterBuildingContext ctx(table_options);
    std::unique_ptr<FilterBitsBuilder> filter_bits_builder(
        table_options.filter_policy->GetBuilderWithContext(ctx));
    ...
    Slice filter_content = filter_bits_builder->Finish(&buf);
    ASSERT_TRUE(filter_content.empty());
    Status post_verify_status = filter_bits_builder->PostVerify(filter_content);
    ASSERT_TRUE(post_verify_status.IsNotSupported());
    SyncPoint::GetInstance()->DisableProcessing();
    SyncPoint::GetInstance()->ClearCallBack("XXPH3FilterBitsBuilder::VerifyHashEntriesChecksum::PreVerification");

    filter_bits_builder.reset(
        table_options.filter_policy->GetBuilderWithContext(ctx));

    char key_buffer_2[sizeof(int)];
    for (std::size_t i = 0; i < 1; ++i) {
      filter_bits_builder->AddKey(Key(static_cast<int>(i), key_buffer_2));
    }
    filter_content = filter_bits_builder->Finish(&buf);
    ASSERT_TRUE(!filter_content.empty());

    Slice corrupted_filter_content = Slice("1234" + filter_content.ToString());
    post_verify_status = filter_bits_builder->PostVerify(corrupted_filter_content);
    ASSERT_TRUE(post_verify_status.IsCorruption());

    // TODO-question: filter corrupiton or hash key corruption
    // bool call_back_called = false;
    // const std::string call_back_name = (mode == BloomFilterPolicy::Mode::kFastLocalBloom) ? "FastLocalBloomBitsBuilder::PostVerify::PreVerification" : "Standard128RibbonBitsBuilder::PostVerify::PreVerification";
    // SyncPoint::GetInstance()->SetCallBack(
    //   call_back_name,
    //   [&](void* arg) {
    //     std::deque<uint64_t>* hash_entries = (std::deque<uint64_t>*) arg;
    //     uint64_t first_h = (*hash_entries)[0];
    //     uint64_t corrupted_first_h = first_h ^ (static_cast<uint64_t>(1) << 40);
    //     (*hash_entries)[0] = corrupted_first_h;
    //     call_back_called = true;
    // });
    // SyncPoint::GetInstance()->EnableProcessing();
    // ASSERT_TRUE(call_back_called);
    // SyncPoint::GetInstance()->DisableProcessing();
    // SyncPoint::GetInstance()->ClearCallBack(call_back_name);
  }
}
