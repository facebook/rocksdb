...
#ifndef NDEBUG
#define INSTALL_COROUTINE_STATS_CONTEXT_SCOPE(__read_executor__, __env__) \
  auto* const coroutine_stats_read_executor = (__read_executor__);        \
  ...
  if (IsCoroutineStatsEnabled()) {                                        \
    stats_scope.emplace(CaptureCoroutineStatsConfig(), (__env__));        \
  }
#else
#define INSTALL_COROUTINE_STATS_CONTEXT_SCOPE(__read_executor__, __env__) \
  std::optional<CoroutineStatsContextScope> stats_scope;                  \
