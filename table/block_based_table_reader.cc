...
Cache::Handle* GetEntryFromCache(
    Cache* block_cache, const Slice& key, const BlockBasedTable::Rep* rep,
    Tickers block_cache_miss_ticker, Tickers block_cache_hit_ticker,
  ...
  if (cache_handle != nullptr) {
    ...
    PERF_COUNTER_BY_LEVEL_ADD(block_cache_hit_count, 1,
                              static_cast<uint32_t>(rep->level));
...
Status BlockBasedTable::GetDataBlockFromCache(
  ...
  if (block_cache != nullptr) {
    auto cache_handle = GetEntryFromCache(
        block_cache, block_cache_key, rep,
...
CachableEntry<FilterBlockReader> BlockBasedTable::GetFilter(
  ...
  Cache::Handle* cache_handle = GetEntryFromCache(
      block_cache, key, rep_, BLOCK_CACHE_FILTER_MISS, BLOCK_CACHE_FILTER_HIT,
...
BlockBasedTable::GetUncompressionDict(Rep* rep,
  ...
  auto cache_handle = GetEntryFromCache(
      rep->table_options.block_cache.get(), cache_key, rep,
