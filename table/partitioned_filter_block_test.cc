...
namespace rocksdb {
  ...
  class PartitionedFilterBlockTest : public testing::Test {
    public:
      ...
      PartitionedFilterBlockReader* NewReader(PartitionedFilterBlockBuilder* builder) {
        BlockHandle bh;
        Status status;
        Slice slice;
        do {
      ...
      void CutABlock(PartitionedFilterBlockBuilder* builder, const std::string& user_key) {
      ...
      void CutABlock(PartitionedFilterBlockBuilder* builder, const std::string& user_key, const std::string& next_user_key) {
        // Assuming a block is cut, add an entry to the index
        std::string key = std::string(*InternalKey(user_key, 0, ValueType::kTypeValue).rep());
        std::string next_key = std::string(*InternalKey(next_user_key, 0, ValueType::kTypeValue).rep());
        BlockHandle dont_care_block_handle(1,1);
