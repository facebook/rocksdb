  ...
  if (protection_bytes_per_key > 0 && num_restarts_ > 0) {
    ...
    std::unique_ptr<DataBlockIter> iter{NewDataIterator(
        raw_ucmp, kDisableGlobalSequenceNumber,
         nullptr /* iter */,
        nullptr /* stats */, true /* block_contents_pinned */, true /* user_defined_timestamps_persisted */)};
    if (iter->status().ok()) {
...
void Block::InitializeIndexBlockProtectionInfo(
  ...
  if (num_restarts_ > 0 && protection_bytes_per_key > 0) {
    ...
    std::unique_ptr<IndexBlockIter> iter{NewIndexIterator(
        ...
        value_is_full,
        true /* block_contents_pinned */,  true /* user_defined_timestamps_persisted*/ ,nullptr /* prefix_index */)};
    if (iter->status().ok()) {
      block_restart_interval_ = iter->GetRestartInterval();
    }
    uint32_t num_keys = 0;
