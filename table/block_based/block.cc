...
int IndexBlockIter::CompareBlockKey(uint32_t block_index, const Slice& target) {
  ...
  uint32_t shared, non_shared, value_offset;
  auto* value_offset_ptr = values_section_ ? &value_offset : nullptr;
  const char* key_ptr =
      value_delta_encoded_
          ? DecodeKeyV4()(data_ + region_offset, data_ + restarts_, &shared,
                          &non_shared, value_offset_ptr)
...
bool Block::DecodeFooter(bool use_separated_kv_storage,
                         BlockBasedTableOptions::DataBlockIndexType* index_type,
                         uint32_t* num_restarts, uint32_t* value_offset) {
  const uint32_t footer_size =
      use_separated_kv_storage ? 2 * sizeof(uint32_t) : sizeof(uint32_t);

  if (size() < footer_size) {
    return false;
  }

  uint32_t type_and_num_restarts =
      DecodeFixed32(data() + size() - sizeof(uint32_t));

  if (use_separated_kv_storage) {
    if (value_offset) {
      *value_offset = DecodeFixed32(data() + size() - footer_size);
    }
    UnPackIndexTypeAndNumRestarts(type_and_num_restarts, index_type,
                                  num_restarts);
  } else if (size() > kMaxBlockSizeSupportedByHashIndex) {
    // Block too large for hash index, footer is just num_restarts
    *index_type = BlockBasedTableOptions::kDataBlockBinarySearch;
    *num_restarts = type_and_num_restarts;
  } else {
    UnPackIndexTypeAndNumRestarts(type_and_num_restarts, index_type,
                                  num_restarts);
  }
...
Block::Block(BlockContents&& contents, size_t read_amp_bytes_per_bit,
             Statistics* statistics, bool use_separated_kv_storage,
             uint32_t restart_interval)
    : contents_(std::move(contents)),
      restart_offset_(0),
      num_restarts_(0),
      block_restart_interval_(restart_interval) {
  assert(!use_separated_kv_storage || restart_interval > 0);
  ...
  if (!DecodeFooter(use_separated_kv_storage, &index_type, &num_restarts_,
  ...
  // Calculate restart_offset_ based on index type
  switch (index_type) {
    case BlockBasedTableOptions::kDataBlockBinarySearch:
      restart_offset_ = static_cast<uint32_t>(size) - footer_size -
                        num_restarts_ * sizeof(uint32_t);
      if (restart_offset_ > size - footer_size) {
        size = 0;
      }
      break;
    case BlockBasedTableOptions::kDataBlockBinaryAndHash: {
      if (size < footer_size + sizeof(uint16_t)) {
        size = 0;
        break;
      }
      uint16_t map_offset;
      data_block_hash_index_.Initialize(
          contents_.data.data(), static_cast<uint16_t>(size - footer_size),
          &map_offset);
  ...
  if (use_separated_kv_storage) {
    if (values_section_offset > restart_offset_) {
      size = 0;
    } else {
      values_section_ = data() + values_section_offset;
    }
  }
