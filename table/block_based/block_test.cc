...
TEST_F(BlockTest, SimpleTest) {
  ...
  int count = 0;
  InternalIterator *iter = reader.NewDataIterator(options.comparator, options.comparator,
...
TEST_F(BlockTest, BlockWithReadAmpBitmap) {
  ...
  // Read the block sequentially using Next()
  {
    ...
    // read contents of block sequentially
    size_t read_bytes = 0;
    DataBlockIter *iter = reader.NewDataIterator(options.comparator, options.comparator,
        kDisableGlobalSequenceNumber, nullptr, stats.get());
...
TEST_P(IndexBlockTest, IndexValueEncodingTest) {
  ...
  InternalIteratorBase<IndexValue> *iter = reader.NewIndexIterator(
      ...
      kNullIter, kNullStats, kTotalOrderSeek, includeFirstKey(), kIncludesSeq, kValueIsFull);
  iter->SeekToFirst();
  ...
  iter = reader.NewIndexIterator(options.comparator, options.comparator,
                                 kDisableGlobalSequenceNumber, kNullIter, kNullStats,
