...
void BuiltinIndexFactoryBuilder::PrepareAddEntry(const Slice& last_key,
  ...
  last_internal_key.append(last_key.data(), last_key.size());
  PutFixed64(&last_internal_key, ctx.last_key_tag);

  std::string next_internal_key;
  Slice next_ik;
  const Slice* next_ik_ptr = nullptr;
  if (next_key != nullptr) {
    next_internal_key.append(next_key->data(), next_key->size());
    PutFixed64(&next_internal_key, ctx.first_key_tag);
    next_ik = Slice(next_internal_key);
    next_ik_ptr = &next_ik;
  }

  internal_builder_->PrepareIndexEntry(Slice(last_internal_key), next_ik_ptr,
