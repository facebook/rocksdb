  ...
  if (rep->table_properties && !rep->table_properties->db_session_id.empty()) {
    ...
    if (version_iter == uprops.end()) {
      ...
      if (rep->table_properties->orig_file_number > 0) {
        ...
        // (If this invariant ever changes, we should move to using current
        // DB session ID here).
        db_session_id = rep->table_properties->db_session_id;
      } else {
        // Have to use current session ID and file number
      }
    } else {
      ...
      // Note: Here orig_file_number refers to the nth file generated by an
      // SstFileWriter under the same db_session_id. When orig_file_number is
      // not present, each external SST file got its own unique session id,
      // so we can use a fixed file number under that session id.
      if (rep->table_properties->orig_file_number > 0) {
        file_num = rep->table_properties->orig_file_number;
      } else {
        file_num = 1;
