  ...
  if (rep_->compression_opts.parallel_threads > 1) {
    ...
    for (uint32_t i = 0; i < rep_->compression_opts.parallel_threads; i++) {
      rep_->pc_rep->compress_thread_pool.emplace_back([=] {
        WriteBlocks(*(rep_->compression_ctxs[i]), rep_->verify_ctxs[i].get());
      });
    }
    rep_->pc_rep->write_thread.reset(
        new port::Thread([=] { WriteRawBlocks(); }));
  }
}
...
void BlockBasedTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (IsValueType(value_type)) {
    ...
    if (should_flush) {
      ...
      r->first_key_in_next_block_ptr = &key;
      Flush();
    ...
    if (r->state == Rep::State::kUnbuffered) {
      if (r->compression_opts.parallel_threads > 1) {
        r->pc_rep->keys_ptr->PushBack(key);
      } else {
...
void BlockBasedTableBuilder::Flush() {
  ...
  if (r->compression_opts.parallel_threads > 1 &&
    ...
    r->data_block.Finish();
    r->data_block.SwapAndReset(*(block_rep.data_ptr));

    block_rep.contents = *(block_rep.data_ptr);

    block_rep.compression_type = r->compression_type;

    std::swap(block_rep.keys_ptr, r->pc_rep->keys_ptr);
    r->pc_rep->keys_ptr->Clear();

    if (r->first_key_in_next_block_ptr == nullptr) {
      delete block_rep.first_key_in_next_block_ptr;
      block_rep.first_key_in_next_block_ptr = nullptr;
    } else {
      block_rep.first_key_in_next_block_ptr->assign(
          r->first_key_in_next_block_ptr->data(),
          r->first_key_in_next_block_ptr->size());
    }
...
void BlockBasedTableBuilder::WriteBlocks(CompressionContext& compression_ctx,
  ...
  while (rep_->pc_rep->compress_queue.pop(block_rep)) {
    CompressAndVerifyBlock(block_rep.contents, true, /* is_data_block*/
                           ...
                           *(block_rep.compressed_data_ptr), block_rep.contents,
                           block_rep.compression_type);
    block_rep.slot_ptr->Fill(std::move(block_rep));
  }
}
...
void BlockBasedTableBuilder::WriteRawBlocks() {
  Rep* r = rep_;
  ParallelCompressionRep::BlockRepSlot* slot_ptr;
  ParallelCompressionRep::BlockRep block_rep;
  while (r->pc_rep->write_queue.pop(slot_ptr)) {
    slot_ptr->Take(block_rep);

    for (size_t i = 0; i < block_rep.keys_ptr->Size(); i++) {
      auto& key = (*block_rep.keys_ptr)[i];
      if (r->filter_builder != nullptr) {
        size_t ts_sz =
            r->internal_comparator.user_comparator()->timestamp_size();
        r->filter_builder->Add(ExtractUserKeyAndStripTimestamp(key, ts_sz));
    ...
    if (block_rep.first_key_in_next_block_ptr == nullptr) {
      r->index_builder->AddIndexEntry(&(block_rep.keys_ptr->Back()), nullptr,
                                      r->pending_handle);
    } else {
      Slice first_key_in_next_block =
          Slice(*block_rep.first_key_in_next_block_ptr);
      r->index_builder->AddIndexEntry(&(block_rep.keys_ptr->Back()),
                                      &first_key_in_next_block,
                                      r->pending_handle);
    }
    block_rep.compressed_data_ptr->clear();
    r->pc_rep->block_rep_pool.push(std::move(block_rep));
  }
}
...
void BlockBasedTableBuilder::EnterUnbuffered() {
  ...
  for (size_t i = 0; ok() && i < r->data_block_and_keys_buffers.size(); ++i) {
    ...
    if (r->compression_opts.parallel_threads > 1) {
      ...
      std::swap(*(block_rep.data_ptr), data_block);
      block_rep.contents = *(block_rep.data_ptr);

      block_rep.compression_type = r->compression_type;

      block_rep.keys_ptr->SwapAssign(keys);
...
Status BlockBasedTableBuilder::Finish() {
  ...
  r->first_key_in_next_block_ptr = nullptr;
  Flush();
  if (r->state == Rep::State::kBuffered) {
    EnterUnbuffered();
  }
