...
void BlockBasedTableBuilder::EnterUnbuffered() {
  ...
  // Picked a Mersenne prime because it's easy on the eyes. No idea about its
  // properties.
  //
  // One downside of this approach is the spread will be poor when
  // `kPrimeGeneratorRemainder` is small enough that we pick all samples before
  // cycling through the whole array.
  const uint64_t kPrimeGenerator = (1ull << 61) - 1;
