...
struct BlockBasedTableBuilder::Rep {
  ...
  Rep(const BlockBasedTableOptions& table_opt, const TableBuilderOptions& tbo,
      ...
      : ioptions(tbo.ioptions),
        ...
        verify_dict(),
        state((tbo.compression_type != kNoCompression) ? State::kBuffered
                                                       : State::kUnbuffered),
...
void BlockBasedTableBuilder::EnterUnbuffered() {
  ...
  if (r->compression_type != kNoCompression) {
    ...
    // compression_dict_samples can be empty
    ...
    // final data block flushed, now we can generate dictionary from the
    // samples. OK if compression_dict_samples is empty, we'll just get empty
    // dictionary.
