...
class KeyConvertingIterator : public InternalIterator {
 public:
  ...
  Slice key() const override {
    ...
    Status pik_status = ParseInternalKey(iter_->key(), &parsed_key, true);
    if (!pik_status.ok()) {
      status_ = pik_status;
...
TEST_P(BlockBasedTableTest, RangeDelBlock) {
  ...
  for (int j = 0; j < 2; ++j) {
    ...
    for (size_t i = 0; i < expected_tombstones.size(); i++) {
      ASSERT_TRUE(iter->Valid());
      ParsedInternalKey parsed_key;
      ASSERT_OK(ParseInternalKey(iter->key(), &parsed_key, true));
...
TEST_P(BlockBasedTableTest, DISABLED_TableWithGlobalSeqno) {
  ...
  for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
    ...
    ASSERT_OK(ParseInternalKey(iter->key(), &pik, true));
