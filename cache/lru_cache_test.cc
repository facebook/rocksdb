...
class DBBTieredCacheTest : public DBTestBase {
 public:
  DBBTieredCacheTest()
      : DBTestBase("/db_tiered_cache_test", /*env_do_fsync=*/true) {}
};
...
TEST_F(DBBTieredCacheTest, TestTieredCache) {
  LRUCacheOptions opts(4 * 1024, 0, false, 0.5, nullptr,
                       kDefaultToAdaptiveMutex, kDontChargeCacheMetadata);
  std::shared_ptr<TestTieredCache> tiered_cache(
      new TestTieredCache(2048 * 1024));
  opts.tiered_cache = tiered_cache;
  std::shared_ptr<Cache> cache = NewLRUCache(opts);
  ...
  // No insertion.
  ASSERT_EQ(tiered_cache->num_inserts(), 0u);
  ASSERT_EQ(tiered_cache->num_lookups(), 1u);
  ...
  ASSERT_EQ(tiered_cache->num_inserts(), 0u);
  ASSERT_EQ(tiered_cache->num_lookups(), 2u);
  ...
  // cache Lookup and tiered cache lookup
  ASSERT_EQ(tiered_cache->num_inserts(), 0u);
  ASSERT_EQ(tiered_cache->num_lookups(), 3u);
  ...
  ASSERT_EQ(tiered_cache->num_lookups(), 4u);
