...
struct LFUHandle {
  ...
  void UnDateVisitTime() {
    std::chrono::time_point t = std::chrono::system_clock::now();
    auto current_mills = std::chrono::duration_cast<std::chrono::microseconds>(t.time_since_epoch()).count();
    rect = current_mills;
  }
