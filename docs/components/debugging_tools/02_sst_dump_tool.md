...
Iterates through all entries and verifies that the entry count matches the `num_entries` value in table properties. Optionally verifies checksums with `--verify_checksum`. Accepts `--from` and `--to` for range-scoped checks.
