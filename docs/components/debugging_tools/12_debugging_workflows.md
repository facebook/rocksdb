...
- **Option B (restore):** Restore from a backup if available via `BackupEngine::RestoreDBFromLatestBackup()` or `db_undump`.
...
Step 3 -- Check block cache hit rate: Compare `block_cache_hit_count` vs `block_read_count` in `PerfContext`. Low hit rate indicates the working set exceeds cache capacity. Check `rocksdb.block-cache-usage` and `rocksdb.block-cache-capacity` properties.
...
| Is this SST file corrupted? | sst_dump --verify | ldb checkconsistency |
