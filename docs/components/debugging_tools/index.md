...
| 4. Block Cache Tracing | [04_block_cache_tracing.md](04_block_cache_tracing.md) | Block cache access tracing, `BlockCacheTraceRecord` format, lookup context propagation, and cache access analysis. |
| 5. IO Tracing | [05_io_tracing.md](05_io_tracing.md) | File system operation tracing via `IOTracer`, `IOTraceRecord` format, `FileSystemTracer` integration, and the `io_tracer_parser` tool. |
...
| 7. Statistics | [07_statistics.md](07_statistics.md) | Global database statistics via `Tickers` and `Histograms`, `StatsLevel` control, stats dumping, and stats history iteration. |
...
| 11. RepairDB and Recovery | [11_repair_and_recovery.md](11_repair_and_recovery.md) | `RepairDB` process (find files, replay WALs, extract metadata, write MANIFEST), `reduce_levels`, and `db_dump`/`db_undump`. |
...
- **Configurable overhead**: `PerfLevel` (6 levels) and `StatsLevel` (6 levels) control instrumentation granularity
...
- The LOG file is written to the database directory by default; `db_log_dir` option redirects it elsewhere
