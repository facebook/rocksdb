...
The `GetThreadList()` API provides real-time visibility into what RocksDB background threads are doing. It reports the operation type, stage, elapsed time, and operation-specific properties for each active thread. This is valuable for diagnosing stuck compactions, long-running flushes, and thread pool utilization issues.

## Enabling Thread Tracking

Thread status tracking must be explicitly enabled via `DBOptions::enable_thread_tracking = true` (see `include/rocksdb/options.h`). When disabled, `GetThreadList()` returns an empty list.
...
The function returns a vector of `ThreadStatus` structs, one per active thread.
