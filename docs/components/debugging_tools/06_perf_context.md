...
Set the level per thread via `SetPerfLevel()` and query it via `GetPerfLevel()`. The default level depends on the build mode: `INFO_LEVEL` in release, `DEBUG_LEVEL` in debug.
...
The `FileIOByTemperature` struct within `IOStatsContext` tracks read bytes and read counts per file temperature tier (`hot`, `warm`, `cool`, `cold`, `ice`, `unknown`). This is useful for validating tiered storage configurations.
