...
| `keep_log_file_num` | 1000 | Number of old LOG files to retain |
...
When `max_log_file_size > 0` or `log_file_time_to_roll > 0`, RocksDB uses `AutoRollLogger` (see `logging/auto_roll_logger.h`) which automatically rotates the LOG file based on size or time. Old log files are named `LOG.old.<timestamp>` and are retained up to `keep_log_file_num` files.
