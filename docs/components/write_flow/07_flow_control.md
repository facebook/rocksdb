...
ranks the DBs sharing the manager and submits configurable batches serially,
with at most one cross-DB job in the LOW-priority pool at a time. Failed or
ineligible candidates are skipped in favor of the next candidate. Writers do
not switch a local memtable until total memory reaches `buffer_size`.
