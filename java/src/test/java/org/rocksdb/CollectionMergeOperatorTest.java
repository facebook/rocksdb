...
public class CollectionMergeOperatorTest {
  ...
  public void set_add() throws RocksDBException {
    try (final MergeOperator collectionMergeOperator =
         ...
         final Options options =
             new Options().setCreateIfMissing(true).setMergeOperator(collectionMergeOperator);
  ...
  public void set_add_str_sorted() throws RocksDBException {
    ...
    try (final MergeOperator collectionMergeOperator = new CollectionMergeOperator(
             (short) fixed_record_len, BuiltinComparator.BYTEWISE_COMPARATOR, MAKE_UNIQUE);
         final Options options =
             new Options().setCreateIfMissing(true).setMergeOperator(collectionMergeOperator);
         final RocksDB db = RocksDB.open(options, dbFolder.getRoot().getAbsolutePath())) {
      final byte[] key1 = "key1".getBytes(UTF_8);
      ...
      final byte[] value = db.get(key1);
      assertThat(new String(value, UTF_8)).isEqualTo("100020003000500060007000");
    }
  }
  ...
  public void set_add_uint_sorted() throws RocksDBException {
    ...
    try (final MergeOperator collectionMergeOperator = new CollectionMergeOperator(
      ...
      int offset = 0;
      assertThat(intFromBytesBE(value, offset)).isEqualTo(1000);
      assertThat(intFromBytesBE(value, offset += fixed_record_len)).isEqualTo(2000);
      assertThat(intFromBytesBE(value, offset += fixed_record_len)).isEqualTo(3000);
      assertThat(intFromBytesBE(value, offset += fixed_record_len)).isEqualTo(5000);
      assertThat(intFromBytesBE(value, offset += fixed_record_len)).isEqualTo(6000);
      assertThat(intFromBytesBE(value, offset += fixed_record_len)).isEqualTo(7000);
    }
  }
  ...
  public void vector_add_sorted() throws RocksDBException {
    try (final MergeOperator collectionMergeOperator =
             new CollectionMergeOperator((short) 4, BuiltinComparator.BYTEWISE_COMPARATOR);
         final Options options =
             new Options().setCreateIfMissing(true).setMergeOperator(collectionMergeOperator);
         final RocksDB db = RocksDB.open(options, dbFolder.getRoot().getAbsolutePath())) {
      final byte[] key1 = "key1".getBytes(UTF_8);
      ...
      final byte[] value = db.get(key1);
      assertThat(new String(value, UTF_8)).isEqualTo("10001000100020003000500060007000");
    }
  }

  @Test
  public void vector_addAndRemove_singleRemoveOperand() throws RocksDBException {
    try (final MergeOperator collectionMergeOperator = new CollectionMergeOperator((short) 4);
         final Options options =
             new Options().setCreateIfMissing(true).setMergeOperator(collectionMergeOperator);
         final RocksDB db = RocksDB.open(options, dbFolder.getRoot().getAbsolutePath())) {
      final byte[] key1 = "key1".getBytes(UTF_8);
