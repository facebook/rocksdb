...
public class SstFileWriter extends RocksObject {
 ...
 /**
   ...
   * Add a Put key with value to currently opened file.
   *
   * @param key the specified key to be inserted (reading from position to limit).
   * @param value the value associated with the specified key (reading from position to limit).
   *
   * @throws RocksDBException thrown if error happens in underlying
   *    native library.
   */
  public void put(final ByteBuffer key, final ByteBuffer value)
          throws RocksDBException {
    put(nativeHandle_, key, key.position(), key.remaining(), value, value.position(), value.remaining());
  }
  ...
  private native void put(final long handle, final ByteBuffer key, int keyOffset, int keyLen,
      final ByteBuffer value, int valueOffset, int valueLen) throws RocksDBException;
