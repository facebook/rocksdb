...
jbyteArray Java_org_rocksdb_CompactionJobInfo_columnFamilyName(JNIEnv* env,
  ...
  auto* compact_job_info =
      reinterpret_cast<rocksdb::CompactionJobInfo*>(jhandle);
  return rocksdb::JniUtil::copyBytes(env, compact_job_info->cf_name);
}
...
jobject Java_org_rocksdb_CompactionJobInfo_status(JNIEnv* env, jclass,
  ...
  auto* compact_job_info =
      reinterpret_cast<rocksdb::CompactionJobInfo*>(jhandle);
  return rocksdb::StatusJni::construct(env, compact_job_info->status);
}
...
jobjectArray Java_org_rocksdb_CompactionJobInfo_inputFiles(JNIEnv* env, jclass,
  ...
  auto* compact_job_info =
      reinterpret_cast<rocksdb::CompactionJobInfo*>(jhandle);
  return rocksdb::JniUtil::toJavaStrings(env, &compact_job_info->input_files);
}
...
jobjectArray Java_org_rocksdb_CompactionJobInfo_outputFiles(JNIEnv* env, jclass,
  ...
  auto* compact_job_info =
      reinterpret_cast<rocksdb::CompactionJobInfo*>(jhandle);
  return rocksdb::JniUtil::toJavaStrings(env, &compact_job_info->output_files);
}
...
jobject Java_org_rocksdb_CompactionJobInfo_tableProperties(JNIEnv* env, jclass,
  ...
  jobject jhash_map =
      rocksdb::HashMapJni::construct(env, static_cast<uint32_t>(map->size()));
  ...
  const rocksdb::HashMapJni::FnMapKV<
      const std::string, std::shared_ptr<const rocksdb::TableProperties>,
      jobject, jobject>
      fn_map_kv = [env](const std::pair<
                        const std::string,
                        std::shared_ptr<const rocksdb::TableProperties>>& kv) {
  ...
  if (!rocksdb::HashMapJni::putAll(env, jhash_map, map->begin(), map->end(),
                                   fn_map_kv)) {
    // exception occurred
    return nullptr;
  }
