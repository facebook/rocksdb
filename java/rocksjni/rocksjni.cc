 ...
 */
jint Java_org_rocksdb_RocksDB_getDirect__JJLjava_nio_ByteBuffer_2IILjava_nio_ByteBuffer_2IIJ(JNIEnv* env, jobject /*jdb*/,
 ...
 * Class:     org_rocksdb_RocksDB
 * Method:    getDirect
 * Signature: (JJ[BIILjava/nio/ByteBuffer;IIJ)I
 */
jint Java_org_rocksdb_RocksDB_getDirect__JJ_3BIILjava_nio_ByteBuffer_2IIJ(
                                           JNIEnv* env, jobject,
                                           jlong jdb_handle, jlong jropt_handle,
  ...
  auto* cf_handle =
      reinterpret_cast<ROCKSDB_NAMESPACE::ColumnFamilyHandle*>(jcf_handle);
  const auto& read_options = ropt == nullptr ? ROCKSDB_NAMESPACE::ReadOptions() : *ropt;

  static const int kNotFound = -1;
  static const int kStatusError = -2;
  static const int kArgumentError = -3;

  jbyte* jbyte_key = new jbyte[jkey_len];
  env->GetByteArrayRegion(jkey, jkey_off, jkey_len, jbyte_key);
  if (env->ExceptionCheck()) {
    // exception thrown: OutOfMemoryError
    delete[] jbyte_key;
    return kStatusError;
  ...
  ROCKSDB_NAMESPACE::Slice key_slice(reinterpret_cast<char*>(jbyte_key), jkey_len);
  ...
  ROCKSDB_NAMESPACE::Status s;
  if (cf_handle != nullptr) {
    s = db->Get(read_options, cf_handle, key_slice, &cvalue);
  } else {
    // backwards compatibility
    s = db->Get(read_options, key_slice, &cvalue);
  }
