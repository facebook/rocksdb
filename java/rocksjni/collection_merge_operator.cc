...
jlong Java_org_rocksdb_CollectionMergeOperator_newCollectionMergeOperator__SJBB(
    JNIEnv*, jclass, jshort jfixed_record_len, jlong jcomparator_handle,
    jbyte jcomparator_type, jbyte junique_constraint) {
  ROCKSDB_NAMESPACE::Comparator* comparator = nullptr;
  if (jcomparator_handle != 0) {
    comparator = ROCKSDB_NAMESPACE::AbstractComparatorJni::castCppComparator(
        jcomparator_handle, jcomparator_type);
  }

  const ROCKSDB_NAMESPACE::UniqueConstraint unique_constraint =
      ROCKSDB_NAMESPACE::UniqueConstraintJni::toCppUniqueConstraint(
          junique_constraint);
 ...
 * Signature: (J)V
 */
void Java_org_rocksdb_CollectionMergeOperator_disposeInternal(JNIEnv*, jobject,
                                                              jlong jhandle) {
  auto* sptr_collection_merge_operator =
