...
jobject Java_org_rocksdb_TransactionDB_getLockStatusData(
  ...
  const rocksdb::HashMapJni::FnMapKV<const int32_t, const rocksdb::KeyLockInfo> fn_map_kv =
      [env, txn_db, &lock_status_data](const std::pair<const int32_t, const rocksdb::KeyLockInfo>& pair) {
          auto* column_family =
                  ((rocksdb::DBImpl*)txn_db->GetBaseDB())->GetColumnFamilyHandleUnlocked(pair.first);
          const jlong jcolumn_family_handle =
              reinterpret_cast<jlong>(column_family);
          const jobject jlong_column_family_handle =
              rocksdb::LongJni::valueOf(env, jcolumn_family_handle);
          if (jlong_column_family_handle == nullptr) {
              //TODO(AR) signal error
          }
          ...
          return std::pair<jobject, jobject>(jlong_column_family_handle,
              jkey_lock_info);
      };
  ...
  for (auto buf_it = deadlock_info_buffer.begin(); buf_it != buf_end; ++buf_it) {
    ...
    for (auto infos_it = deadlock_infos.begin(); infos_it != infos_end; ++infos_it) {
        ...
        const jobject jdeadlock_info = rocksdb::TransactionDBJni::newDeadlockInfo(
            env, jobj, txn_db, deadlock_info.m_txn_id, deadlock_info.m_cf_id,
