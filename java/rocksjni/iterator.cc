...
void if_valid_helper(
    ...
    std::function<
        void(APIIterator<ROCKSDB_NAMESPACE::DB, ROCKSDB_NAMESPACE::Iterator>&)>
        lambda) {
  auto& iteratorAPI = *reinterpret_cast<
      APIIterator<ROCKSDB_NAMESPACE::DB, ROCKSDB_NAMESPACE::Iterator>*>(handle);
...
void Java_org_rocksdb_RocksIterator_next0(JNIEnv* env, jobject /*jobj*/,
  ...
  if_valid_helper(
      env, handle,
      [](APIIterator<ROCKSDB_NAMESPACE::DB, ROCKSDB_NAMESPACE::Iterator>& it) {
        it->Next();
      });
}
