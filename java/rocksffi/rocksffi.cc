...
rocksdb_input_slice_t* key, rocksdb_output_slice_t* value) {
    ...
    ROCKSDB_NAMESPACE::PinnableSlice* value_slice = new ROCKSDB_NAMESPACE::PinnableSlice();
    std::cout << "key size: " << key->size <<  ", key data: " << key->data << std::endl;
    auto status = db->Get(read_options, cf, key_slice, value_slice);
    if (status.ok()) {
        std::cout << "value size: " << value_slice->size() <<  ", value data: " << value_slice->data() << std::endl;
