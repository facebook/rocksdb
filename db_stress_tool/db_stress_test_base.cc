...
Status StressTest::TestIterate(ThreadState* thread,
  ...
  auto check_no_key_in_range = [&](int64_t start, int64_t end) {
    for (auto j = std::max(start, lb); j < std::min(end, ub); ++j) {
      ...
      if (expected_value != shared->DELETION_SENTINEL &&
        ...
        if (iter->Valid()) {
          fprintf(stderr, "Expected state has key %s, iterator is at key %s\n",
  ...
  // The random sequence Next and Prev test below tends to be very short ranged.
  ...
  // start from middle of [lb, ub) otherwise it is easy to iterate out of locked
  // range
