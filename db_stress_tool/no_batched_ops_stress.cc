...
class NonBatchedOpsStressTest : public StressTest {
 public:
  ...
  Status TestGet(ThreadState* thread, const ReadOptions& read_opts,
    ...
    const uint32_t pre_read_expected_value =
        thread->shared->Get(rand_column_families[0], rand_keys[0]);
    Status s = db_->Get(read_opts_copy, cfh, key, &from_db);
    const uint32_t post_read_expected_value =
        thread->shared->Get(rand_column_families[0], rand_keys[0]);
    if (fault_fs_guard) {
    ...
    if (s.ok()) {
      ...
      if (!FLAGS_skip_verifydb && !read_older_ts) {
        ...
        if (!ExpectedValueHelper::InExpectedValueBaseRange(
                value_base_from_db,
                ExpectedValueHelper::GetValueBase(pre_read_expected_value),
                ExpectedValueHelper::GetValueBase(post_read_expected_value),
                ExpectedValueHelper::PendingWrite(post_read_expected_value))) {
          thread->shared->SetVerificationFailure();
          fprintf(stderr,
      ...
      if (use_txn) {
        ...
        if (thread->rand.OneIn(10)) {
          ...
          switch (op) {
            case 0:
            case 1: {
              // No need to use `ExpectedValueHelper::NextValueBase()` here
              // since the value generated by this `value_base` won't be
              // committed to db as `txn` containing this value will be rolled
              // back
              const uint32_t value_base =
                  thread->rand.Next() %
                  thread->shared->RANDOM_VALUE_BASE_GENERATOR;
              char value[100];
  ...
  Status TestIterateAgainstExpected(
    ...
    std::vector<uint32_t> pre_read_expected_values(ub - lb);
    std::vector<uint32_t> post_read_expected_values(ub - lb);

    for (int64_t i = 0;
         i < static_cast<int64_t>(pre_read_expected_values.size()); ++i) {
      pre_read_expected_values[i] = shared->Get(rand_column_family, i + lb);
    }
    std::unique_ptr<Iterator> iter(db_->NewIterator(ro, cfh));
    for (int64_t i = 0;
         i < static_cast<int64_t>(post_read_expected_values.size()); ++i) {
      post_read_expected_values[i] = shared->Get(rand_column_family, i + lb);
    }
    ...
    auto check_no_key_in_range = [&](int64_t start, int64_t end) {
      for (auto j = std::max(start, lb); j < std::min(end, ub); ++j) {
        const uint32_t pre_read_expected_value =
            pre_read_expected_values[j - lb];
        const uint32_t post_read_expected_value =
            post_read_expected_values[j - lb];
        if (ExpectedValueHelper::MustHaveExisted(pre_read_expected_value,
                                                 post_read_expected_value)) {
          // Fail fast to preserve the DB state.
          thread->shared->SetVerificationFailure();
    ...
    for (int64_t i = 0; i < num_iter && iter->Valid(); ++i) {
      ...
      if (static_cast<int64_t>(curr) < lb) {
        ...
        const uint32_t pre_read_expected_value =
            pre_read_expected_values[curr - lb];
        const uint32_t post_read_expected_value =
            post_read_expected_values[curr - lb];
        if (ExpectedValueHelper::MustHaveNotExisted(pre_read_expected_value,
                                                    post_read_expected_value) ||
  ...
  bool VerifyOrSyncValue(int cf, int64_t key, const ReadOptions& /*opts*/,
    ...
    const uint32_t expected_value = shared->Get(cf, key);

    if (ExpectedValueHelper::PendingWrite(expected_value) ||
    ...
    if (s.ok()) {
      ...
      if (ExpectedValueHelper::MustHaveNotExisted(expected_value,
          ...
          !ExpectedValueHelper::InExpectedValueBaseRange(
              value_base_from_db,
              ExpectedValueHelper::GetValueBase(expected_value),
              ExpectedValueHelper::GetValueBase(expected_value),
              ExpectedValueHelper::PendingWrite(expected_value))) {
      ...
      size_t expected_value_data_size =
          GenerateValue(ExpectedValueHelper::GetValueBase(expected_value),
