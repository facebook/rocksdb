...
DECLARE_uint64(key_window_scale_factor);
...
struct KeyGenContext {
  // Max length of the key in 8-byte units. Each level is a int64 encoded in
  // big endian format
  unsigned int levels;
  // Number of keys of each possible length in a given window
  std::vector<uint64_t> weights;
  // Size of window for varying the key length
  uint64_t window;
};
extern KeyGenContext key_gen_ctx;

// Generate a variable length key string from the given int64 val. The
// algorithm picks the length based on the offset of the val within a
// configured window and the distribution of the number of keys of various
// lengths in that window. For example, if x, y, x are the weights assigned
// to each possible key length, the keys generated would be - {0}...{x-1}
// {(x-1),0}..{(x-1),(y-1)},{(x-1),(y-1),0}..{(x-1),(y-1),(z-1)} and so on
extern inline std::string Key(int64_t val) {
  uint64_t window = key_gen_ctx.window;
  std::string key;

  for (unsigned int level = 0; level < key_gen_ctx.levels; ++level) {
...
extern inline bool GetIntVal(std::string big_endian_key, uint64_t* key_p) {
  unsigned int levels = key_gen_ctx.levels;
  ...
  for (size_t level = 0; level < size_key; level += sizeof(int64_t)) {
    ...
    Slice little_endian_slice = Slice(&little_endian_key[start],
                                      sizeof(int64_t));
    uint64_t pfx;
...
extern inline uint64_t GetPrefixKeyCount(const std::string prefix,
  ...
  if (!GetIntVal(prefix, &start) || !GetIntVal(ub, &start)) {
    return 0;
  }
