...
struct SstFileWriterBlobColumn {
  size_t column_index = 0;
  Slice blob_index;
};

struct SstFileWriterBlobValue {
  size_t column_index = 0;
  Slice value;
};

struct SstFileWriterBlobOptions {
  // Directory where external blob files should be written. If empty, the SST
  // file's directory is used.
  std::string blob_file_dir;

  // File numbers are preserved during ingestion because BlobIndex values in the
  // SST refer to them directly. Pick a range that does not overlap the target
  // DB's allocated file numbers.
  uint64_t starting_blob_file_number = uint64_t{1} << 56;

  // Target blob file size. If zero, ColumnFamilyOptions::blob_file_size is used.
  uint64_t blob_file_size = 0;

  Temperature blob_file_temperature = Temperature::kUnknown;
};
...
class SstFileWriter {
 public:
  ...
  // Prepare SstFileWriter to write an SST plus external blob files.
  Status OpenWithBlobFiles(const std::string& file_path,
                           const SstFileWriterBlobOptions& blob_options,
                           Temperature temp = Temperature::kUnknown);
  ...
  // Add a PutEntity whose selected columns contain encoded BlobIndex values.
  // This prototype API only writes the blob references into the SST; callers
  // are responsible for ensuring the referenced blob files are already known to
  // the DB version that will read the ingested file.
  Status PutEntityWithBlobIndexes(
      const Slice& user_key, const WideColumns& columns,
      const std::vector<SstFileWriterBlobColumn>& blob_columns);

  // Add a PutEntity whose selected columns are written to external blob files.
  // The generated blob files must be passed through
  // IngestExternalFileArg::external_blob_files when ingesting the SST.
  Status PutEntityWithBlobValues(
      const Slice& user_key, const WideColumns& columns,
  ...
  Status Finish(ExternalSstFileInfo* file_info,
