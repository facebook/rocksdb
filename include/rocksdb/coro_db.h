...
class CoroDB {
 ...
 protected:
  ...
  // Reads the wide-column entity for key and completes after populating
  // columns. The returned Status describes the operation and its outputs.
  virtual folly::coro::Task<Status> GetEntityCoroutine(
      const ReadOptions& options, ColumnFamilyHandle* column_family,
      const Slice& key, PinnableWideColumns* columns) = 0;

  // Reads key from every column family named by result and completes after
  // populating all of them. The returned Status describes the request; each
  // group carries its own status for that column family's read.
  virtual folly::coro::Task<Status> GetEntityCoroutine(
