 ...
 *     and `new_value_buffer` are not NULL if and only if `valuetype` is
 *     `rocksdb_compactionfilter_valuetype_value` or
 *     `rocksdb_compactionfilter_valuetype_merge_operand`. `existing_columns`
 *     and `new_columns` are not NULL if only if `valuetype` is
 *     rocksdb_compactionfilter_valuetype_wide_column_entity. Except for the
 *     state pointer that the filter instance owns the callback implementation
 *     must not access the passed pointers after the call.
