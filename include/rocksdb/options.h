...
struct DBOptions {
  ...
  // performance issue with file Close() separate from Sync(), but setting it
  // to true can mean Checkpoint calling LinkFile on a WAL still open for
  // write, which might be unsupported on some FileSystem implementations. As
  // this is intended as a temporary kill switch, it is already DEPRECATED.
