...
class PinnableWideColumns {
 public:
  ...
  // Lazy column resolution support.
  // These methods are only meaningful when the entity was loaded with
  // ReadOptions::lazy_column_resolution = true and the entity contains blob
  // column references.

  // Returns true if the column at the given index is an unresolved blob
  // reference. Returns false for inline columns, already-resolved columns,
  // or if the index is out of bounds.
  bool IsUnresolvedColumn(size_t column_index) const;

  // Returns true if any columns are still unresolved.
  bool HasUnresolvedColumns() const;

  // Resolve the blob value for the column at the given index. After a
  // successful call, columns()[column_index].value() returns the actual data.
  // Returns OK if the column is inline or already resolved.
  Status ResolveColumn(size_t column_index);

  // Resolve all unresolved blob columns at once.
  Status ResolveAllColumns();

  // Set wide column value with lazy blob resolution. The serialized entity
  // data is copied, and the columns index is built from the provided
  // pre-parsed columns. The resolver is attached for on-demand blob value
  // fetching. Internal use only.
  void SetWideColumnValueLazy(std::string&& serialized_entity,
                              WideColumns&& columns,
 ...
 private:
  ...
  std::unique_ptr<ReadPathBlobResolver> resolver_;
};

// Reset(), Move(), move constructor, move assignment, and
// SetWideColumnValue(string&&) are defined out-of-line in wide_columns.cc
// because they need the complete type of ReadPathBlobResolver.
...
// All SetWideColumnValue overloads, move ctor, and move assignment are defined
// out-of-line in wide_columns.cc because they call Reset() which needs
// the complete type of ReadPathBlobResolver.
