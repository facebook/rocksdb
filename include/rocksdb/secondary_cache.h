...
class SecondaryCache : public Customizable {
 public:
  ...
  // the call returns, then Warm() must be overridden not to rely on Insert().
  // For example, there could be a "holding area" in memory where Lookup might
  // return the same parsed value back. But more typically, if the
  // implementation only uses `value` for getting persistable data during the
  // call, then the default implementation of `Warm()` suffices.
  virtual Status Insert(const Slice& key, void* value,
                        const Cache::CacheItemHelper* helper) = 0;

  // Insert the given value from its persistable data (typically uncompressed
  // block), as if generated by SaveToCallback/SizeCallback. This path is
  // typically used in "warming up" the cache from some auxiliary source, so
  ...
  virtual Status Warm(const Slice& key, const Slice& block);
