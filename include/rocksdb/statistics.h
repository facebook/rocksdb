...
const std::vector<std::pair<Tickers, std::string>> TickersNameMap = {
    ...
    {BLOB_DB_GC_NUM_SCAN, "rocksdb.blobdb.gc.num.scan"},
...
enum Histograms : uint32_t {
  ...
  // Time garbage collection spent on scanning blob indexes.
  BLOB_DB_GC_SCAN_MICROS,
  // Time spent on garbage collect a blob file.
  BLOB_DB_GC_FILE_MICROS,
...
const std::vector<std::pair<Histograms, std::string>> HistogramsNameMap = {
    ...
    {BLOB_DB_GC_SCAN_MICROS, "rocksdb.blobdb.gc.scan.micros"},
    {BLOB_DB_GC_FILE_MICROS, "rocksdb.blobdb.gc.file.micros"},
