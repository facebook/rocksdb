...
struct LsmEdit {
  ...
  // removes data is not expressible yet. Before installation, RocksDB
  // validates sequence-number ordering between overlapping files. Disjoint
  // sequence-number ranges use a metadata-only fast path, but ambiguous pairs
  // require scanning their contents. A file can be scanned once per ambiguous
  // overlapping peer, producing quadratic work in the worst case.
