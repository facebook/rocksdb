...
/* Superblocks are stored in every metadata zone.
   The latest written superblock can be found by checking the sequence number.
 */
class Superblock {
 private:
  uint32_t magic_ = 0;
 ...
 public:
   ...
   */
  explicit Superblock(ZonedBlockDevice* zbd,
                      const std::string& aux_fs_path = "",
  ...
  uint32_t GetFinishTreshold() { return finish_threshold_; }
...
/* ZenFS is implemented as a FileSystemWrapper where all files
   except Log and Lock files are stored on a zoned block device.
   The directory stucture (and the log and lock files) are stored
   at the aux file system path */
class ZenFS : public FileSystemWrapper {
  ...
  /* Record type tags for storing metadata */
 ...
 public:
  ...
  const char* Name() const override { return "ZenFS"; }
