...
KinesisSystem::KinesisSystem(AwsEnv* env, std::shared_ptr<Logger> info_log)
  ...
  cache_dir_ = "/tmp/ROCKSET" + pathsep + env_->GetSrcBucketPrefix() + pathsep +
               random + pathsep;
  ...
  if (status_.ok()) {
    status_ = env_->GetPosixEnv()->CreateDirIfMissing(
        "/tmp/ROCKSET/" + env_->GetSrcBucketPrefix());
  }
  if (status_.ok()) {
    status_ = env_->GetPosixEnv()->CreateDirIfMissing(
        "/tmp/ROCKSET/" + env_->GetSrcBucketPrefix() + pathsep + random +
        pathsep);
  }
}
...
Status KinesisSystem::Apply(const Slice& in) {
  ...
  if (operation == Append) {
    ...
    if (iter == cache_fds_.end()) {
      ...
      st =
          env_->GetPosixEnv()->NewRandomRWFile(pathname, &result, EnvOptions());
      if (st.ok()) {
    ...
    if (!st.ok()) {
      Log(InfoLogLevel::DEBUG_LEVEL, env_->info_log_,
          "[kinesis] Tailer Error writing to cached file %s", pathname.c_str(),
          st.ToString().c_str());
    }
    ...
    Log(InfoLogLevel::DEBUG_LEVEL, env_->info_log_,
        "[kinesis] Tailer Delete file %s %s", pathname.c_str(),
        st.ToString().c_str());
    if (st.IsNotFound()) {
      st = Status::OK();
    }
    ...
    Log(InfoLogLevel::DEBUG_LEVEL, env_->info_log_,
        "[kinesis] Tailer Closed file %s %s", pathname.c_str(),
        st.ToString().c_str());
  } else {
    st = Status::IOError("Unknown operation");
    Log(InfoLogLevel::DEBUG_LEVEL, env_->info_log_,
        "[kinesis] Tailer file %s unknown operation %s", pathname.c_str(),
        st.ToString().c_str());
  }
  return st;
}
...
Status KinesisSystem::WaitForStreamReady(
  ...
  while (!isSuccess) {
    ...
    if (!isSuccess) {
      ...
      Log(InfoLogLevel::DEBUG_LEVEL, info_log, " Waiting for stream ready %s",
          st.ToString().c_str());
    } else {
...
Status KinesisSystem::InitializeShards() {
  ...
  DescribeStreamOutcome outcome =
      env_->kinesis_client_->DescribeStream(request);
  bool isSuccess = outcome.IsSuccess();
  if (!isSuccess) {
    const Aws::Client::AWSError<KinesisErrors>& error = outcome.GetError();
...
void KinesisSystem::SeekShards() {
  ...
  for (size_t i = 0; i < shards_.size(); i++) {
    ...
    GetShardIteratorOutcome outcome =
        env_->kinesis_client_->GetShardIterator(request);
    bool isSuccess = outcome.IsSuccess();
    if (!isSuccess) {
      const Aws::Client::AWSError<KinesisErrors>& error = outcome.GetError();
      status_ = Status::IOError(topic_.c_str(), error.GetMessage().c_str());
...
//
void KinesisSystem::SerializeLogRecordClosed(const Slice& filename,
                                             uint64_t file_size,
                                             std::string* out) {
  // write the operation type
...
//
void KinesisSystem::SerializeLogRecordDelete(const std::string& filename,
                                             std::string* out) {
  // write the operation type
...
//
bool KinesisSystem::ExtractLogRecord(const Slice& input, uint32_t* operation,
...
KinesisWritableFile::KinesisWritableFile(AwsEnv* env, const std::string& fname,
...
Status KinesisWritableFile::Append(const Slice& data) {
  ...
  std::string buffer;
  KinesisSystem::SerializeLogRecordAppend(fname_, data, current_offset_,
                                          &buffer);
