...
IOStatus SetCurrentFile(FileSystem* fs, const std::string& dbname,
  ...
  if (s.ok()) {
    ...
    fs->DeleteFile(tmp, IOOptions(), nullptr).PermitUncheckedError();  // NOTE: PermitUncheckedError is acceptable here as we are already handling an error case, and this is just a best-attempt effort at some cleanup
  }
  return s;
}
