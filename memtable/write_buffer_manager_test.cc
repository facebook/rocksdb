...
TEST_F(WriteBufferManagerTest,
       PendingCoordinatorHandoffForcesPeriodicLocalProgress) {
  WriteBufferManager wbf(1, nullptr /* cache */, false /* allow_stall */,
                         WriteBufferFlushPolicy::kFlushLargestAcrossDBs);
  BlockingFlushInitiator initiator;
  wbf.RegisterFlushInitiator(&initiator);
  wbf.ReserveMem(2);

  ASSERT_TRUE(wbf.InitiateFlushOnLargestDB());
  initiator.WaitUntilEntered();
  Env::Default()->SleepForMicroseconds(21 * 1000);

  EXPECT_FALSE(wbf.InitiateFlushOnLargestDB());
