...
TEST_F(WriteBufferManagerTest, WriterFallsBackOnlyAtHardLimit) {
  ...
  EXPECT_FALSE(wbf.ShouldFlushLocally());

  wbf.ReserveMem(24);
  EXPECT_TRUE(wbf.ShouldFlushLocally());
  EXPECT_EQ(1, initiator.ScheduleCalls());

  initiator.Release();
  wbf.NotifyFlushInitiatorFlushCompleted();
...
TEST_F(WriteBufferManagerTest, BackgroundFlushBatchUsesOnePoolSlot) {
  ...
  first.ScheduleFreeMem(300);
  wbf.NotifyFlushInitiatorFlushCompleted();
  second.WaitForScheduleCalls(1);
  EXPECT_EQ(0, third.ScheduleCalls());

  second.ScheduleFreeMem(200);
  wbf.NotifyFlushInitiatorFlushCompleted();
...
TEST_F(WriteBufferManagerTest, InaccurateLargestMutableCFIsNotSelected) {
  ...
  wbf.TEST_RefreshFlushInitiatorCandidate();
  EXPECT_FALSE(wbf.TEST_ScheduleFlushOnLargestDB(&stale));
