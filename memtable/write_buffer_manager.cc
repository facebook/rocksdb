...
void WriteBufferManager::ProcessFlushHandoffRequest() {
  ...
  if (flush_policy() != WriteBufferFlushPolicy::kFlushLargestAcrossDBs ||
      !ShouldFlush()) {
    flush_candidate_batch_.clear();
    flush_candidate_next_ = 0;
    flush_candidate_cycle_active_ = false;
    return;
  }

  if (flush_candidate_next_ == flush_candidate_batch_.size()) {
    if (!flush_candidate_cycle_active_) {
      flush_initiator_registry_->StartCandidateCycle();
      flush_candidate_cycle_active_ = true;
    }
    flush_candidate_batch_ =
        flush_initiator_registry_->GetCandidateBatch(flush_batch_size_);
    flush_candidate_next_ = 0;
    if (flush_candidate_batch_.empty()) {
      flush_candidate_cycle_active_ = false;
      return;
    }
  }

  while (flush_candidate_next_ < flush_candidate_batch_.size()) {
    const std::shared_ptr<FlushInitiator::RegistrationState> candidate =
        flush_candidate_batch_[flush_candidate_next_++];
    if (candidate->registry_index.load(std::memory_order_acquire) ==
            FlushInitiator::kInvalidRegistryIndex ||
...
void WriteBufferManager::NotifyFlushInitiatorFlushCancelled() {
  NotifyFlushInitiatorFlushCompleted();
}
