...
ZXC_VER ?= 0.11.0
ZXC_SHA256 ?= a8e82e811aca54ca3c2f8b27e1184e795826ae87981e255d36630a6924ece202
ZXC_DOWNLOAD_BASE ?= https://rocksdb-deps.s3.us-west-2.amazonaws.com/pkgs/zxc
ZXC_STATIC_LIB = libzxc.a
ifeq ($(DEBUG_LEVEL),2)
ZXC_CMAKE_BUILD_TYPE ?= Debug
else
ZXC_CMAKE_BUILD_TYPE ?= Release
endif
ifneq ($(filter-out 0,$(PORTABLE)),)
ZXC_NATIVE_ARCH ?= OFF
else
ZXC_NATIVE_ARCH ?= ON
endif
ZXC_CFLAGS = -fPIC $(ARCHFLAG) ${JAVA_STATIC_DEPS_CCFLAGS} ${EXTRA_CFLAGS}
ifeq ($(ZXC_NATIVE_ARCH),ON)
ZXC_CFLAGS += -march=native
endif

ifeq ($(WITH_ZXC),1)
PLATFORM_CCFLAGS += -DZXC -I./zxc-$(ZXC_VER)/include
PLATFORM_CXXFLAGS += -DZXC -I./zxc-$(ZXC_VER)/include
PLATFORM_LDFLAGS += ./$(ZXC_STATIC_LIB)
JAVA_LDFLAGS += ./$(ZXC_STATIC_LIB)
endif
...
clean-ext-libraries-all:
	rm -rf bzip2* snappy* zlib* lz4* zstd* zxc*

clean-ext-libraries-bin:
	$(FIND) . -maxdepth 1 -type d \( -name bzip2\* -or -name snappy\* -or -name zlib\* -or -name lz4\* -or -name zstd\* -or -name zxc\* \) -prune -exec rm -rf {} \;
...
ifeq ($(WITH_ZXC),1)
$(OBJ_DIR)/util/compression.o: $(ZXC_STATIC_LIB)
$(OBJ_DIR)/util/compression.cc.d: zxc-$(ZXC_VER)/include/zxc.h
$(STATIC_LIBRARY): $(ZXC_STATIC_LIB)
$(SHARED4): $(ZXC_STATIC_LIB)
rocksdbjava: $(ZXC_STATIC_LIB)
endif
...
zxc-$(ZXC_VER).tar.gz:
	curl --fail --output zxc-$(ZXC_VER).tar.gz --location ${CURL_SSL_OPTS} ${ZXC_DOWNLOAD_BASE}/zxc-$(ZXC_VER).tar.gz
	ZXC_SHA256_ACTUAL=`$(SHA256_CMD) zxc-$(ZXC_VER).tar.gz | cut -d ' ' -f 1`; \
	if [ "$(ZXC_SHA256)" != "$$ZXC_SHA256_ACTUAL" ]; then \
		echo zxc-$(ZXC_VER).tar.gz checksum mismatch, expected=\"$(ZXC_SHA256)\" actual=\"$$ZXC_SHA256_ACTUAL\"; \
		exit 1; \
	fi

libzxc.a: zxc-$(ZXC_VER).tar.gz
	-rm -rf zxc-$(ZXC_VER)
	tar xvzf zxc-$(ZXC_VER).tar.gz
	mkdir zxc-$(ZXC_VER)/build
	cd zxc-$(ZXC_VER)/build && CFLAGS='$(ZXC_CFLAGS)' LDFLAGS='${JAVA_STATIC_DEPS_LDFLAGS} ${EXTRA_LDFLAGS}' cmake -DCMAKE_BUILD_TYPE=$(ZXC_CMAKE_BUILD_TYPE) -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DBUILD_SHARED_LIBS=OFF -DZXC_BUILD_CLI=OFF -DZXC_BUILD_TESTS=OFF -DZXC_NATIVE_ARCH=$(ZXC_NATIVE_ARCH) -DZXC_ENABLE_LTO=OFF ${PLATFORM_CMAKE_FLAGS} .. && $(MAKE) zxc_lib
	cp zxc-$(ZXC_VER)/build/libzxc.a .

zxc-$(ZXC_VER)/include/%.h: libzxc.a
	@test -f $@

# A version of each $(LIB_OBJECTS) compiled with -fPIC and a fixed set of static compression libraries
ifneq ($(ROCKSDB_JAVA_NO_COMPRESSION), 1)
JAVA_COMPRESSIONS = libz.a libbz2.a libsnappy.a liblz4.a libzstd.a libzxc.a
endif

JAVA_STATIC_FLAGS = -DZLIB -DBZIP2 -DSNAPPY -DLZ4 -DZSTD -DZXC
JAVA_STATIC_INCLUDES = -I./zlib-$(ZLIB_VER) -I./bzip2-$(BZIP2_VER) -I./snappy-$(SNAPPY_VER) -I./snappy-$(SNAPPY_VER)/build -I./lz4-$(LZ4_VER)/lib -I./zstd-$(ZSTD_VER)/lib -I./zstd-$(ZSTD_VER)/lib/dictBuilder -I./zxc-$(ZXC_VER)/include
