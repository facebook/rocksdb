...
ifeq ($(PLATFORM), OS_MACOSX)
        comma := ,
        AM_LINK_WHOLE = $(AM_V_CCLD)$(CXX) -L. $(patsubst lib%.a, -Wl$(comma)-force_load lib%.a, $(patsubst lib%.$(PLATFORM_SHARED_EXT), -l%, $^)) $(EXEC_LDFLAGS) -o $@ $(LDFLAGS) $(COVERAGEFLAGS)
else
...
PLATFORM_LDFLAGS += $(foreach plugin, $(ROCKSDB_PLUGINS), $($(plugin)_LDLIBS))
...
gen-pc:
	...
	-echo 'Libs: -L$${libdir} $(EXEC_LDFLAGS) -Wl,--whole-archive -lrocksdb -Wl,--no-whole-archive' >> rocksdb.pc
