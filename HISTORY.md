...
* Enable SST file ingestion with file checksum information. Added files_checksum and files_checksum_func_name to IngestExternalFileArg such that use can call IngestExternalFiles with file checksum information. Add generate_and_verify_file_checksum to IngestExternalFileOptions (default is False). If it is set to True and DB enables file checksum, DB will generate the checksum for each ingested SST file and verify with the ingested checksum informaiton. Any mismatch will fail the ingestion. If the ingested checksum information is non-empty, DB enables file checksum, and generate_and_verify_file_checksum is False, DB will still verify the checksum function name. If the function name matches, the injested checksum will be directly stored in Manifest without verification. Note that, if IngestExternalFileOptions::write_global_seqno is True, the seqno will be changed in the ingested file. Therefore, the checksum of the file will be changed. In this case, no checksum information will be stored in the Manifest, unless generate_and_verify_file_checksum is set to True and a new checksum will be generated after the seqno is updated.
