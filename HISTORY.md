...
* Using NewBloomFilterPolicy with bits_per_key < 0.5 now skips generating a filter, equivalent to no filter policy. Other values 0.5 <= bits_per_key < 1.0 are still rounded up to 1.0. Custom filter policies can now implement the same behavior with custom FilterBitsBuilder overriding new function `ShouldSkipFilter`.
