...
* Added a new blob garbage collection option `blob_garbage_collection_space_amp_limit` to enable customers to directly set a space amplification target (as opposed to a per-blob-file-batch garbage threshold) and therefore support an improved/global limit on BlobDB's space amplification.
