...
* Fixed a bug where a data race can happens when a handle from secondary cache is `Promote()` and then both the `SetIsInSecondaryCache()` and a new `Lookup()` try to set flags.
