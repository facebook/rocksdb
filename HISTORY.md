...
### API Changes
* `rocksdb_approximate_sizes` in the C API now requires an error pointer (`char** errptr`) for receiving any error.
* `DB::GetApproximateSizes` now returns a Status object which indicates where the sizes could be correctly obtained.
