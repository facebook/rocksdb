...
* Support compaction filters for the new implementation of BlobDB (not the original StackableDB-based version). This includes adding `IsStackedBlobDbCompactionFilter()` to `CompactionFilter` to distinguish between compaction filters for legacy StackableDB-based BlobDBImpl and newly-introduced integrated BlobDB impl. Also add `ShouldFilterBlobByKey()` to `CompactionFilter`. Subclasses can override this method so that compaction filters can determine whether the actual blob value has to be read during compaction. Use a new `kUndetermined` in `CompactionFilter::Decision` to indicated that further action is necessary for compaction filter to make a decision.
