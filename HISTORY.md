...
* Add `WaitForCompact()` to wait for all flush and compactions jobs to finish. Jobs to wait include the unscheduled (queued, but not scheduled yet). `WaitForCompactOptions` has an option, `abort_on_pause`, to abort on pause. When set to true and `PauseBackgroundWork()` is called while or before waiting, `Status::Aborted` will be returned immediately.
