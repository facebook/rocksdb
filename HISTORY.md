...
### New Features
* A new option `std::shared_ptr<FileChecksumGenFactory> backup_checksum_gen_factory` is added to `BackupableDBOptions`. The default value for this option is `nullptr`. If this option is null, the default backup engine checksum function (crc32c) will be used for creating, verifying, or restoring backups. If it is not null, a custom checksum function will be used. When creating a new backup with non-null `backup_checksum_gen_factory`, the checksum function to be used is the one returned by `backup_checksum_gen_factory->CreateFileChecksumGenerator(FileChecksumGenContext)` with `FileChecksumGenContext::requested_function_name` being empty. When verifying or restoring a backup that was created using a custom checksum function, `Status::NotSupported` will be returned if no checksum factory is provided, and `Status::NotFound` will be returned if the name of the custom checksum function cannot be found in the checksum factory passed in by `backup_checksum_gen_factory`.
