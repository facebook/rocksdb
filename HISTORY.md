...
* Add `WaitForCompact()` to wait for all flush and compactions jobs to finish. Jobs to wait include the unscheduled (queued, but not scheduled yet). If the db is shutting down, `Status::ShutdownInProgress` will be returned. If `PauseBackgroundWork()` was called prior to this, this may potentially wait for unscheduled jobs indefinitely. `WaitForCompactOptions` has an option, `abort_on_pause`, to abort on pause. When it is set to true, if `PauseBackgroundWork()` is called while waiting, `Status::Aborted` will be returned immediately.
