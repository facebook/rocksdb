...
* Optionally charged memory usage of Bloom filter (new) and Ribbon filter construction to block cache in the block-based table. It also constrained the memory usage of constructing the banding in Ribbon filter by switching to Bloom Filter if that banding uses up too much memory relative to the available space in the block cache (i.e, causing a cache full when `strict_capacity_limit` = true). To enable this option. set `BlockBasedTableOptions::charge_bloom_ribbon_filter_construction_memory=true`
