...
* Introduced an overload of `NewJemallocNodumpAllocator()` that accepts a `num_arenas` argument. Setting `num_arenas > 1` may mitigate mutex contention in the allocator, particularly in scenarios where block allocations commonly bypass jemalloc tcache.
