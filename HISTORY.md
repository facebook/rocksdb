...
* Add new option `track_and_verify_wals_in_manifest`. If enabled, the WALs and their sizes are tracked in MANIFEST, during DB recovery, if there is a missing WAL or the on-disk WAL size is unexpected, an error will be reported. Note that this option cannot be enabled if you are using secondary instance.
