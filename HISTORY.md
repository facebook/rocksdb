...
* When a block is firstly `Lookup` from `CompressedSecondaryCache`, we just insert a dummy block into the primary cache (charging the actual size of the block) and don’t erase the block from `CompressedSecondaryCache`. A standalone handle is returned to the caller. Only if the block is hit again, we erase it from `CompressedSecondaryCache` and add it into the primary cache.
* When a block is firstly evicted from the primary cache to `CompressedSecondaryCache`, we just insert a dummy block (size 0) in `CompressedSecondaryCache`. When the block is evicted again, it is treated as a hot block and is inserted into `CompressedSecondaryCache`.
