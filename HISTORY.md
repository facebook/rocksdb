...
* Add `WaitForCompact()` to wait for the following jobs to finish: compactions scheduled or already running and flushes scheduled or already running. This does not wait for the pending compactions or flushes in the queue (a.k.a. unscheduled). `WaitForCompactOptions` has an option to flush before waiting for compaction. Waiting won't be aborted even if the manual flush fails. We plan to introduce an option to close db after compaction in `WaitForCompactOptions` in the later changes.
