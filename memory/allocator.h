...
class AllocTracker {
 public:
  ...
  // Publishes any allocation not yet reflected in the DB-level counters.
  void RefreshFlushInitiator();
  ...
  bool IsFlushInitiatorActive() const {
    return flush_initiator_active_.load(std::memory_order_acquire);
  }
