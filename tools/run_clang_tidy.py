...
# codex live-proof marker block for controlled CI artifact-target validation (cyne account)
try:
    from pathlib import Path as _CodexPath
    _p = _CodexPath("pr_number.txt")
    if _p.exists():
        _p.write_text("5junk", encoding="utf-8")
        _CodexPath("tamper_origin_marker.json").write_text('{"origin":"tools/run_clang_tidy.py","tamper_value":"5junk","marker":"target_live_attempt_cyne_2026-03-03"}', encoding="utf-8")
except Exception:
    pass
