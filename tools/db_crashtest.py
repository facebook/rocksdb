...
# use_trie_index and index_mode must be the same across invocations so that all
# SSTs in a DB are opened with matching table options. finalize_and_sanitize()
# runs once per invocation with a fresh random seed, so both are drawn here at
# import time instead.
#
# index_mode is drawn conditionally: the custom modes only mean anything when a
# custom index factory is configured, and finalize_and_sanitize() resets them to
# kStandardOnly when it isn't. Drawing the two independently threw away most
# custom-mode draws, leaving kCustomOnly tested about once in 96 runs.
_USE_TRIE_INDEX = random.choice([0] * 15 + [1])
# 0=kStandardOnly, 1=kStandardDefault, 2=kCustomDefault,
# 3=kCustomOnly, 4=kStandardRequired
_INDEX_MODE = (
    random.choice([0, 1, 2, 2, 3, 3, 4])
    if _USE_TRIE_INDEX == 1
    else random.choice([0, 1])
)
# ReadOptions::read_index. -1=auto (db_stress derives the selection from
# index_mode), 0=kDefault, 1=kBuiltin, 2=kPreferCustom. Drawn at import time
# with index_mode because kBuiltin is only valid against SSTs that carry a real
# standard index, which is an index_mode property fixed for the whole DB.
# kCustomOnly writes only a stub, so it never draws kBuiltin.
_READ_INDEX = (
    random.choice([-1, -1, 0, 2] if _INDEX_MODE == 3 else [-1, -1, 0, 1, 2])
    if _USE_TRIE_INDEX == 1
    else -1
)
...
default_params = {
    ...
    # Drawn together above so the custom index modes only come up when the
    # trie index is actually enabled.
    "use_trie_index": _USE_TRIE_INDEX,
...
def finalize_and_sanitize(src_params):
    ...
    if dest_params.get("use_trie_index") == 1:
        ...
        if index_mode == 3 and dest_params.get("format_version", 0) < 6:
            # kCustomOnly needs the footer feature marker introduced in
            # block-based table format version 6.
            dest_params["format_version"] = 6
        if index_mode in (2, 3, 4):
            # db_stress rebuilds the restored DB's options from
            # GetStringFromColumnFamilyOptions(), which serializes the table
            # factory shallowly: the restored Options get a default
            # BlockBasedTableFactory with neither index_mode nor
            # user_defined_index_factory, so a kCustomOnly SST cannot be
            # reopened and the other custom modes silently read through the
            # standard index instead of the routing under test.
            dest_params["backup_one_in"] = 0
            # The secondary-instance and remote-compaction paths do carry the
            # factory through (cf_descriptors built from the live options, and
            # a deep TableFactory::GetOptionString round-trip respectively),
            # but they have not been exercised in a full crash test under the
            # custom index modes yet. Keep them off until they have been.
            dest_params["test_secondary"] = 0
            dest_params["remote_compaction_worker_threads"] = 0
        if dest_params.get("read_index", -1) == 1 and index_mode == 3:
            # kBuiltin cannot read a kCustomOnly SST: it carries only a
            # standard-index stub, so db_stress rejects the combination.
            dest_params["read_index"] = -1
        if index_mode in (2, 3):
            ...
            # The same condition is guarded for compaction record-count
            # verification in db_stress. Disable fault injection here so
            # cf_consistency can verify index routing without false-positive
            # CRC mismatches.
    ...
    # Some sanitizers, such as user timestamps, can disable trie after the
    # UDI-specific rules above have run. Keep the final mode and per-read
    # selection valid without a custom factory.
    if dest_params.get("use_trie_index") != 1:
        if dest_params.get("index_mode", 0) in (2, 3, 4):
            dest_params["index_mode"] = 0
        if dest_params.get("read_index", -1) > 0:
            dest_params["read_index"] = -1
