...
_COMPATIBILITY_MODE_ENV_VAR = "ROCKSDB_CRASHTEST_COMPATIBILITY_MODE"
_COMPATIBILITY_MODE_DEPENDENCY = "dependency"
_COMPATIBILITY_MODE_LEGACY = "legacy"
_MULTISCAN_MAX_PREFETCH_MEMORY_BYTES_CHOICES = (0, 0, 64 * 1024, 256 * 1024)
_FAULT_INJECTION_LOG_DIR_NAME = "fault_injection_logs"


def _compatibility_mode():
    mode = os.environ.get(
        _COMPATIBILITY_MODE_ENV_VAR, _COMPATIBILITY_MODE_DEPENDENCY
    ).strip().lower()
    if mode in {_COMPATIBILITY_MODE_DEPENDENCY, "solver", "new"}:
        return _COMPATIBILITY_MODE_DEPENDENCY
    if mode in {_COMPATIBILITY_MODE_LEGACY, "old"}:
        return _COMPATIBILITY_MODE_LEGACY
    print(
        f"ERROR: unsupported {_COMPATIBILITY_MODE_ENV_VAR}={mode!r}; "
        f"expected '{_COMPATIBILITY_MODE_DEPENDENCY}' or "
        f"'{_COMPATIBILITY_MODE_LEGACY}'.",
        file=sys.stderr,
    )
    sys.exit(1)


def _use_legacy_feature_compatibility_sanitizer():
    return _compatibility_mode() == _COMPATIBILITY_MODE_LEGACY
...
def _apply_legacy_feature_compatibility_sanitizer(dest_params):
    return legacy_compatibility.run_legacy_compatibility_sanitizer(
        dest_params,
        is_release_mode=is_release_mode,
        is_direct_io_supported=is_direct_io_supported,
        is_remote_db=is_remote_db,
    )


def _apply_special_rules_and_get_changed_keys(dest_params):
    before = dict(dest_params)
    _apply_special_rules(dest_params)
    changed_keys = {
        key
        for key in set(before) | set(dest_params)
        if before.get(key) != dest_params.get(key)
    }
    return changed_keys


def _apply_feature_compatibility_solver(dest_params, explicit_keys):
    special_keys = _apply_special_rules_and_get_changed_keys(dest_params)
    try:
        compatibility.run_compatibility_solver(dest_params, explicit_keys, special_keys)
        # Special rules are runtime/computed constraints and must still win if
        # compatibility solving changed one of their inputs.
        final_special_keys = _apply_special_rules_and_get_changed_keys(dest_params)
        if final_special_keys:
            compatibility.run_compatibility_solver(
                dest_params,
                explicit_keys,
                special_keys | final_special_keys,
            )
        _apply_legacy_wal_write_fault_rule(dest_params, explicit_keys)
    except compatibility.CompatibilitySolverConflict as exc:
        print(
            f"ERROR: incompatible db_crashtest feature options: {exc}",
            file=sys.stderr,
        )
        sys.exit(1)


def _set_compatibility_value(dest_params, explicit_keys, key, value):
    if key in explicit_keys and dest_params.get(key) != value:
        print(
            "ERROR: incompatible db_crashtest feature options: "
            f"explicit {key}={dest_params.get(key)} conflicts with required "
            f"{key}={value}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _apply_legacy_wal_write_fault_rule(dest_params, explicit_keys):
    # The old sanitizer treats an absent column_families option as not equal to
    # one. Apply that compatibility guard after solving so absent fallback
    # domains do not change random priority among real feature options.
...
def _apply_special_rules(dest_params):
    ...
    if dest_params.get("compression_max_dict_bytes") == 0:
        set_param("compression_zstd_max_train_bytes", 0)
        set_param("compression_max_dict_buffer_bytes", 0)
    if dest_params.get("compression_type") != "zstd":
        set_param("compression_zstd_max_train_bytes", 0)

    if dest_params.get("mmap_read") == 1:
        set_param("use_direct_io_for_flush_and_compaction", 0)
        set_param("use_direct_reads", 0)
        set_param("use_direct_io_for_compaction_reads", 0)
        set_param("multiscan_use_async_io", 0)
    ...
    if dest_params.get("min_tombstones_for_range_conversion", 0) > 0:
        ...
        set_param("use_sqfc_for_range_queries", 0)
        # Delete range is not compatible with inplace_update_support.
        set_param("inplace_update_support", 0)
    ...
    if (
        ...
        or dest_params.get("use_direct_io_for_compaction_reads") == 1
    ...
    # 3. Remote file systems do not make active blob files safely readable.
    if is_remote_db:
    ...
    if (
        dest_params.get("use_multiscan") == 1
        and "multiscan_max_prefetch_memory_bytes" not in dest_params
    ):
        set_param(
            "multiscan_max_prefetch_memory_bytes",
            random.choice(_MULTISCAN_MAX_PREFETCH_MEMORY_BYTES_CHOICES),
        )
...
def finalize_and_sanitize(src_params, explicit_keys=None):
    ...
    if _use_legacy_feature_compatibility_sanitizer():
        return _apply_legacy_feature_compatibility_sanitizer(dest_params)

    _apply_feature_compatibility_solver(dest_params, explicit_keys)
...
def _build_db_stress_cmd(finalized_params, unknown_params):
    ...
    num_dbs = finalized_params.get("num_dbs", 1)
    ev_val = finalized_params.get("expected_values_dir", "")
    if ev_val:
        if finalized_params.get("destroy_db_initially", 0) and os.path.exists(ev_val):
    ...
    return cmd, finalized_params


def gen_cmd(params, unknown_params):
    if _use_legacy_feature_compatibility_sanitizer():
        return _build_db_stress_cmd(finalize_and_sanitize(params), unknown_params)
    ...
    for key in sorted(explicit_overrides):
        ...
        if explicit_value != finalized_value:
            if key in compatibility.OPERATION_TOTAL_PERCENT_KEYS:
                print(
                    "WARNING: explicit operation-percent flag was adjusted by "
                    "compatibility normalization: "
                    f"{_format_flag_assignment(key, explicit_value)} vs "
                    f"{_format_flag_assignment(key, finalized_value)}. "
                    "Operation percentages are solved as a group to preserve "
                    "the total mix.",
                    file=sys.stderr,
                )
                continue
    ...
    return _build_db_stress_cmd(finalized_params, remaining_unknown)
