...
class StressTest {
 ...
 protected:
  ...
  virtual void OperateDb(ThreadState* thread) = 0;
...
class NonBatchedOpsStressTest : public StressTest {
      ...
      if (i != 0 && (i % (FLAGS_ops_per_thread / (FLAGS_reopen + 1))) == 0) {
        {
          ...
          if (thread->shared->AllVotedReopen()) {
            auto stress_test = static_cast<NonBatchedOpsStressTest*>(
 ...
 protected:
  uint64_t GenerateOneKey(ThreadState* thread, uint64_t iteration) const {
    const double completed_ratio =
...
class BatchedOpsStressTest : public NonBatchedOpsStressTest {
 public:
      ...
      if (i != 0 && (i % (FLAGS_ops_per_thread / (FLAGS_reopen + 1))) == 0) {
        {
          ...
          thread->shared->IncVotedReopen();
          if (thread->shared->AllVotedReopen()) {
            auto stress_test = static_cast<BatchedOpsStressTest*>(
                thread->shared->GetStressTest());
            stress_test->Reopen();
            thread->shared->GetCondVar()->SignalAll();
          }
...
class MultiCFConsistencyStressTest : public BatchedOpsStressTest {
 public:
  MultiCFConsistencyStressTest() {}

  virtual ~MultiCFConsistencyStressTest() {}
  ...
  virtual void OperateDb(ThreadState* thread) {
    ...
    for (uint64_t i = 0; i < FLAGS_ops_per_thread; i++) {
      ...
      if (i != 0 && (i % (FLAGS_ops_per_thread / (FLAGS_reopen + 1))) == 0) {
        {
          ...
          thread->shared->IncVotedReopen();
          if (thread->shared->AllVotedReopen()) {
            auto stress_test = static_cast<MultiCFConsistencyStressTest*>(
                thread->shared->GetStressTest());
            stress_test->Reopen();
            thread->shared->GetCondVar()->SignalAll();
          }
...
#endif                // !ROCKSDB_LITE
      uint64_t rand_key = GenerateOneKey(thread, i);
      std::string key_str = Key(rand_key);
      Slice key = Slice(key_str);
      int rand_column_family = thread->rand.Next() % FLAGS_column_families;
      ...
      if (FLAGS_acquire_snapshot_one_in > 0 &&
        ...
        // that the results are the same.
        auto status_at =
            db_->Get(ropt, column_family, key, &value_at);
        ThreadState::SnapshotState snap_state = {
            snapshot, rand_column_family, column_family->GetName(),
            key_str, status_at, value_at};
        thread->snapshot_queue.emplace(
            std::min(FLAGS_ops_per_thread - 1, i + FLAGS_snapshot_hold_ops),
            snap_state);
      }
      ...
      if (prob_op >= 0 && prob_op < (int)FLAGS_readpercent) {
        // OPERATION read
        BeforeGet(thread, i, &cfhs, &keys);
        MultiGet(thread, read_opts, cfhs, keys);
      } else if ((int)FLAGS_readpercent <= prob_op && prob_op < prefixBound) {
        // OPERATION prefix scan
        // keys are 8 bytes long, prefix size is FLAGS_prefix_size. There are
        // (8 - FLAGS_prefix_size) bytes besides the prefix. So there will
        // be 2 ^ ((8 - FLAGS_prefix_size) * 8) possible keys with the same
        // prefix
        BeforePrefixScan(thread, i, &cfhs, &keys);
        MultiPrefixScan(thread, read_opts, cfhs, keys);
      } else if (prefixBound <= prob_op && prob_op < writeBound) {
        // OPERATION write
        std::vector<Slice> values;
        BeforePut(thread, i, &cfhs, &keys, &values);
        MultiPut(thread, write_opts, cfhs, keys, values);
      } else if (writeBound <= prob_op && prob_op < delBound) {
        // OPERATION delete
        BeforeDelete(thread, i, &cfhs, &keys);
        MultiDelete(thread, write_opts, cfhs, keys);
      } else if (delBound <= prob_op && prob_op < delRangeBound) {
        // OPERATION delete range
      } else {
        // OPERATION iterate
        BeforeIterate(thread, i, &cfhs, &keys);
        MultiIterate(thread, read_opts, cfhs, keys);
      }
      thread->stats.FinishedSingleOp();
    }
 ...
 protected:
  virtual void GenerateColumnFamilies(ThreadState* thread, int batch_size,
      std::vector<ColumnFamilyHandle*>* cfhs) const {
    // Choose at least one column family.
    cfhs->resize(batch_size);
    int start_cf_id = thread->rand.Next() % (FLAGS_column_families + 1 - batch_size);
    (*cfhs)[0] = column_families_[start_cf_id];
    for (int i = 1; i != batch_size; ++i) {
      (*cfhs)[i] = column_families_[++start_cf_id];
    }
  }

  virtual void BeforeGet(ThreadState* thread, uint64_t iteration,
      std::vector<ColumnFamilyHandle*>* cfhs, std::vector<Slice>* keys) const {
    int batch_size = thread->rand.Next() % FLAGS_column_families + 1;
    GenerateColumnFamilies(thread, batch_size, cfhs);
    keys->resize(batch_size);
    uint64_t rand_key = GenerateOneKey(thread, iteration);
    for (int i = 0; i != batch_size; ++i) {
      std::string key_str = std::to_string(i) + Key(rand_key);
      (*keys)[i] = Slice(key_str);
    }
  }

  virtual Status MultiGet(ThreadState* thread, const ReadOptions& read_opts,
  ...
  virtual void BeforePut(ThreadState* thread, uint64_t iteration,
      std::vector<ColumnFamilyHandle*>* cfhs,
      std::vector<Slice>* keys, std::vector<Slice>* values) const {
    int batch_size = thread->rand.Next() % FLAGS_column_families + 1;
    GenerateColumnFamilies(thread, batch_size, cfhs);
    keys->resize(batch_size);
    ...
    for (int i = 0; i != batch_size; ++i) {
      std::string key_str = std::to_string(i) + Key(rand_key);
      (*keys)[i] = Slice(key_str);
      uint32_t base_value = thread->rand.Next() % thread->shared->UNKNOWN_SENTINEL;
      size_t sz = GenerateValue(base_value, value, sizeof(value));
      (*values)[i] = Slice(value, sz);
    }
  }

  virtual Status MultiPut(ThreadState* thread, const WriteOptions& write_opts,
      const std::vector<ColumnFamilyHandle*>& column_families,
      const std::vector<Slice>& keys,
      const std::vector<Slice>& values) {
    WriteBatch batch;
    int batch_size = static_cast<int>(column_families.size());
    size_t nbytes = 0;
    for (int i = 0; i != batch_size; ++i) {
      if (FLAGS_use_merge) {
        batch.Merge(column_families[i], keys[i], values[i]);
      } else {
        batch.Put(column_families[i], keys[i], values[i]);
      }
      nbytes += (values[i].size() + 1);
    }
    Status s = db_->Write(write_opts, &batch);
    if (s.ok()) {
      thread->stats.AddBytesForWrites(batch_size, nbytes);
    } else {
      fprintf(stderr, "multiput error: %s\n", s.ToString().c_str());
      thread->stats.AddErrors(1);
    }
  ...
  virtual void BeforeDelete(ThreadState* thread, uint64_t iteration,
      std::vector<ColumnFamilyHandle*>* cfhs, std::vector<Slice>* keys) const {
    int batch_size = thread->rand.Next() % FLAGS_column_families + 1;
    GenerateColumnFamilies(thread, batch_size, cfhs);
    keys->resize(batch_size);
  ...
  virtual Status MultiDelete(ThreadState* thread,
  ...
  virtual void BeforePrefixScan(ThreadState* thread, uint64_t iteration,
      std::vector<ColumnFamilyHandle*>* cfhs, std::vector<Slice>* keys) const {
    int batch_size = thread->rand.Next() % FLAGS_column_families + 1;
    GenerateColumnFamilies(thread, batch_size, cfhs);
    keys->resize(batch_size);
    ...
    for (int i = 0; i != batch_size; ++i) {
      std::string key_str = std::to_string(i) + Key(rand_key);
      key_str.resize(FLAGS_prefix_size);
      (*keys)[i] = Slice(key_str);
    }
  }

  virtual Status MultiPrefixScan(ThreadState* thread,
  ...
  virtual void BeforeIterate(ThreadState* thread, uint64_t iteration,
      std::vector<ColumnFamilyHandle*>* cfhs, std::vector<Slice>* keys) const {
    int batch_size = thread->rand.Next() % FLAGS_column_families + 1;
    GenerateColumnFamilies(thread, batch_size, cfhs);
    keys->resize(batch_size);
  ...
  virtual Status MultiIterate(ThreadState* thread, const ReadOptions& read_opts,
      const std::vector<ColumnFamilyHandle*>& column_families,
      const std::vector<Slice>& keys) {
    (void) thread;
    (void) read_opts;
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_test_batches_snapshots) {
    ...
    stress.reset(new rocksdb::MultiCFConsistencyStressTest());
  } else {
