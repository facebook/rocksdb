...
#
# Environment options:
#  SHORT_TEST=1 - Test only the oldest branch for each kind of test
...
if ! git diff-index --quiet HEAD; then
  echo "You have modified files. Aborting."
  exit 1
fi

current_checkout_name=${1:-"$tmp_origin/master"}
# This allows the script to work even if with transient refs like "HEAD"
current_checkout_hash=`git rev-parse $current_checkout_name`
