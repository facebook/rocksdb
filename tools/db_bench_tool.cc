...
DEFINE_string(
    ...
    "\tmultiprefixexistsrandom -- check random prefix batches with a "
...
DEFINE_uint32(multiprefix_exists_hit_rate, 100,
              "Percentage of prefixes generated by multiprefixexistsrandom "
              "that match an existing key");
DEFINE_int32(multiprefix_exists_query_size, 0,
             "Prefix length queried by multiprefixexistsrandom; 0 uses "
             "prefix_size");
DEFINE_uint32(multiprefix_exists_duplicate_rate, 0,
              "Percentage of multiprefixexistsrandom queries copied from the "
              "previous query in the batch");
DEFINE_bool(multiprefix_exists_sort, false,
            "Sort each multiprefixexistsrandom batch before querying");
DEFINE_string(
    multiprefix_exists_mode, "api",
    "End-to-end strategy used by multiprefixexistsrandom: api, "
    "reuse_iterator, iterator_per_prefix, or multi_scan. Strategies "
    "may differ in sorting, deduplication, value materialization, and batch "
    "snapshot semantics; compare iterator strategies only on a static DB");
DEFINE_uint64(multiprefix_exists_prefix_stride, 1,
              "Stride between generated prefix IDs. Use the same value when "
              "creating the DB and running multiprefixexistsrandom");
DEFINE_bool(multiprefix_exists_fill_cache, true,
            "Whether multiprefixexistsrandom may populate the block cache");
...
static std::unordered_map<OperationType, std::string, std::hash<unsigned char>>
    OperationTypeString = {
        ...
        {kMultiPrefixExistsBatch, "multiprefixexists batch"}};
...
class Stats {
 private:
  ...
  std::string operation_rate_name_;
  std::string operation_count_name_;
 ...
 public:
  ...
  void Start(int id) {
    ...
    message_.clear();
    operation_name_ = "op";
    operation_rate_name_ = "ops";
    operation_count_name_ = "operations";
  ...
  void Merge(const Stats& other) {
    ...
    operation_name_ = other.operation_name_;
    operation_rate_name_ = other.operation_rate_name_;
    operation_count_name_ = other.operation_count_name_;
  ...
  void SetOperationUnits(const char* name, const char* rate_name,
                         const char* count_name) {
    operation_name_ = name;
    operation_rate_name_ = rate_name;
    operation_count_name_ = count_name;
  }
  ...
  void Report(const Slice& name) {
    ...
    fprintf(stdout,
            "%-12s : %11.3f micros/%s %ld %s/sec %.3f seconds %" PRIu64
            " %s;%s%s\n",
            name.ToString().c_str(), seconds_ * 1e6 / done_,
            operation_name_.c_str(), (long)throughput,
            operation_rate_name_.c_str(), elapsed, done_,
            operation_count_name_.c_str(), (extra.empty() ? "" : " "),
            extra.c_str());
    if (FLAGS_histogram) {
...
class CombinedStats {
 public:
  void AddStats(const Stats& stat) {
    ...
    operation_name_ = stat.operation_name_;
    operation_rate_name_ = stat.operation_rate_name_;
  ...
  void Report(const std::string& bench_name) {
    ...
    if (throughput_mbs_.size() == throughput_ops_.size()) {
      fprintf(stdout,
              "%s [AVG %d runs] : %d (\xC2\xB1 %d) %s/sec; %6.1f (\xC2\xB1 "
              "%.1f) MB/sec\n",
              name, num_runs, static_cast<int>(CalcAvg(throughput_ops_)),
              static_cast<int>(CalcConfidence95(throughput_ops_)),
              operation_rate_name_.c_str(), CalcAvg(throughput_mbs_),
              CalcConfidence95(throughput_mbs_));
    } else {
      fprintf(stdout, "%s [AVG %d runs] : %d (\xC2\xB1 %d) %s/sec\n", name,
              num_runs, static_cast<int>(CalcAvg(throughput_ops_)),
              static_cast<int>(CalcConfidence95(throughput_ops_)),
              operation_rate_name_.c_str());
    }
  }
  ...
  void ReportWithConfidenceIntervals(const std::string& bench_name) {
    ...
    if (throughput_mbs_.size() == throughput_ops_.size()) {
      ...
      fprintf(stdout,
              "%s [CI95 %d runs] : (%d, %d) %s/sec; (%.1f, %.1f) MB/sec\n",
              name, num_runs, ops_avg - ops_confidence_95,
              ops_avg + ops_confidence_95, operation_rate_name_.c_str(),
              mbs_avg - mbs_confidence_95, mbs_avg + mbs_confidence_95);
    } else {
      fprintf(stdout, "%s [CI95 %d runs] : (%d, %d) %s/sec\n", name, num_runs,
  ...
  void ReportFinal(const std::string& bench_name) {
    ...
    if (throughput_mbs_.size() == throughput_ops_.size()) {
      ...
      fprintf(stdout,
              "%s [AVG    %d runs] : %d (\xC2\xB1 %d) %s/sec; %.3f ms/%s; "
              "%6.1f (\xC2\xB1 "
              "%.1f) MB/sec\n"
              "%s [MEDIAN %d runs] : %d %s/sec; %6.1f MB/sec\n",
              name, num_runs, static_cast<int>(CalcAvg(throughput_ops_)),
              static_cast<int>(CalcConfidence95(throughput_ops_)),
              operation_rate_name_.c_str(), avg_millis_per_op,
              operation_name_.c_str(), CalcAvg(throughput_mbs_),
              CalcConfidence95(throughput_mbs_), name, num_runs,
              static_cast<int>(CalcMedian(throughput_ops_)),
              operation_rate_name_.c_str(), CalcMedian(throughput_mbs_));
    } else {
      fprintf(stdout,
              "%s [AVG    %d runs] : %d (\xC2\xB1 %d) %s/sec; %.3f ms/%s\n"
              "%s [MEDIAN %d runs] : %d %s/sec\n",
              name, num_runs, static_cast<int>(CalcAvg(throughput_ops_)),
              static_cast<int>(CalcConfidence95(throughput_ops_)),
              operation_rate_name_.c_str(), avg_millis_per_op,
              operation_name_.c_str(), name, num_runs,
              static_cast<int>(CalcMedian(throughput_ops_)),
              operation_rate_name_.c_str());
    }
  }
 ...
 private:
  ...
  std::string operation_name_ = "op";
  std::string operation_rate_name_ = "ops";
};
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  Benchmark()
    ...
    if (keys_per_prefix_ > 0 && FLAGS_multiprefix_exists_prefix_stride == 0) {
      fprintf(stderr, "multiprefix_exists_prefix_stride must be positive\n");
      db_bench_exit(1);
    }
  ...
  static void EncodeKeyPart(uint64_t value, int size, char* destination) {
    assert(size >= 0);
    assert(size <= 8);
    for (int i = 0; i < size; ++i) {
      destination[i] = static_cast<char>(value >> ((size - i - 1) * 8));
    }
  }
  ...
  void GenerateKeyFromInt(uint64_t v, int64_t num_keys, Slice* key) {
    ...
    if (keys_per_prefix_ > 0) {
      const uint64_t num_prefix =
          static_cast<uint64_t>(num_keys / keys_per_prefix_);
      if (num_prefix == 0) {
        fprintf(stderr, "keys_per_prefix must not exceed num\n");
        ErrorExit();
      }
      const uint64_t prefix_index = v % num_prefix;
      if (prefix_index > std::numeric_limits<uint64_t>::max() /
                             FLAGS_multiprefix_exists_prefix_stride) {
        fprintf(stderr, "Generated prefix ID exceeds uint64_t\n");
        ErrorExit();
      }
      const uint64_t prefix =
          prefix_index * FLAGS_multiprefix_exists_prefix_stride;
      int bytes_to_fill = std::min(prefix_size_, 8);
      EncodeKeyPart(prefix, bytes_to_fill, pos);
      if (prefix_size_ > 8) {
    ...
    EncodeKeyPart(v, bytes_to_fill, pos);
    pos += bytes_to_fill;
  ...
  void Run(ToolHooks& hooks) {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "multiprefixexistsrandom") {
        if (FLAGS_multiprefix_exists_mode != "api" &&
            FLAGS_multiprefix_exists_mode != "reuse_iterator" &&
            FLAGS_multiprefix_exists_mode != "iterator_per_prefix" &&
            FLAGS_multiprefix_exists_mode != "multi_scan") {
          fprintf(stderr,
                  "multiprefix_exists_mode must be api, reuse_iterator, "
                  "iterator_per_prefix, or multi_scan\n");
          ErrorExit();
        }
        if (FLAGS_multiprefix_exists_hit_rate > 100) {
          fprintf(stderr, "multiprefix_exists_hit_rate must be at most 100\n");
          ErrorExit();
        }
        if (FLAGS_multiprefix_exists_duplicate_rate > 100) {
          fprintf(stderr,
                  "multiprefix_exists_duplicate_rate must be at most 100\n");
          ErrorExit();
        }
        const int32_t query_size = FLAGS_multiprefix_exists_query_size == 0
                                       ? FLAGS_prefix_size
                                       : FLAGS_multiprefix_exists_query_size;
        if (FLAGS_prefix_size <= 0 || FLAGS_keys_per_prefix <= 0 ||
            query_size <= 0 || query_size > FLAGS_key_size ||
            FLAGS_num < FLAGS_keys_per_prefix || entries_per_batch_ <= 0 ||
            FLAGS_multiprefix_exists_prefix_stride == 0) {
          fprintf(stderr,
                  "multiprefixexistsrandom requires a positive batch size and "
                  "prefix stride, num >= keys_per_prefix > 0, positive "
                  "prefix_size, and a query size in [1, key_size]\n");
          ErrorExit();
        }
        if (FLAGS_use_existing_keys) {
          fprintf(stderr,
                  "multiprefixexistsrandom does not support "
                  "use_existing_keys\n");
          ErrorExit();
        }
        if (FLAGS_use_uint64_comparator) {
          fprintf(stderr,
                  "multiprefixexistsrandom requires bytewise key ordering\n");
          ErrorExit();
        }
        if (FLAGS_num_column_families != 1) {
          fprintf(stderr,
                  "multiprefixexistsrandom requires num_column_families=1\n");
          ErrorExit();
        }
        if (FLAGS_threads != 1 || FLAGS_num_multi_db > 1) {
          fprintf(stderr,
                  "multiprefixexistsrandom requires one thread and one DB\n");
          ErrorExit();
        }
        if (FLAGS_duration == 0 &&
            (reads_ <= 0 || reads_ % entries_per_batch_ != 0)) {
          fprintf(stderr,
                  "multiprefixexistsrandom reads must be positive and "
                  "divisible by batch_size when duration is zero\n");
          ErrorExit();
        }
        if (FLAGS_tolerate_io_errors_for_remote_dbs) {
          fprintf(stderr,
                  "multiprefixexistsrandom does not support tolerated I/O "
                  "errors\n");
          ErrorExit();
        }
        if (FLAGS_multiprefix_exists_hit_rate < 100) {
          const uint64_t num_existing_prefixes =
              static_cast<uint64_t>(FLAGS_num / FLAGS_keys_per_prefix);
          const size_t encoded_prefix_size =
              std::min<size_t>(FLAGS_prefix_size, 8);
          bool has_in_range_misses = false;
          if (static_cast<size_t>(query_size) >= encoded_prefix_size) {
            has_in_range_misses = FLAGS_multiprefix_exists_prefix_stride > 1 &&
                                  num_existing_prefixes > 1;
          } else {
            const unsigned int shift = static_cast<unsigned int>(
                8 * (encoded_prefix_size - static_cast<size_t>(query_size)));
            const uint64_t block = uint64_t{1} << shift;
            has_in_range_misses =
                FLAGS_multiprefix_exists_prefix_stride % block == 0 &&
                FLAGS_multiprefix_exists_prefix_stride / block > 1 &&
                num_existing_prefixes > 1;
          }
          if (!has_in_range_misses) {
            fprintf(stderr,
                    "multiprefixexistsrandom misses require sparse in-range "
                    "prefixes; increase multiprefix_exists_prefix_stride\n");
            ErrorExit();
          }
        }
        fprintf(stderr, "entries_per_batch = %" PRIi64 "\n",
                entries_per_batch_);
        fprintf(stderr, "multiprefix_exists_hit_rate = %u\n",
                FLAGS_multiprefix_exists_hit_rate);
        fprintf(stderr, "multiprefix_exists_query_size = %d\n", query_size);
        fprintf(stderr, "multiprefix_exists_duplicate_rate = %u\n",
                FLAGS_multiprefix_exists_duplicate_rate);
        fprintf(stderr, "multiprefix_exists_sort = %s\n",
                FLAGS_multiprefix_exists_sort ? "true" : "false");
        fprintf(stderr, "multiprefix_exists_mode = %s\n",
                FLAGS_multiprefix_exists_mode.c_str());
        fprintf(stderr, "multiprefix_exists_prefix_stride = %" PRIu64 "\n",
                FLAGS_multiprefix_exists_prefix_stride);
        fprintf(stderr, "multiprefix_exists_fill_cache = %s\n",
                FLAGS_multiprefix_exists_fill_cache ? "true" : "false");
        fprintf(stderr,
                "multiprefixexistsrandom reports requested prefixes/second; "
                "end-to-end latency samples cover one batch\n");
        method = &Benchmark::MultiPrefixExistsRandom;
...
// optimization, so skip the check of the max level.
  ...
  void MultiPrefixExistsRandom(ThreadState* thread) {
    if (user_timestamp_size_ > 0) {
      fprintf(stderr,
              "multiprefixexistsrandom does not support user timestamps\n");
      db_bench_exit(1);
    }

    int64_t checked = 0;
    int64_t found = 0;
    int64_t unique_checked = 0;
    int64_t misses = 0;
    const size_t query_size =
        FLAGS_multiprefix_exists_query_size == 0
            ? static_cast<size_t>(prefix_size_)
            : static_cast<size_t>(FLAGS_multiprefix_exists_query_size);
    const size_t encoded_prefix_size = std::min<size_t>(prefix_size_, 8);
    const uint64_t num_keys = static_cast<uint64_t>(FLAGS_num);
    const uint64_t keys_per_prefix = static_cast<uint64_t>(keys_per_prefix_);
    const uint64_t num_existing_prefixes = num_keys / keys_per_prefix;
    const uint64_t prefix_id_stride = FLAGS_multiprefix_exists_prefix_stride;
    if (num_existing_prefixes - 1 >
        std::numeric_limits<uint64_t>::max() / prefix_id_stride) {
      fprintf(stderr, "Generated prefix IDs exceed uint64_t\n");
      ErrorExit();
    }
    const uint64_t max_existing_prefix =
        (num_existing_prefixes - 1) * prefix_id_stride;
    const uint64_t max_encoded_prefix =
        encoded_prefix_size < 8 ? (uint64_t{1} << (8 * encoded_prefix_size)) - 1
                                : std::numeric_limits<uint64_t>::max();
    if (max_existing_prefix > max_encoded_prefix) {
      fprintf(stderr,
              "Generated prefix IDs do not fit in the configured prefix "
              "size\n");
      ErrorExit();
    }
    const size_t prefix_stride = key_size_;
    std::vector<char> prefix_buffers(entries_per_batch_ * prefix_stride);
    std::vector<Slice> prefixes;
    std::vector<unsigned char> expected_results(entries_per_batch_);
    std::vector<unsigned char> actual_results(entries_per_batch_);
    std::vector<size_t> sort_order(entries_per_batch_);
    std::vector<Slice> sorted_prefixes(entries_per_batch_);
    std::vector<unsigned char> sorted_expected_results(entries_per_batch_);
    prefixes.reserve(entries_per_batch_);
    for (int64_t i = 0; i < entries_per_batch_; ++i) {
      prefixes.emplace_back(prefix_buffers.data() + i * prefix_stride,
                            key_size_);
    }
    ...
    Mode mode = Mode::kApi;
    if (FLAGS_multiprefix_exists_mode == "reuse_iterator") {
      mode = Mode::kReuseIterator;
    } else if (FLAGS_multiprefix_exists_mode == "iterator_per_prefix") {
      mode = Mode::kIteratorPerPrefix;
    } else if (FLAGS_multiprefix_exists_mode == "multi_scan") {
      mode = Mode::kMultiScan;
    }
    std::vector<Status> statuses;
    if (mode == Mode::kApi) {
      statuses.resize(entries_per_batch_);
    }
    auto io_dispatcher =
        mode == Mode::kMultiScan ? MaybeCreateIODispatcher() : nullptr;

    const auto get_upper_bound = [](const Slice& prefix, std::string* storage) {
      storage->assign(prefix.data(), prefix.size());
      for (size_t i = storage->size(); i > 0; --i) {
        unsigned char byte = static_cast<unsigned char>((*storage)[i - 1]);
        if (byte != 0xff) {
          (*storage)[i - 1] = static_cast<char>(byte + 1);
          storage->resize(i);
          return true;
        }
      }
    ...
    const auto check_iterator = [&](Iterator* iterator, const Slice& prefix) {
      if (!iterator->status().ok()) {
        HandleBenchmarkIOError(iterator->status(),
                               "Iterator returned an error");
        return false;
      }
      return iterator->Valid() && iterator->key().starts_with(prefix);
    };

    ReadOptions benchmark_read_options(read_options_);
    benchmark_read_options.fill_cache = FLAGS_multiprefix_exists_fill_cache;

    DB* const db = SelectDB(thread);
    const auto verify_existing_key = [&](uint64_t key_index) {
      std::vector<char> key_buffer(key_size_);
      Slice key(key_buffer.data(), key_buffer.size());
      GenerateKeyFromInt(key_index, FLAGS_num, &key);
      std::string value;
      Status status = db->Get(benchmark_read_options, db->DefaultColumnFamily(),
                              key, &value);
      if (!status.ok()) {
        fprintf(stderr,
                "multiprefixexistsrandom expected generated key index %" PRIu64
                " in the database: %s\n",
                key_index, status.ToString().c_str());
        ErrorExit();
      }
    };
    verify_existing_key(0);
    verify_existing_key(num_existing_prefixes - 1);
    verify_existing_key(num_keys - 1);

    thread->stats.Start(thread->tid);
    thread->stats.SetOperationUnits("prefix", "prefixes", "prefixes");
    auto duration = thread->shared->MakeDuration(FLAGS_duration, reads_);
    while (!duration.Done(entries_per_batch_)) {
      for (int64_t i = 0; i < entries_per_batch_; ++i) {
        if (i > 0 && thread->rand.Uniform(100) <
                         FLAGS_multiprefix_exists_duplicate_rate) {
          prefixes[i] = prefixes[i - 1];
          expected_results[i] = expected_results[i - 1];
          continue;
        }
        char* const prefix_buffer = prefix_buffers.data() + i * prefix_stride;
        Slice key(prefix_buffer, key_size_);
        const bool expected_found =
            thread->rand.Uniform(100) < FLAGS_multiprefix_exists_hit_rate;
        if (expected_found) {
          GenerateKeyFromInt(GetRandomKey(&thread->rand), FLAGS_num, &key);
        } else {
          uint64_t missing_prefix;
          if (query_size < encoded_prefix_size) {
            const unsigned int shift = static_cast<unsigned int>(
                8 * (encoded_prefix_size - query_size));
            const uint64_t block = uint64_t{1} << shift;
            const uint64_t class_stride = prefix_id_stride / block;
            assert(class_stride > 1);
            const uint64_t existing_index =
                thread->rand.Uniform(num_existing_prefixes - 1);
            const uint64_t missing_class =
                existing_index * class_stride + 1 +
                thread->rand.Uniform(class_stride - 1);
            missing_prefix = missing_class * block;
          } else {
            assert(prefix_id_stride > 1);
            const uint64_t existing_index =
                thread->rand.Uniform(num_existing_prefixes - 1);
            missing_prefix = existing_index * prefix_id_stride + 1 +
                             thread->rand.Uniform(prefix_id_stride - 1);
          }

          memset(prefix_buffer, '0', key_size_);
          EncodeKeyPart(missing_prefix, static_cast<int>(encoded_prefix_size),
                        prefix_buffer);
        }
        prefixes[i] = Slice(prefix_buffer, query_size);
        expected_results[i] = expected_found;
      }
      if (FLAGS_multiprefix_exists_sort) {
        ...
        for (size_t i = 0; i < prefixes.size(); ++i) {
          sorted_prefixes[i] = prefixes[sort_order[i]];
          sorted_expected_results[i] = expected_results[sort_order[i]];
        }
        prefixes.swap(sorted_prefixes);
        expected_results.swap(sorted_expected_results);
      }
      std::fill(actual_results.begin(), actual_results.end(), 0);

      if (mode == Mode::kApi) {
        db->MultiPrefixExists(benchmark_read_options, db->DefaultColumnFamily(),
                              prefixes.size(), prefixes.data(),
                              statuses.data());
        for (size_t i = 0; i < statuses.size(); ++i) {
          ...
          if (status.ok()) {
            ...
            HandleBenchmarkIOError(status,
                                   "MultiPrefixExists returned an error");
          }
        }
      } else if (mode == Mode::kReuseIterator) {
        const auto can_prefix_seek = [&](const Slice& prefix) {
          return open_options_.prefix_extractor != nullptr &&
                 open_options_.prefix_extractor->SameResultWhenAppended(prefix);
        };
        bool has_prefix_seek = false;
        bool has_total_order_seek = false;
        for (const Slice& prefix : prefixes) {
          const bool prefix_seek = can_prefix_seek(prefix);
          has_prefix_seek |= prefix_seek;
          has_total_order_seek |= !prefix_seek;
        }

        const auto check_group = [&](bool prefix_seek) {
          ReadOptions options(benchmark_read_options);
          options.allow_unprepared_value = true;
          options.auto_prefix_mode = false;
          options.total_order_seek = !prefix_seek;
          options.prefix_same_as_start = prefix_seek;
          std::unique_ptr<Iterator> iterator(
              db->NewIterator(options, db->DefaultColumnFamily()));
          bool positioned = false;
          for (size_t i = 0; i < prefixes.size(); ++i) {
            const Slice& prefix = prefixes[i];
            if (can_prefix_seek(prefix) != prefix_seek) {
              continue;
            }
            if (!positioned || !FLAGS_multiprefix_exists_sort ||
                !iterator->Valid() || iterator->key().compare(prefix) < 0) {
              iterator->Seek(prefix);
              positioned = true;
            }
            actual_results[i] = check_iterator(iterator.get(), prefix);
            if (!iterator->status().ok()) {
              break;
            }
          }
        ...
        if (has_prefix_seek) {
          check_group(/*prefix_seek=*/true);
        }
        if (has_total_order_seek) {
          check_group(/*prefix_seek=*/false);
        }
      } else if (mode == Mode::kIteratorPerPrefix) {
        for (size_t i = 0; i < prefixes.size(); ++i) {
          const Slice& prefix = prefixes[i];
          ReadOptions options(benchmark_read_options);
          options.allow_unprepared_value = true;
          ...
          Slice upper_bound;
          if (get_upper_bound(prefix, &upper_bound_storage)) {
            upper_bound = upper_bound_storage;
            options.iterate_upper_bound = &upper_bound;
            const bool prefix_seek =
                open_options_.prefix_extractor != nullptr &&
                open_options_.prefix_extractor->SameResultWhenAppended(prefix);
            options.total_order_seek = !prefix_seek;
            options.prefix_same_as_start = prefix_seek;
          } else {
        ...
        std::vector<Slice> scan_prefixes(prefixes);
        if (!FLAGS_multiprefix_exists_sort) {
          std::sort(scan_prefixes.begin(), scan_prefixes.end(),
                    [](const Slice& lhs, const Slice& rhs) {
                      return lhs.compare(rhs) < 0;
                    });
        }
        size_t unique_count = 0;
        for (const Slice& prefix : scan_prefixes) {
          if (unique_count > 0 && prefix == scan_prefixes[unique_count - 1]) {
            continue;
          } else {
            scan_prefixes[unique_count++] = prefix;
          }
        }
        scan_prefixes.resize(unique_count);
        std::vector<unsigned char> scan_results(scan_prefixes.size(), 0);

        MultiScanArgs scan_options(open_options_.comparator);
        scan_options.io_coalesce_threshold = FLAGS_multiscan_coalesce_threshold;
        scan_options.use_async_io = FLAGS_multiscan_use_async_io;
        scan_options.max_prefetch_size = FLAGS_multiscan_max_prefetch_size;
        scan_options.io_dispatcher = io_dispatcher;
        scan_options.reserve(scan_prefixes.size());
        std::vector<std::string> upper_bound_storage(scan_prefixes.size());
        for (size_t i = 0; i < scan_prefixes.size(); ++i) {
          if (get_upper_bound(scan_prefixes[i], &upper_bound_storage[i])) {
            scan_options.insert(scan_prefixes[i], upper_bound_storage[i]);
          } else {
            scan_options.insert(scan_prefixes[i]);
          }
        }
        auto multi_scan = db->NewMultiScan(
            benchmark_read_options, db->DefaultColumnFamily(), scan_options);
        try {
          size_t range_index = 0;
          for (auto scan : *multi_scan) {
            for ([[maybe_unused]] auto entry : scan) {
              scan_results[range_index] = 1;
              break;
            }
            ++range_index;
          }
          assert(range_index == scan_prefixes.size());
        } catch (MultiScanException& e) {
          HandleBenchmarkIOError(e.status(), "MultiScan returned an error");
        }
        for (size_t i = 0; i < prefixes.size(); ++i) {
          const auto it = std::lower_bound(
              scan_prefixes.begin(), scan_prefixes.end(), prefixes[i],
              [](const Slice& lhs, const Slice& rhs) {
                return lhs.compare(rhs) < 0;
              });
          assert(it != scan_prefixes.end());
          assert(*it == prefixes[i]);
          actual_results[i] =
              scan_results[static_cast<size_t>(it - scan_prefixes.begin())];
        }
      }
      ...
      for (size_t i = 0; i < prefixes.size(); ++i) {
        if (actual_results[i] != expected_results[i]) {
          fprintf(stderr,
                  "multiprefixexistsrandom result mismatch at index %zu: "
                  "expected %u, got %u\n",
                  i, static_cast<unsigned int>(expected_results[i]),
                  static_cast<unsigned int>(actual_results[i]));
          ErrorExit();
        }
        found += actual_results[i];
        if (!expected_results[i]) {
          ++misses;
        }
      }
      if (FLAGS_multiprefix_exists_sort) {
        int64_t batch_unique = 1;
        for (size_t i = 1; i < prefixes.size(); ++i) {
          batch_unique += prefixes[i] != prefixes[i - 1];
        }
        unique_checked += batch_unique;
      }
      checked += entries_per_batch_;
    ...
    if (FLAGS_multiprefix_exists_sort) {
      snprintf(msg, sizeof(msg),
               "(mode:%s, %" PRIi64 " of %" PRIi64 " found, %" PRIi64
               " unique prefixes, %" PRIi64 " sparse in-range misses)",
               FLAGS_multiprefix_exists_mode.c_str(), found, checked,
               unique_checked, misses);
    } else {
      snprintf(msg, sizeof(msg),
               "(mode:%s, %" PRIi64 " of %" PRIi64
               " found, unique prefixes not counted, %" PRIi64
               " sparse in-range misses)",
               FLAGS_multiprefix_exists_mode.c_str(), found, checked, misses);
    }
    thread->stats.AddMessage(msg);
  }
