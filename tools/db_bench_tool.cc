...
DEFINE_double(key_dist_b, 0.0,
              ...
              "f(x)=a*exp(b*x)+c*exp(d*x)");
DEFINE_double(key_dist_c, 0.0,
              "The parameter 'c' of key access distribution model "
              "f(x)=a*exp(b*x)+c*exp(d*x)");
DEFINE_double(key_dist_d, 0.0,
              "The parameter 'd' of key access distribution model"
              "f(x)=a*exp(b*x)+c*exp(d*x)");
...
// key access distribution of the following model:
// f(x) = a*exp(b*x) + c*exp(d*x)
// x is the access count of the key or the rank of
// prefix that has the average access count of f(x);
// The key access distribution will be generated by sorting the
// access count of each key from high to low and fit it to the
// model. The prefix model is generated by fitting the sorted
// prefix average access count in descending order

// The unit for each key, it is used in a vector to transfer
// the uniform access to the keys with different access probability
struct ExpKeyUnit {
  int64_t key_start;
  int64_t key_access;
};

// The unit for each prefix, it is used in a vector to transfer a
// random access to one of the prefix. And combining the offset in
// the prefix, we will generate the key ID
struct PrefixUnit {
...
// The class to use two term exponential distribution
// to generate the key access based on the distribution
class GenerateTwoTermExpKeys {
 public:
  ...
  bool initiated_;
  std::vector<ExpKeyUnit> key_set_;
  std::vector<PrefixUnit> prefix_set_;

  GenerateTwoTermExpKeys() {
    prefix_rand_max_ = FLAGS_num;
    key_rand_max_ = FLAGS_num;
    initiated_ = false;
  }

  ~GenerateTwoTermExpKeys() {}

  // Initiate the prefix-key access model. When we get the uniform distribution
  // access ini_rand, we will map it to one of the keys. The access will follow
  // the prefix access distribution and key access distribution
  Status InitiateExpDistribution(const int64_t total_keys, const double a,
    ...
    initiated_ = true;
    prefix_size_ = total_keys / FLAGS_prefix_num;

    for (int64_t pfx = FLAGS_prefix_num; pfx >= 1; pfx--) {
      double prefix_p = prefix_a * std::exp(prefix_b * pfx) +
                        prefix_c * std::exp(prefix_d * pfx);

      // To bound the prefix_rand_max_
    ...
    // Initiate the vector of keys in one prefix
    amplify = 0;
    int64_t key_start = 0;
    for (int64_t key = prefix_size_; key >= 1; key--) {
      double exp_key = a * std::exp(b * key) + c * std::exp(d * key);
      if (exp_key < std::pow(10.0, -16.0)) {
        exp_key = 0.0;
      }
      ...
      ExpKeyUnit tmp_unit;
      tmp_unit.key_start = key_start;
      if (0 >= exp_key) {
        tmp_unit.key_access = 0;
      } else {
        tmp_unit.key_access =
            static_cast<int64_t>(std::floor(amplify * exp_key));
      }
      key_set_.push_back(tmp_unit);
      key_start += tmp_unit.key_access;
    }
    key_rand_max_ = key_start;

    // shuffle the key access vector
    Random64 rand_pos(key_rand_max_);
    for (int64_t i = 0; i < prefix_size_; i++) {
      int64_t pos = rand_pos.Next() % prefix_size_;
      std::swap(key_set_[i], key_set_[pos]);
    }

    // Recalculate the buck start of each key
    offset = 0;
    for (auto& k_unit : key_set_) {
      k_unit.key_start = offset;
      offset += k_unit.key_access;
    }
  ...
  int64_t DistGetKeyID(const int64_t& ini_rand) {
    ...
    // Get the prefix of this generated random number
    ...
    // Get the key offset in the prefix
...
// optimization, so skip the check of the max level.
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    if (FLAGS_key_dist_a != 0.0 || FLAGS_key_dist_c != 0.0 ||
