...
SOURCE_ROOT = ROOT / "db"
GENERATED_INCLUDE_ROOT = INCLUDE_ROOT / "c_api_gen"
GENERATED_SOURCE_ROOT = SOURCE_ROOT / "c_api_gen"
...
HEADER_INCLUDE_RE = re.compile(r'^#include "rocksdb/([^"]+\.inc)"$', re.MULTILINE)
SOURCE_INCLUDE_RE = re.compile(r'^#include "db/([^"]+\.inc)"$', re.MULTILINE)
...
def collect_function_names(root_file: Path, include_re: re.Pattern[str]) -> set[str]:
    ...
    for rel in include_re.findall(root_file.read_text()):
        include_path = root_file.parent / Path(rel)
...
def collect_existing_bindings() -> tuple[set[str], set[str]]:
    decls = collect_function_names(INCLUDE_ROOT / "c.h", HEADER_INCLUDE_RE)
    defs = collect_function_names(SOURCE_ROOT / "c.cc", SOURCE_INCLUDE_RE)
    return decls, defs
