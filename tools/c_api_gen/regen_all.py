...
def _verify_clang(clang: str) -> str:
    """Verify the given clang++ actually runs before any files are written.

    ``clang`` is always supplied by `make regen-c-api` (via CLANG_CXX, resolved
    by build_tools/build_detect_platform); this script is a codegen step, not
    meant to be run by hand. Verifying up front means a broken or missing
    toolchain aborts cleanly instead of leaving a partially regenerated tree.
    """
    try:
        subprocess.run(
            [clang, "--version"],
        ...
        raise SystemExit(
            f"error: clang++ '{clang}' is not usable ({exc}). No files were "
            "modified. Run `make regen-c-api`, which supplies a working "
            "CLANG_CXX."
        )
    return clang
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="clang++",
        help=(
            "clang++ binary used to parse C++ headers for the auto-generated "
            "bindings (default: clang++). `make regen-c-api` overrides this "
            "with CLANG_CXX from build_detect_platform."
        ),
    )
    args = parser.parse_args()

    # Verify the clang++ actually runs BEFORE writing any files, so a missing
    # or broken toolchain aborts cleanly instead of leaving a partial tree.
    clang = _verify_clang(args.clang)

    for sections, header_out, source_out in JOBS:
        run_job(sections, header_out, source_out)
    subprocess.run(
        [sys.executable, str(AUTO_GEN), "--clang", clang], cwd=ROOT, check=True
    )
