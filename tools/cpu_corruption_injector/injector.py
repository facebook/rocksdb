...
from injector_register_corruption import apply_corruption, CorruptedInstruction  # noqa: E402
...
def parse_args() -> Args:
    ...
    parser.add_argument("--corruptions_per_op", type=int, default=1)
    ...
    return Args(
        ...
        corruptions_per_op=max(parsed.corruptions_per_op, 1),
        dir=parsed.dir,
        seed=parsed.seed,
    )
...
def run(record: InjectRecord, args: Args) -> None:
    ...
    while site.reach_target_fn_call():
        corruptions, record.critical_instructions_seen = _corrupt_in_call(
            site, critical_instruction_index, args.corruptions_per_op, rng
        )
        if corruptions:
            record.corruptions = corruptions
            record.injection_result = "injected"
            write_inject_record_json(
                out_path, record
            )  # checkpoint: the injection happened
            break
    _finish_run_and_write(record, out_path, site)
...
def _pick_critical_instruction_to_corrupt(
    ...
    while True:
        critical_instruction_count = sum(1 for _ in site.critical_instructions())
...
def _corrupt_in_call(
    site: nav.Navigator,
    critical_instruction_index: int,
    corruptions_per_op: int,
    rng: random.Random,
) -> tuple[list[CorruptedInstruction], int]:
    """Corrupt up to corruptions_per_op critical instructions of the current target_fn call,
    starting at critical_instruction_index. Returns the corruptions (empty if the call had
    no critical instruction at that index) and the call's critical instruction count.

    We do not check db_stress's state here -- site.critical_instructions() owns that:
    if a corruption crashes db_stress (or db_stress makes its own exit), the generator's
    next step sees it and stops, ending this loop. Our only policy is the corruptions_per_op
    cap."""
    corruptions: list[CorruptedInstruction] = []
    critical_instructions_seen = 0
    for index, instruction in site.critical_instructions():
        # index is 0-based; this ends as the call's total critical count when no corruption
        # lands (the loop runs to the end), so a no-corruption outcome shows whether
        # critical_instruction_index was even reachable.
        critical_instructions_seen = index + 1
        if index < critical_instruction_index:
            continue
        corruptions.append(apply_corruption(instruction, rng, site.target_fn))
        if len(corruptions) == corruptions_per_op:
            break  # cap at corruptions_per_op; fewer is fine (the call may run out first)
    return corruptions, critical_instructions_seen
