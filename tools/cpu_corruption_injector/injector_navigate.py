...
from injector_critical_instruction import (
    classify_current_instruction,
    CriticalInstruction,
)
...
# A single gdb `stepi` is slow (~ms), and the outer per-run timeout (runner_execute
# RUN_TIMEOUT) would kill the WHOLE run if one pathological target_fn call (a giant
# key loop) stepped forever. This per-call cap lets us give up on one such call and
# move to the next target_fn call, still producing an injection within the timeout.
_MAX_STEPS_PER_CALL = int(os.environ.get("ICC_MAX_OP_STEPS", "20000"))
...
#
# Lifecycle of one run (db_stress runs UNDER gdb throughout); every path ends EXITED:
#
#     HEALTHY ──a corruption's bad value is used─────────────────────► CRASHED
#        │                                                          │
#        │ finish_db_stress: let db_stress make its own exit,       │ finish_db_stress:
#        │ which itself either exits or crashes ────────────────────► kill it (the only
#        │                                                          │  way to end a crash)
#        ▼                                                          ▼
#     EXITED                                                     EXITED
#
# finish_db_stress() is the one place that drives whatever state we are in to EXITED
# (and the only place a still-running db_stress is ended).
EXITED = "exited"
HEALTHY = "healthy"
...
class Navigator:
    ...
    def critical_instructions(self) -> Iterator[tuple[int, CriticalInstruction]]:
        """Single-step the CURRENT target_fn call, yielding (index, instruction) for
        each critical (flippable value-movement) instruction in it.

        `index` counts only YIELDED instructions: if the call runs 5 instructions and
        3 are critical, they come out as index 0, 1, 2. That is the instruction index
        space the warmup picks a corruption target in -- a corruption can only land on a critical
        instruction, so indexing over just those is exactly what callers need.

        Stops when db_stress is no longer HEALTHY (it exited, or a corruption mid-call
        crashed it), or this target_fn call returns, or the per-call step cap is hit."""
        ...
        for _ in range(_MAX_STEPS_PER_CALL):
            ...
            if instruction is not None:
                ...
                index += 1
            # _step_one_instruction returns db_stress's state after the step (so we do
            # not re-query info program). Stop if it is not HEALTHY (exited / crashed),
            # or this target_fn call returned -- depth < call_start_depth means
            # target_fn's own frame popped.
