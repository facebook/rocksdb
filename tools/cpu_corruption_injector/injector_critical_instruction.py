...
class CriticalInstruction:
    ...
    corruption_type    -- how we corrupt it (this is the corruption_type in
                          inject.json):
                            ...
                            lane_flip -- flip a 64-bit lane of a vector register
...
def _gdb_can_corrupt_int64_lane(reg: str) -> bool:
    ...
    breakdown: if lane_flip stays near zero, we are losing SIMD coverage here. TODO:
...
def _critical_instruction_for_memory_move(
    ...
    # register -> lane_flip (only if gdb can address its int64 lanes), else a
    # general-purpose register -> bit_flip.
    if "xmm" in reg or "ymm" in reg or "zmm" in reg:
        return (
            CriticalInstruction(reg, "lane_flip", after_exec, text)
