...
def _extract_relevant(params):
    ...
    keys = [
        ...
        "use_multiscan", "prefix_size", "prefixpercent", "iterpercent",
        "readpercent",
    ]
    return {k: params.get(k) for k in keys if params.get(k) is not None}


def _check_invariants(params):
    """Return db_stress flag invariants violated by finalized params."""
    failures = []
    if params.get("prefixpercent", 0) > 0 and params.get("prefix_size", 0) < 0:
        failures.append("prefixpercent is non-zero while prefix_size is negative")
    return failures
...
def worker(args):
    ...
    for i in range(num_trials):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            for failure in _check_invariants(result):
                invariant_failures.append({
                    "seed": seed,
                    "failure": failure,
                    "params": _extract_relevant(result),
                })
...
def main():
    ...
    for r in results:
        ...
        total_invariants.extend(r["invariant_failures"])
    ...
    print(f"  Invariant failures:   {len(total_invariants)}")
    print()
    ...
    if total_invariants:
        print(f"\nINVARIANT FAILURES ({len(total_invariants)} found):")
        seen = set()
        for fail in total_invariants[:20]:
            sig = fail["failure"]
            if sig not in seen:
                seen.add(sig)
                print(f"  Seed {fail['seed']}: {fail['failure']}")
                for k, v in sorted(fail["params"].items()):
                    print(f"    {k}={v}")

    if not total_oscillations and not total_idempotency and not total_invariants:
        print("ALL CLEAR -- no oscillations, idempotency failures, or invariant failures!")
    ...
    with open(result_file, "w") as f:
        json.dump({
            ...
            "invariant_failures": total_invariants,
        }, f, indent=2)
    print(f"\nDetailed results: {result_file}")

    sys.exit(1 if total_oscillations or total_idempotency or total_invariants else 0)
