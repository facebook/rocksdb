  ...
  if (!db_) {
    ...
    switch (type) {
      case kWalFile:
        ...
        break;
      case kTableFile:
        DumpSstFile(options_, path_, is_key_hex_, /* show_properties */ true, /* decode_blob_index */ false);
        break;
...
DBFileDumperCommand::DBFileDumperCommand(
    ...
    const std::map<std::string, std::string>& options,
    const std::vector<std::string>& flags)
    : LDBCommand(options, flags, true, BuildCmdLineOptions({ARG_DECODE_BLOB_INDEX})),
