  ...
  while (NextOptions(FLAGS_bucket_count)) {
    ...
    for (auto prefix : prefixes) {
       for (uint64_t sorted = 0; sorted < FLAGS_items_per_prefix; sorted++) {
        ...
        hist_put_comparison.Add(GetPerfContext()->user_key_comparison_count);
      }
    }
    ...
    for (auto prefix : prefixes) {
      ...
      hist_seek_comparison.Add(GetPerfContext()->user_key_comparison_count);
    ...
    for (auto prefix = FLAGS_total_prefixes;
      ...
      hist_no_seek_comparison.Add(GetPerfContext()->user_key_comparison_count);
      ASSERT_TRUE(!iter->Valid());
    }
