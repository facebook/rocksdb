...
BlobFilePartitionManager::BlobFilePartitionManager(
  ...
  if (buffer_size_ > 0) {
    ...
    for (uint32_t i = 0; i < num_bg_threads; ++i) {
      bg_threads_.emplace_back(&BlobFilePartitionManager::BackgroundIOLoop,
                               this);
    }
  }
}
...
Status BlobFilePartitionManager::CloseBlobFile(Partition* partition) {
  ...
  if (buffer_size_ > 0 && !partition->pending_records.empty()) {
    ...
    Status flush_err =
        FlushRecordsToDisk(writer, partition, records, &records_written);
...
Status BlobFilePartitionManager::SealDeferredFile(Partition* partition,
  ...
  size_t records_written = 0;
  Status write_err = FlushRecordsToDisk(writer, partition, deferred->records,
                                        &records_written);
...
void BlobFilePartitionManager::BackgroundIOLoop() {
  while (true) {
          ...
          for (auto& p : partitions_) {
            if (p->pending_bytes.load(std::memory_order_relaxed) > 0 &&
              ...
              for (const auto& item : p->bg_queue) {
                if (item.type == BGWorkItem::kBGFlush) {
                  has_flush = true;
                  break;
                }
              }
    ...
    for (auto& item : items) {
      ...
      } else if (item.type == BGWorkItem::kBGFlush) {
...
Status BlobFilePartitionManager::FlushRecordsToDisk(
    ...
    std::deque<PendingRecord>& records, size_t* records_written) {
  assert(writer);
...
void BlobFilePartitionManager::AddFilePartitionMapping(uint64_t file_number,
  ...
  std::unique_lock<std::shared_mutex> lock(file_partition_mutex_);
...
Status BlobFilePartitionManager::FlushPendingRecords(Partition* partition) {
  ...
  //    BG queue, but the DeferredSeal keeps the writer alive until the BG
  //    thread processes it (sequentially, after this flush completes).
...
Status BlobFilePartitionManager::SealAllPartitions(
  ...
  if (blobs_written_since_seal_.exchange(0, std::memory_order_acq_rel) == 0) {
  ...
  for (auto& [partition, seal] : seals) {
    ...
    size_t records_written = 0;
    Status s =
        FlushRecordsToDisk(writer, partition, seal.records, &records_written);
    ...
    if (s.ok()) {
      ...
      std::string checksum_method;
      std::string checksum_value;
      WriteOptions footer_wo;
      s = writer->AppendFooter(footer_wo, footer, &checksum_method,
                               &checksum_value);
      if (s.ok()) {
        ...
        if (blob_callback_) {
          ...
          blob_callback_->OnBlobFileCompleted(
              file_path, /*column_family_name=*/"", /*job_id=*/0,
              seal.file_number, BlobFileCreationReason::kDirectWrite, s,
