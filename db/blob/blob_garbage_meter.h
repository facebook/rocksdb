...
class BlobGarbageMeter {
 public:
  ...
  using BlobFileSchemaVersions = std::unordered_map<uint64_t, uint8_t>;

  // Collects per-file schema versions from manifest-visible blob metadata.
  static BlobFileSchemaVersions CollectBlobFileSchemaVersions(
      const VersionStorageInfo* version_storage_info);

  // `use_blog_format` is only a fallback for blob files that are not yet
  // manifest-visible (e.g. write-path-owned files during direct-write flush).
  explicit BlobGarbageMeter(bool use_blog_format,
                            BlobFileSchemaVersions blob_file_schema_versions =
                                BlobFileSchemaVersions());
 ...
 private:
  ...
  uint8_t default_blob_file_schema_version_;
  BlobFileSchemaVersions blob_file_schema_versions_;
