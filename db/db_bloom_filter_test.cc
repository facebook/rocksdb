...
TEST_P(BloomStatsTestWithParam, BloomStatsTest) {
  ...
  ASSERT_EQ(1, GetPerfContext()->bloom_memtable_hit_count);
  ASSERT_EQ(value3, Get(key3));
  ASSERT_EQ(2, GetPerfContext()->bloom_memtable_hit_count);
  ASSERT_EQ(0, GetPerfContext()->bloom_memtable_miss_count);

  ASSERT_EQ("NOT_FOUND", Get(key2));
  ASSERT_EQ(1, GetPerfContext()->bloom_memtable_miss_count);
  ASSERT_EQ(2, GetPerfContext()->bloom_memtable_hit_count);

  // sanity checks
  ASSERT_EQ(0, GetPerfContext()->bloom_sst_hit_count);
  ASSERT_EQ(0, GetPerfContext()->bloom_sst_miss_count);
  ...
  ASSERT_EQ(value1, Get(key1));
  ASSERT_EQ(1, GetPerfContext()->bloom_sst_hit_count);
  ASSERT_EQ(value3, Get(key3));
  ASSERT_EQ(2, GetPerfContext()->bloom_sst_hit_count);

  ASSERT_EQ("NOT_FOUND", Get(key2));
  ASSERT_EQ(1, GetPerfContext()->bloom_sst_miss_count);
}
