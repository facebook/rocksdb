...
TEST_F(DBBloomFilterTest, GetFilterByPrefixBloomCustomPrefixExtractor) {
  for (bool partition_filters : {true, false}) {
    ...
    ASSERT_EQ(0, get_perf_context()->perf_context_by_level[0].bloom_filter_useful);
    ...
    ASSERT_EQ(1, get_perf_context()->perf_context_by_level[0].bloom_filter_useful);

    ASSERT_EQ("NOT_FOUND", Get("foobarbar"));
    ASSERT_EQ(TestGetTickerCount(options, BLOOM_FILTER_USEFUL), 2);
    ASSERT_EQ(2, get_perf_context()->perf_context_by_level[0].bloom_filter_useful);
...
TEST_F(DBBloomFilterTest, WholeKeyFilterProp) {
  for (bool partition_filters : {true, false}) {
    ...
    ASSERT_EQ("bar", Get("barfoo"));
    ASSERT_EQ(TestGetTickerCount(options, BLOOM_FILTER_USEFUL), 12);
    ASSERT_EQ(12, get_perf_context()->perf_context_by_level[0].bloom_filter_useful);
    get_perf_context()->Reset();
    get_perf_context()->DisablePerLevelPerfContext();
  }
}
...
TEST_F(DBBloomFilterTest, BloomFilterRate) {
  while (ChangeFilterOptions()) {
    ...
    ASSERT_GE(get_perf_context()->perf_context_by_level[0].bloom_filter_useful, maxKey*0.98);
    get_perf_context()->Reset();
    get_perf_context()->DisablePerLevelPerfContext();
  }
}
...
TEST_F(DBBloomFilterTest, OptimizeFiltersForHits) {
  ...
  ASSERT_GT(get_perf_context()->perf_context_by_level[0].bloom_filter_useful, 65000*2);
  ASSERT_LT(get_perf_context()->perf_context_by_level[0].bloom_filter_useful, 120000*2);
