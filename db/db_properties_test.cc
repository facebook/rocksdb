...
TEST_F(DBPropertiesTest, NumImmutableMemTable) {
  do {
    ...
    Get(1, "k1");
    ASSERT_EQ(1, static_cast<int>(GetPerfContext()->get_from_memtable_count));
    ...
    GetPerfContext()->Reset();
    Get(1, "k1");
    ASSERT_EQ(2, static_cast<int>(GetPerfContext()->get_from_memtable_count));
    GetPerfContext()->Reset();
    Get(1, "k2");
    ASSERT_EQ(1, static_cast<int>(GetPerfContext()->get_from_memtable_count));
    ...
    GetPerfContext()->Reset();
    Get(1, "k1");
    ASSERT_EQ(3, static_cast<int>(GetPerfContext()->get_from_memtable_count));
...
TEST_F(DBPropertiesTest, TablePropertiesNeedCompactTest) {
...

  {
    ...
    ASSERT_EQ(c, 0);
    ASSERT_LT(GetPerfContext()->internal_delete_skipped_count, 30u);
    ASSERT_LT(GetPerfContext()->internal_key_skipped_count, 30u);
    SetPerfLevel(kDisable);
  }
}
...
TEST_F(DBPropertiesTest, NeedCompactHintPersistentTest) {
  ...
  ASSERT_EQ(NumTableFilesAtLevel(0), 0);
  {
    ...
    ASSERT_EQ(c, 2);
    ASSERT_EQ(GetPerfContext()->internal_delete_skipped_count, 0);
    // We iterate every key twice. Is it a bug?
    ASSERT_LE(GetPerfContext()->internal_key_skipped_count, 2);
    SetPerfLevel(kDisable);
  }
}
