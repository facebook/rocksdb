...
IOStatus Writer::EmitPhysicalRecord(const WriteOptions& write_options,
  ...
  if (s.ok() && track_and_verify_wals_) {
    std::cout << " " << std::endl;
    std::cout << "Emit" << std::endl;
    std::cout << "Type: " << std::to_string(static_cast<int>(t)) << std::endl;
    std::bitset<8> type_buffer(buf[6]);
    std::cout << "Type buffer: " << type_buffer << std::endl;
    std::cout << "Length: " << std::to_string(n) << std::endl;
    std::bitset<8> length_buffer_4(buf[4]);
    std::cout << "Length buffer 4: " << length_buffer_4 << std::endl;
    std::bitset<8> length_buffer_5(buf[5]);
