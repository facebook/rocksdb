...
struct FileMetaData {
  ...
  // When reading this file, ignore the sequence number of keys in file and use
  // `largest_seqno` as the sequence number of all keys for this file. This is
  // used for ingested files that were generated by some live DB (ingested with
  // IngestExternalFileOptions::from_live_db).
  bool ignore_seqno_in_file = false;
...
class VersionEdit {
 public:
  ...
  // @param ignore_seqno_in_file Whether to use largest seqno of the file as a
  //  global sequence number for reading keys in the file.
  void AddFile(int level, uint64_t file, uint32_t file_path_id,
    ...
    new_files_.emplace_back(
        ...
        FileMetaData(file, file_path_id, file_size, smallest, largest,
                     ...
                     user_defined_timestamps_persisted, ignore_seqno_in_file));
