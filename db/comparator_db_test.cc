...
static const Comparator* test_comparator;
...
void DoRandomIteraratorTest(DB* db, std::vector<std::string> source_strings,
  ...
  stl_wrappers::KVMap map((stl_wrappers::LessOfComparator(test_comparator)));
...
class ComparatorDBTest
 ...
 public:
  ComparatorDBTest() : env_(Env::Default()), db_(nullptr) {
    test_comparator = BytewiseComparator();
...
TEST_P(ComparatorDBTest, SimpleSuffixReverseComparator) {
  ...
  for (int rnd_seed = 301; rnd_seed < 316; rnd_seed++) {
    ...
    opt->comparator = test_comparator;
    DestroyAndReopen();
    Random rnd(rnd_seed);
