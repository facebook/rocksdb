...
TEST_F(DBSSTTest, StoppingManualCompactionsWorks) {
  std::shared_ptr<SstFileManager> sst_file_manager(NewSstFileManager(env_));
  ...
  dbfull()->CompactFiles(rocksdb::CompactionOptions(), l0_files_before_compact, 0);

  // Wait for manual compaction to get scheduled and finish
  dbfull()->TEST_WaitForCompact(true);

  auto l0_files_after_compact = collector->GetFlushedFiles();
  ASSERT_EQ(l0_files_before_compact, l0_files_after_compact);
