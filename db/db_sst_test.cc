...
TEST_F(DBSSTTest, CompactionConcurrencyControl) {
  const int kNumKeysPerFile = 10;
  const int kMaxBackgroundThreads = 64;

  struct DeviceIOConcurrency {
    std::string device_name;
    int compaction_limit;
  };

  std::vector<DeviceIOConcurrency> device_settings;
  device_settings.push_back({"DEVICE1", 1});
  device_settings.push_back({"DEVICE2", 2});
  device_settings.push_back({"DEVICE3", 3});

  const char* cf_names[] = {"default", "1", "2", "3", "4", "5", "6", "7", "8", "9" };
  const int cf_count = sizeof cf_names / sizeof cf_names[0];

  std::shared_ptr<SstFileManager> sfm(NewSstFileManager(env_));

  std::unordered_map<std::string, int> device_compact_limit;  
  std::unordered_map<std::string, int> device_compact_tasks;
  std::unordered_map<std::string, int> device_compact_max;


  for (const auto& ds : device_settings) {
    sfm->SetMaxOutstandingCompaction(ds.device_name, ds.compaction_limit);
    device_compact_limit[ds.device_name] = ds.compaction_limit;
    device_compact_tasks[ds.device_name] = 0;
    device_compact_max[ds.device_name] = 0;
  }
  ...
  // Map from CF name to device name
  std::unordered_map<std::string, std::string> cf_device_map;

  option_vector.reserve(cf_count);
  // Default CF is using undefined device (no concurrency control)
  option_vector.emplace_back(options);
  for (int cf = 1; cf < cf_count; cf++) {
    ColumnFamilyOptions cf_opt(options);
    // Assign device by mod
    cf_opt.device_name = device_settings[cf % 3].device_name;
    cf_device_map[cf_names[cf]] = cf_opt.device_name;
    option_vector.emplace_back(DBOptions(options), cf_opt);
    CreateColumnFamilies({cf_names[cf]}, option_vector.back());
  }

  ReopenWithColumnFamilies(std::vector<std::string>(cf_names, cf_names + cf_count), 
                           option_vector);
  ...
  rocksdb::SyncPoint::GetInstance()->SetCallBack(
    "DBImpl::BackgroundCompaction:BeforeCompaction", [&](void* arg) { 
      ...
      auto iter = cf_device_map.find(cf_name);
      if (iter != cf_device_map.end()) {
        MutexLock l(&mutex);      
        ASSERT_GE(device_compact_limit[iter->second], ++device_compact_tasks[iter->second]);
        device_compact_max[iter->second] = std::max(device_compact_max[iter->second], 
          device_compact_tasks[iter->second]);
      }
    });
  ...
  rocksdb::SyncPoint::GetInstance()->SetCallBack(
    "DBImpl::BackgroundCompaction:AfterCompaction", [&](void* arg) { 
      ...
      if (iter != cf_device_map.end()) {
        MutexLock l(&mutex);
        ASSERT_GE(--device_compact_tasks[iter->second], 0);
      }
    });
  ...
  for (int cf = 0; cf < cf_count; cf++) {
    ASSERT_EQ(NumTableFilesAtLevel(0, cf), options.level0_file_num_compaction_trigger);
  }
  ...
  // Start background writes to queue more tasks during compaction
  rocksdb::port::Thread background_write(
    [&]() {
  ...
  for (const auto& compact_max : device_compact_max) {
    ASSERT_EQ(compact_max.second, device_compact_limit[compact_max.first]);
  }
}
