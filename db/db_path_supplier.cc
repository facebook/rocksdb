...
size_t get_cf_path_size(const ImmutableCFOptions* ioptions) {
  size_t paths_size = ioptions->cf_paths.size();

  if (paths_size == 0) {
    paths_size = ioptions->db_paths.size();
  }
...
uint32_t RandomDbPathSupplier::GetPathId(int level) const {
  ...
  size_t paths_size = get_cf_path_size(ioptions_);
...
uint32_t LeveledTargetSizeDbPathSupplier::GetPathId(int level) const {
  ...
  const MutableCFOptions& mutable_cf_options = *(cfd_->GetCurrentMutableCFOptions());
