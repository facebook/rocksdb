...
Status ImportColumnFamilyJob::GetIngestedFileInfo(
  ...
  if (file_meta.smallest.empty()) {
    ...
    // the block cache, if we later change the global seqno of this file, we
    // will have block in cache that will include keys with wrong seqno. We need
    // to disable fill_cache so that we read from the file without updating the
    // block cache.
    ro.fill_cache = false;
