...
std::optional<SequenceNumber> MemTable::MaxCoveringTombstoneSeqnum(const ReadOptions& read_options, const LookupKey& key) {
  if (read_options.ignore_range_deletions ||
      is_range_del_table_empty_.load(std::memory_order_relaxed)) {
    return std::nullopt;
  }
  ...
  for (;iter->Valid(); iter->Next()) {
    ...
    if (user_comparator->Compare(tombstone_end_key, key.user_key()) <= 0) {
      continue;
    }
    if (tombstone_seq <= read_seq) {
      max_seq = max_seq ? std::max(*max_seq, tombstone_seq) : tombstone_seq;
    }
  }
  delete iter;
  return max_seq;
...
Status MemTable::Add(SequenceNumber s, ValueType type,
  ...
  char* buf = nullptr;
  uint32_t encoded_len;
  KeyHandle handle = FormatEntry(s, type, key, value, table, &buf, &encoded_len, std::nullopt);
  ...
  if (!allow_concurrent) {
    ...
    num_entries_.store(num_entries_.load(std::memory_order_relaxed) + inserted_entries,
                       std::memory_order_relaxed);
    ...
    bool res = (hint == nullptr)
                   ? InsertKey(table, handle, s, type, key, value,
                               encoded_len, &inserted_entries, &inserted_len,
