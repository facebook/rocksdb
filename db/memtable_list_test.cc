...
TEST_F(MemTableListTest, GetTest) {
  ...
  bool found = list.current()->Get(
      lkey, &value, /*timestamp*/ /*timestamp*/ nullptr, &s, &merge_context,
  ...
  found = mem->Get(LookupKey("key1", seq), &value,
                   /*timestamp*/ /*timestamp*/ nullptr, &s, &merge_context,
                   &max_covering_tombstone_seq, ReadOptions());
  ...
  found = list.current()->Get(
      LookupKey("key1", seq), &value, /*timestamp*/ /*timestamp*/ nullptr, &s,
      &merge_context, &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(found && s.IsNotFound());

  merge_context.Clear();
  found = list.current()->Get(
      LookupKey("key1", saved_seq), &value, /*timestamp=*//*timestamp*/ nullptr,
  ...
  merge_context.Clear();
  found = list.current()->Get(
      LookupKey("key2", seq), &value, /*timestamp=*//*timestamp*/ nullptr, &s,
      &merge_context, &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(s.ok() && found);
  ...
  merge_context.Clear();
  found = list.current()->Get(
      LookupKey("key2", 1), &value, /*timestamp=*//*timestamp*/ nullptr, &s,
      &merge_context, &max_covering_tombstone_seq, ReadOptions());
  ASSERT_FALSE(found);
