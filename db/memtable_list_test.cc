...
TEST_F(MemTableListTest, GetTest) {
  ...
  bool found = list.current()->Get(lkey, &value, nullptr, &s, &merge_context,
  ...
  merge_context.Clear();
  found = mem->Get(LookupKey("key1", seq), &value, nullptr, &s, &merge_context,
                   &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(s.ok() && found);
  ASSERT_EQ(value, "value1");

  merge_context.Clear();
  found = mem->Get(LookupKey("key1", 2), &value, nullptr, &s, &merge_context,
                   &max_covering_tombstone_seq, ReadOptions());
  // MemTable found out that this key is *not* found (at this sequence#)
  ASSERT_TRUE(found && s.IsNotFound());

  merge_context.Clear();
  found = mem->Get(LookupKey("key2", seq), &value, nullptr, &s, &merge_context,
                   &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(s.ok() && found);
  ...
  merge_context.Clear();
  found = list.current()->Get(LookupKey("key1", seq), &value, nullptr, &s,
                              &merge_context, &max_covering_tombstone_seq,
                              ReadOptions());
  ASSERT_TRUE(found && s.IsNotFound());

  merge_context.Clear();
  found = list.current()->Get(LookupKey("key1", saved_seq), &value, nullptr, &s,
                              &merge_context, &max_covering_tombstone_seq,
                              ReadOptions());
  ASSERT_TRUE(s.ok() && found);
  ...
  merge_context.Clear();
  found = list.current()->Get(LookupKey("key2", seq), &value, nullptr, &s,
                              &merge_context, &max_covering_tombstone_seq,
                              ReadOptions());
  ASSERT_TRUE(s.ok() && found);
  ...
  merge_context.Clear();
  found = list.current()->Get(LookupKey("key2", 1), &value, nullptr, &s,
                              &merge_context, &max_covering_tombstone_seq,
                              ReadOptions());
  ASSERT_FALSE(found);
...
TEST_F(MemTableListTest, GetFromHistoryTest) {
  ...
  found = list.current()->GetFromHistory(
      LookupKey("key1", seq), &value, nullptr, &s, &merge_context,
      &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(found && s.IsNotFound());

  merge_context.Clear();
  found = list.current()->GetFromHistory(
      LookupKey("key2", seq), &value, nullptr, &s, &merge_context,
      &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(found);
  ASSERT_EQ("value2.2", value);
  ...
  merge_context.Clear();
  found = list.current()->Get(LookupKey("key3", seq), &value, nullptr, &s,
                              &merge_context, &max_covering_tombstone_seq,
                              ReadOptions());
  ASSERT_FALSE(found);
  ...
  merge_context.Clear();
  found = list.current()->GetFromHistory(
      LookupKey("key3", seq), &value, nullptr, &s, &merge_context,
      &max_covering_tombstone_seq, ReadOptions());
  ASSERT_TRUE(found);
