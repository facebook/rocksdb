...
void CompactionIterator::NextFromInput() {
  ...
  while (!valid_ && input_->Valid() && !IsShuttingDown()) {
    ...
    if (need_skip) {
      ...
      // In case snapshot_checker presents, we can probably merge further beyond
      // prev_snapshot, since there could be more keys with sequence smaller
      // than prev_snapshot, but reported by snapshot_checker as not visible by
      // prev_snapshot. But it will make the logic more complicated.
