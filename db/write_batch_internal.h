...
class WriteBatchInternal {
 public:
  ...
  // // Return the timestamp for the batch.
  // static uint64_t Timestamp(const WriteBatch* batch);
  //
  // // Store the specified number as the timestamp for the entire batch.
  // static void SetTimestamp(WriteBatch* batch, uint64_t ts);
  ...
  static Status InsertInto(WriteThread::WriteGroup& write_group,
                           ...
                           bool batch_per_txn = true, uint64_t timestamp = 0);
