...
TEST_P(DBWriteBufferManagerTest, FlushLargestAcrossDBsSkipsWriteStoppedDB) {
  ...
  env_->SleepForMicroseconds(21 * 1000);
  ASSERT_OK(Put(Key(2), DummyString(1), WriteOptions()));
...
// A queued LOW-pool handoff periodically permits local progress while it waits.
TEST_P(DBWriteBufferManagerTest,
  ...
  // The first write queues a handoff to other_db. Once the coalescing interval
  // expires, the next write must seal db_ without queuing another handoff.
  ASSERT_OK(Put(Key(2), DummyString(1), WriteOptions()));
