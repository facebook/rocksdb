...
Status ExternalSstFileIngestionJob::ValidateDeclaredLevelSequenceOrder(
  ...
  // Pair selection is O(n^2). Disjoint sequence-number ranges avoid table
  // scans, and each TableReader is opened at most once. If large edits become
  // common, use a key-range sweep followed by a tagged multiway scan.
  for (size_t i = 0; i < num_files; ++i) {
    for (size_t j = i + 1; j < num_files; ++j) {
      IngestedFileInfo* first = &files_to_ingest_[i];
      IngestedFileInfo* second = &files_to_ingest_[j];
