...
Status ExternalSstFileIngestionJob::Prepare(
  ...
  for (const IngestedFileInfo& f : files_to_ingest_) {
    if (!ingestion_options_.ingest_live_db_files && f.version == 0) {
      return Status::InvalidArgument("External file don't have version table "
          "property");
    }
    if (ingestion_options_.check_cf_id_property_before_ingest && f.cf_id !=
...
Status ExternalSstFileIngestionJob::GetIngestedFileInfo(
  ...
  if (version_iter == uprops.end()) {
    // Version 0 meaning ingest SST files which generated by live DB
  ...
  if (iter->Valid()) {
    ...
    if (file_to_ingest->version != 0 && key.sequence != 0) {
...
Status ExternalSstFileIngestionJob::AssignGlobalSeqnoForIngestedFile(
  ...
  if (ingestion_options_.write_global_seqno) {
    if (file_to_ingest->version == 0) {
      return Status::InvalidArgument("Trying to write global seqno to SST files"
          " generated by live DB");
    }
