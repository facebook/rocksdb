  ...
  if (is_manual) {
    ...
    if (!mutable_cf_options.disable_auto_compactions && !cfd->IsDropped()) {
      ...
      c.reset(cfd->PickCompaction(
          ...
          mutable_cf_options.universal_pick_compaction_by_thread_pri
              ? (thread_pri == Env::Priority::LOW &&
                         !cfd->IsBottomPriCompactionIntentForwarded()
                     ? Env::Priority::TOTAL
                     : thread_pri)
  ...
  if (!c) {
    ...
    *made_progress = true;
    ...
    const bool need_repick =
        c->mutable_cf_options().universal_pick_compaction_by_thread_pri;
...
Compaction* DBImpl::CreateIntendedCompactionForwardedToBottomPriorityPool(
  ...
  auto* vstorage = c->input_version()->storage_info();
  const std::vector<CompactionInputFiles>* original_inputs = c->inputs();
  ...
  for (const auto& original_input_files : *original_inputs) {
    if (!original_input_files.files.empty() &&
        original_input_files.level >= max_intput_level) {
      max_intput_level_files = &(original_input_files.files);
      max_intput_level = original_input_files.level;
    }
  }
