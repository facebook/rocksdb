...
void DBImpl::RefreshMutableMemAccounting() {
  mutex_.AssertHeld();
  versions_->GetColumnFamilySet()->RefreshMutableMemAccounting();
}

bool DBImpl::TryRefreshMutableMemAccounting() {
  if (!mutex_.TryLock()) {
    return false;
  }
...
bool DBImpl::ScheduleWriteBufferManagerFlush() {
  std::unique_ptr<FlushThreadArg> fta;
  {
    ...
    // makes the coordinator publish a local fallback instead of queuing more
    // work behind a busy LOW pool.
...
void DBImpl::BGWorkWBMFlush(void* arg) {
  ...
  TEST_SYNC_POINT_CALLBACK("DBImpl::BGWorkWBMFlush:done", fta->db_);
}
