...
Status DBImpl::ReFitLevel(ColumnFamilyData* cfd, int level, int target_level) {
  ...
  if (to_level != level) {
    ...
    uint64_t l0_epoch_num = kUnknownL0EpochNumber;
    if (to_level == 0) {
      versions_->FetchAddL0EpochNumber(vstorage->LevelFiles(level).size());
      l0_epoch_num = versions_->current_next_l0_epoch_number() - 1;
    }
    for (const auto& f : vstorage->LevelFiles(level)) {
      ...
      edit.AddFile(
          ...
          to_level == 0 ? l0_epoch_num-- : f->l0_epoch_number, f->file_checksum,
