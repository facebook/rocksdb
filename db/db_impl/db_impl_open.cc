...
Status DBImpl::ValidateOptions(const DBOptions& db_options) {
  ...
  if (db_options.dynamic_offpeak_window_percent > 50) {
    return Status::InvalidArgument(
        "dynamic_offpeak_window_percent must be 0 (disabled) or in the range "
        "[1, 50]");
  }
  if (db_options.dynamic_offpeak_window_percent > 0) {
    if (db_options.statistics == nullptr) {
      return Status::InvalidArgument(
          "dynamic_offpeak_window_percent requires statistics when enabled");
    }
    if (db_options.stats_persist_period_sec == 0 ||
        db_options.stats_persist_period_sec >
            DynamicOffpeakModel::kBucketSeconds) {
      return Status::InvalidArgument(
          "dynamic_offpeak_window_percent requires stats_persist_period_sec in "
