...
// TEST_F(DBBlockCacheTest, WarmCacheWithL0FilterBlock) {
//   Options options = CurrentOptions();
//   options.create_if_missing = true;
//   options.statistics = rocksdb::CreateDBStatistics();
//   BlockBasedTableOptions table_options;
//   table_options.cache_index_and_filter_blocks = false;
//   table_options.prepopulate_data_blocks = true;
//   table_options.filter_policy.reset(NewBloomFilterPolicy(20));
//   options.table_factory.reset(new BlockBasedTableFactory(table_options));
...
//
//   // // filter & data blocks added to block cache during L0 table creation.
...
//   // ASSERT_EQ(0, TestGetTickerCount(options, BLOCK_CACHE_INDEX_MISS));
//   // ASSERT_EQ(0, TestGetTickerCount(options, BLOCK_CACHE_FILTER_MISS));
//   // ASSERT_EQ(2, TestGetTickerCount(options, BLOCK_CACHE_DATA_MISS));
...
TEST_F(DBBlockCacheTest, WarmCacheWithL0DataBlock) {
  ...
  // ASSERT_EQ(1, TestGetTickerCount(options, BLOCK_CACHE_ADD));
  //
  // fprintf(stderr, "getting a key\n");
  // ASSERT_EQ("value", Get("foo"));
  //
  ...
  for (int i = 1; i < 101; i++) {
    ...
    ASSERT_EQ(i, TestGetTickerCount(options, BLOCK_CACHE_ADD));
    ASSERT_EQ(i, TestGetTickerCount(options, BLOCK_CACHE_DATA_ADD));
    ASSERT_EQ("value", Get("key_" + ToString(i)));
    ASSERT_EQ(0, TestGetTickerCount(options, BLOCK_CACHE_DATA_MISS));
    ASSERT_EQ(i, TestGetTickerCount(options, BLOCK_CACHE_DATA_HIT));
  }
}
