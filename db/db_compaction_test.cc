...
TEST_F(DBCompactionTest, CompactRangeOverlappedWithOngoingFileIngestion) {
  Options options = CurrentOptions();
  options.create_if_missing = true;
  options.disable_auto_compactions = true;
  DestroyAndReopen(options);
  ...
  // (2) CompactRange() on an overlapping range with s2 then proceeds with the
  // lock. It checks whether it can refits s1 from L6 to L3 and finds out it
  ...
  port::Thread t2([&] {
    ...
    EXPECT_TRUE(s.ToString().find("have ouput of ongoing compaction") !=
                std::string::npos);
  });
...
TEST_F(DBCompactionTest, IngestFileOfRangeOverlappedWithOngoingCompactRange) {
  Options options = CurrentOptions();
  options.create_if_missing = true;
  options.disable_auto_compactions = true;
  DestroyAndReopen(options);
  ...
  // Coerce following sequence of events:
  // (1) File ingestion of s2 with an overlapping range with s1 enters its
  // preparation phase and releases the lock. By this point, it hasn't assign
  // the output level. (2) CompactRange() then proceeds with the lock, refits s1
  // from L6 to L3, pauses on
  // TEST_SYNC_POINT("VersionSet::LogAndApply:WriteManifest") and releases lock.
