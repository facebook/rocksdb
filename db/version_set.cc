...
void Version::GetColumnFamilyMetaData(
  ...
  const bool has_timestamps = ts_sz > 0;

  int first_level = (options.level >= 0) ? options.level : 0;
  int last_level =
      (options.level >= 0) ? options.level + 1 : cfd_->NumberLevels();

  bool has_key_range = !options.start_key.empty() || !options.end_key.empty();
  ...
  for (int l = first_level; l < last_level; ++l) {
    ...
    for (const auto& file : vstorage->LevelFiles(l)) {
      ...
      if (has_key_range) {
        if (has_timestamps) {
          overlaps =
              ucmp->CompareWithoutTimestamp(file->smallest.user_key(), true,
                                            options.end_key, false) <= 0 &&
              ucmp->CompareWithoutTimestamp(file->largest.user_key(), true,
                                            options.start_key, false) >= 0;
        } else {
          overlaps =
              ucmp->Compare(file->smallest.user_key(), options.end_key) <= 0 &&
              ucmp->Compare(file->largest.user_key(), options.start_key) >= 0;
        }
      }
