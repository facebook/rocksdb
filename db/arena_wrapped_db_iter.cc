...
void ArenaWrappedDBIter::MaybeAutoRefresh() {
  if (cfh_ != nullptr && allow_refresh_ &&
    ...
    uint64_t step_threshold =
        read_options_.auto_refresh_iterator_step_threshold;
    if (step_threshold == 0) {
      step_threshold = 1;
    }

    if (++iter_ops_since_last_auto_refresh_check == step_threshold) {
      uint64_t cur_sv_number = cfh_->cfd()->GetSuperVersionNumberRelaxed();
      if (sv_number_ != cur_sv_number) {
        DoRefresh(read_options_.snapshot, cur_sv_number);
      }
      iter_ops_since_last_auto_refresh_check = 0;
    }
  }
}
