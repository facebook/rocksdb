...
class MergeHelper {
 public:
  ...
  // Legacy helper for methods not passing a ValueSink. TODO (AP) - can all such
  // calls be rewritten ?
  inline static Status TimedFullMerge(const MergeOperator* merge_operator,
    ...
    return TimedFullMerge(merge_operator, key, value, operands, result_sink,
                          logger, statistics, clock, result_operand,
                          update_num_ops_stats);
  };
