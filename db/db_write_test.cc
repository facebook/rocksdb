...
TEST_P(DBWriteTest, MultiThreadWrite) {
        ...
        [&](int index) {
          ...
          for (int j = 0; j < 64; j ++) {
            ...
            for (int i = 0; i < 4; i ++) {
              ...
              for (int k = 0; k < 64; k ++) {
                batch->Put("key_"+ToString(index) + "_" + ToString(j) + "_"+ ToString(i) + "_" + ToString(k), "value" + ToString(k));
              }
  ...
  for (int t = 0; t < kNumThreads; t ++) {
    ...
    for (int i = 0; i < 64; i ++) {
      for (int j = 0; j < 4; j ++) {
        for (int k = 0; k < 64;  k ++) {
          ASSERT_OK(dbfull()->Get(opt, "key_" + ToString(t) + "_" + ToString(i) + "_" + ToString(j) + "_" + ToString(k), &value));
