...
void CompactionJob::AggregateStatistics() {
  assert(compaction_state_);

  for (SubcompactionState& sc : compaction_state_->sub_compact_states) {
    ...
    compaction_state_->num_output_files += outputs.size();
    compaction_state_->total_bytes += sc.total_bytes;

    const auto& blobs = sc.blob_file_additions;

    compaction_state_->num_blob_output_files += blobs.size();

    for (const auto& blob : blobs) {
      compaction_state_->total_blob_bytes += blob.GetTotalBlobBytes();
    }

    compaction_state_->num_output_records += sc.num_output_records;
...
CompactionJob::CompactionJob(
    ...
    : job_id_(job_id),
      compaction_state_(new CompactionState(compaction)),
      compaction_job_stats_(compaction_job_stats),
      compaction_stats_(compaction->compaction_reason(), 1),
      service_(service),
      file_options_(*(service_->file_options_)),
      file_options_for_read_(service_->fs_->OptimizeForCompactionTableRead(
          file_options_, service_->db_options_)),
      manual_compaction_paused_(manual_compaction_paused),
  ...
  ThreadStatusUtil::SetColumnFamily(
      cfd, cfd->ioptions()->env, service_->db_options_.enable_thread_tracking);
...
void CompactionJob::ReportStartedCompaction(Compaction* compaction) {
  ...
  ThreadStatusUtil::SetThreadOperationProperty(
      ...
      (static_cast<uint64_t>(compaction_state_->compaction->start_level())
       << 32) +
...
void CompactionJob::Prepare() {
  ...
  auto* c = compaction_state_->compaction;
  assert(c->column_family_data() != nullptr);
  assert(c->column_family_data()->current()->storage_info()->NumLevelFiles(
             compaction_state_->compaction->level()) > 0);
  ...
  if (c->ShouldFormSubcompactions()) {
    {
      StopWatch sw(service_->clock_, service_->stats_,
    ...
    for (size_t i = 0; i <= boundaries_.size(); i++) {
      ...
      compaction_state_->sub_compact_states.emplace_back(c, start, end,
                                                         sizes_[i]);
    }
    RecordInHistogram(service_->stats_, NUM_SUBCOMPACTIONS_SCHEDULED,
                      compaction_state_->sub_compact_states.size());
  } else {
    constexpr Slice* start = nullptr;
    constexpr Slice* end = nullptr;
    constexpr uint64_t size = 0;

    compaction_state_->sub_compact_states.emplace_back(c, start, end, size);
  }
}
...
void CompactionJob::GenSubcompactionBoundaries() {
  ...
  auto* v = compaction_state_->compaction->input_version();
...
Status CompactionJob::Run() {
  ...
  const size_t num_threads = compaction_state_->sub_compact_states.size();
  assert(num_threads > 0);
  const uint64_t start_micros = service_->clock_->NowMicros();

  // Launch a thread for each of subcompactions 1...num_threads-1
  std::vector<port::Thread> thread_pool;
  thread_pool.reserve(num_threads - 1);
  for (size_t i = 1; i < compaction_state_->sub_compact_states.size(); i++) {
    thread_pool.emplace_back(&CompactionJob::ProcessKeyValueCompaction, this,
                             &compaction_state_->sub_compact_states[i]);
  }

  // Always schedule the first subcompaction (whether or not there are also
  // others) in the current thread to be efficient with resources
  ProcessKeyValueCompaction(&compaction_state_->sub_compact_states[0]);
  ...
  compaction_stats_.micros = service_->clock_->NowMicros() - start_micros;
  compaction_stats_.cpu_micros = 0;
  for (size_t i = 0; i < compaction_state_->sub_compact_states.size(); i++) {
    compaction_stats_.cpu_micros += compaction_state_->sub_compact_states[i]
                                        .compaction_job_stats.cpu_micros;
  }

  RecordTimeToHistogram(service_->stats_, COMPACTION_TIME,
                        compaction_stats_.micros);
  RecordTimeToHistogram(service_->stats_, COMPACTION_CPU_TIME,
  ...
  for (const auto& state : compaction_state_->sub_compact_states) {
    if (!state.status.ok()) {
      status = state.status;
  ...
  if (status.ok()) {
    ...
    ColumnFamilyData* cfd = compaction_state_->compaction->column_family_data();
    auto prefix_extractor = compaction_state_->compaction->mutable_cf_options()
      ...
      while (true) {
        ...
        InternalIterator* iter = cfd->table_cache()->NewIterator(
            ...
            MaxFileSizeForL0MetaPin(
                *compaction_state_->compaction->mutable_cf_options()),
    ...
    for (size_t i = 1; i < compaction_state_->sub_compact_states.size(); i++) {
      thread_pool.emplace_back(
          verify_table,
          std::ref(compaction_state_->sub_compact_states[i].status));
    }
    verify_table(compaction_state_->sub_compact_states[0].status);
    for (auto& thread : thread_pool) {
      thread.join();
    }
    for (const auto& state : compaction_state_->sub_compact_states) {
  ...
  compaction_state_->compaction->SetOutputTableProperties(std::move(tp));
  ...
  LogFlush(service_->db_options_.info_log);
  TEST_SYNC_POINT("CompactionJob::Run():End");

  compaction_state_->status = status;
  return status;
}
...
Status CompactionJob::Install(const MutableCFOptions& mutable_cf_options) {
  ...
  Status status = compaction_state_->status;

  ColumnFamilyData* cfd = compaction_state_->compaction->column_family_data();
  assert(cfd);

  cfd->internal_stats()->AddCompactionStats(
      compaction_state_->compaction->output_level(), thread_pri_,
      compaction_stats_);
  ...
  stream << "job" << job_id_ << "event"
         ...
         << compaction_state_->compaction->output_level() << "num_output_files"
         << compaction_state_->num_output_files << "total_output_size"
         << compaction_state_->total_bytes;

  if (compaction_state_->num_blob_output_files > 0) {
    stream << "num_blob_output_files"
           << compaction_state_->num_blob_output_files
           << "total_blob_output_size" << compaction_state_->total_blob_bytes;
  }

  stream << "num_input_records" << stats.num_input_records
         << "num_output_records" << compaction_state_->num_output_records
         << "num_subcompactions" << compaction_state_->sub_compact_states.size()
         << "output_compression"
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  uint64_t prev_cpu_micros = service_->clock_->CPUNanos() / 1000;
  ...
  MergeHelper merge(
      service_->db_options_.env, cfd->user_comparator(),
      cfd->ioptions()->merge_operator, compaction_filter,
      service_->db_options_.info_log.get(),
      false /* internal key corruption is expected */,
      existing_snapshots_.empty() ? 0 : existing_snapshots_.back(),
      snapshot_checker_, compaction_state_->compaction->level(),
      service_->stats_);
  ...
  sub_compact->c_iter.reset(new CompactionIterator(
      ...
      service_->db_options_.env,
      ShouldReportDetailedTime(service_->db_options_.env, service_->stats_),
      /*expect_valid_internal_key=*/true, &range_del_agg,
      blob_file_builder.get(), service_->db_options_.allow_data_in_errors,
      sub_compact->compaction, compaction_filter, service_->shutting_down_,
      preserve_deletes_seqnum_, manual_compaction_paused_,
      service_->db_options_.info_log, full_history_ts_low));
  ...
  RecordTick(service_->stats_, FILTER_OPERATION_TOTAL_TIME,
  ...
  sub_compact->compaction_job_stats.cpu_micros =
      service_->clock_->CPUNanos() / 1000 - prev_cpu_micros;
...
void CompactionJob::RecordDroppedKeys(
  ...
  if (c_iter_stats.num_record_drop_user > 0) {
    RecordTick(service_->stats_, COMPACTION_KEY_DROP_USER,
               c_iter_stats.num_record_drop_user);
  }
  if (c_iter_stats.num_record_drop_hidden > 0) {
    RecordTick(service_->stats_, COMPACTION_KEY_DROP_NEWER_ENTRY,
  ...
  if (c_iter_stats.num_record_drop_obsolete > 0) {
    RecordTick(service_->stats_, COMPACTION_KEY_DROP_OBSOLETE,
  ...
  if (c_iter_stats.num_record_drop_range_del > 0) {
    RecordTick(service_->stats_, COMPACTION_KEY_DROP_RANGE_DEL,
               c_iter_stats.num_record_drop_range_del);
  }
  if (c_iter_stats.num_range_del_drop_obsolete > 0) {
    RecordTick(service_->stats_, COMPACTION_RANGE_DEL_DROP_OBSOLETE,
               c_iter_stats.num_range_del_drop_obsolete);
  }
  if (c_iter_stats.num_optimized_del_drop_obsolete > 0) {
    RecordTick(service_->stats_, COMPACTION_OPTIMIZED_DEL_DROP_OBSOLETE,
...
Status CompactionJob::FinishCompactionOutputFile(
  ...
  if (s.ok()) {
    ...
    io_s = sub_compact->outfile->Sync(service_->db_options_.use_fsync);
  }
  if (s.ok() && io_s.ok()) {
  ...
  if (s.ok() && current_entries == 0 && tp.num_range_deletions == 0) {
    ...
    Status ds = service_->db_options_.env->DeleteFile(fname);
    if (!ds.ok()) {
      ROCKS_LOG_WARN(
          service_->db_options_.info_log,
  ...
  if (s.ok() && (current_entries > 0 || tp.num_range_deletions > 0)) {
    ...
    ROCKS_LOG_INFO(service_->db_options_.info_log,
  ...
  EventHelpers::LogAndNotifyTableFileCreationFinished(
      service_->event_logger_, service_->db_options_.listeners,
      service_->dbname_, cfd->GetName(), fname, job_id_, output_fd,
      oldest_blob_file_number, tp, TableFileCreationReason::kCompaction, s,
      file_checksum, file_checksum_func_name);

#ifndef ROCKSDB_LITE
  // Report new file to SstFileManagerImpl
  auto sfm = static_cast<SstFileManagerImpl*>(
      service_->db_options_.sst_file_manager.get());
...
Status CompactionJob::InstallCompactionResults(
  ...
  auto* compaction = compaction_state_->compaction;
  assert(compaction);

  // paranoia: verify that the files that we started with
  // still exist in the current version and in the same original level.
  // This ensures that a concurrent compaction did not erroneously
  // pick the same files to compaction_state_.
  if (!service_->versions_->VerifyCompactionFileConsistency(compaction)) {
    Compaction::InputLevelSummaryBuffer inputs_summary;

    ROCKS_LOG_ERROR(service_->db_options_.info_log,
  ...
  for (const auto& sub_compact : compaction_state_->sub_compact_states) {
...
void CompactionJob::RecordCompactionIOStats() {
  RecordTick(service_->stats_, COMPACT_READ_BYTES, IOSTATS(bytes_read));
  RecordTick(service_->stats_, COMPACT_WRITE_BYTES, IOSTATS(bytes_written));
  CompactionReason compaction_reason =
      compaction_state_->compaction->compaction_reason();
  if (compaction_reason == CompactionReason::kFilesMarkedForCompaction) {
    RecordTick(service_->stats_, COMPACT_READ_BYTES_MARKED,
               IOSTATS(bytes_read));
    RecordTick(service_->stats_, COMPACT_WRITE_BYTES_MARKED,
               IOSTATS(bytes_written));
  } else if (compaction_reason == CompactionReason::kPeriodicCompaction) {
    RecordTick(service_->stats_, COMPACT_READ_BYTES_PERIODIC,
               IOSTATS(bytes_read));
    RecordTick(service_->stats_, COMPACT_WRITE_BYTES_PERIODIC,
               IOSTATS(bytes_written));
  } else if (compaction_reason == CompactionReason::kTtl) {
    RecordTick(service_->stats_, COMPACT_READ_BYTES_TTL, IOSTATS(bytes_read));
    RecordTick(service_->stats_, COMPACT_WRITE_BYTES_TTL,
               IOSTATS(bytes_written));
  }
...
Status CompactionJob::OpenCompactionOutputFile(
...
#endif
  Status s;
  IOStatus io_s = NewWritableFile(service_->fs_.get(), fname, &writable_file,
                                  file_options_);
  s = io_s;
  if (sub_compact->io_status.ok()) {
    sub_compact->io_status = io_s;
  ...
  if (!s.ok()) {
    ...
    EventHelpers::LogAndNotifyTableFileCreationFinished(
        service_->event_logger_, service_->db_options_.listeners,
        service_->dbname_, cfd->GetName(), fname, job_id_, FileDescriptor(),
  ...
  FileTypeSet tmp_set = service_->db_options_.checksum_handoff_file_types;
  writable_file->SetPreallocationBlockSize(static_cast<size_t>(
      sub_compact->compaction->OutputFilePreallocationSize()));
  sub_compact->outfile.reset(new WritableFileWriter(
      std::move(writable_file), fname, file_options_, service_->clock_,
      service_->io_tracer_, service_->db_options_.statistics.get(),
      service_->db_options_.listeners,
      service_->db_options_.file_checksum_gen_factory.get(),
...
void CompactionJob::CleanupCompaction() {
  for (SubcompactionState& sub_compact :
       compaction_state_->sub_compact_states) {
  ...
  delete compaction_state_;
  compaction_state_ = nullptr;
}
...
void CompactionJob::UpdateCompactionStats() {
  ...
  Compaction* compaction = compaction_state_->compaction;
...
void CompactionJob::UpdateCompactionInputStatsHelper(int* num_files,
                                                     uint64_t* bytes_read,
                                                     int input_level) {
  const Compaction* compaction = compaction_state_->compaction;
...
void CompactionJob::UpdateCompactionJobStats(
  ...
  if (stats.num_output_files > 0) {
    ...
    CopyPrefix(compaction_state_->LargestUserKey(),
               CompactionJobStats::kMaxPrefixLength,
...
void CompactionJob::LogCompaction() {
  ...
  // we're not logging
  if (service_->db_options_.info_log_level <= InfoLogLevel::INFO_LEVEL) {
    Compaction::InputLevelSummaryBuffer inputs_summary;
    ROCKS_LOG_INFO(service_->db_options_.info_log,
