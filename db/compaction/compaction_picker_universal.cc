...
Compaction* UniversalCompactionBuilder::PickCompaction() {
  ...
  // The order in which we attempt to pick compactions depends on the thread
  // priority.
  // This is done to minimize fruitless picking attempts and optimize compaction
  // selection.
  if (thread_priority_for_picking_ == Env::Priority::TOTAL ||
      thread_priority_for_picking_ == Env::Priority::BOTTOM) {
    // For TOTAL or BOTTOM priority threads we prioritize compactions that are
    // more likely to output to the max output level. Periodic compaction is
    // given the highest priority as it is a hard requirement.
    ...
    // For LOW priority threads, we intentionally prioritize compactions that
    // are more likely to output to non-max output levels first.
    // Size amplification and periodic compaction often include
    // the last sorted run, making them less desirable for low-priority threads.
    // By deferring these compactions, we maximize the opportunity for other
    // compactions to be picked first.
