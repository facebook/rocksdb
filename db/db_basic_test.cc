...
TEST_F(DBMultiPrefixExistsTest, MultiColumnFamily) {
  Options options = CurrentOptions();
  options.prefix_extractor.reset(NewFixedPrefixTransform(3));
  options.memtable_prefix_bloom_size_ratio = 0.1;
  options.create_if_missing = true;
  ...
  // Write different prefixes to different CFs
  ASSERT_OK(Put(0, "foo_key1", "value1"));  // default CF
  ASSERT_OK(Put(1, "bar_key1", "value2"));  // cf1
  ASSERT_OK(Put(2, "baz_key1", "value3"));  // cf2

  // Test multi-CF API
  Slice prefixes[3] = {Slice("foo"), Slice("bar"), Slice("baz")};
  ColumnFamilyHandle* cfs[3] = {handles_[0], handles_[1], handles_[2]};
  bool results[3] = {false, false, false};

  db_->MultiPrefixExists(ReadOptions(), 3, cfs, prefixes, results);

  EXPECT_TRUE(results[0]);  // foo in default CF
  EXPECT_TRUE(results[1]);  // bar in cf1
  EXPECT_TRUE(results[2]);  // baz in cf2
}
...
TEST_F(DBMultiPrefixExistsTest, NullColumnFamily) {
  ...
  // Test multi-CF version with null in array
  ColumnFamilyHandle* cfs[3] = {db_->DefaultColumnFamily(), nullptr,
                                db_->DefaultColumnFamily()};
  Slice prefixes2[3] = {Slice("foo"), Slice("foo"), Slice("bar")};
  bool results2[3] = {false, true, false};  // Middle one true to verify

  db_->MultiPrefixExists(ReadOptions(), 3, cfs, prefixes2, results2);

  EXPECT_TRUE(results2[0]) << "foo in valid CF should exist";
  EXPECT_FALSE(results2[1]) << "foo with null CF should return false";
  EXPECT_FALSE(results2[2]) << "bar should not exist";
}
...
TEST_F(DBMultiPrefixExistsTest, MultiCFFastPath) {
  // Test that the multi-CF fast path is triggered when all prefixes
  // belong to the same column family.
  Options options = CurrentOptions();
  options.prefix_extractor.reset(NewFixedPrefixTransform(3));
  options.create_if_missing = true;
  ...
  // Write to cf1
  ASSERT_OK(Put(1, "foo_key1", "value1"));
  ASSERT_OK(Put(1, "bar_key1", "value2"));

  // All prefixes for the same CF - should use fast path
  Slice prefixes[4] = {Slice("foo"), Slice("bar"), Slice("baz"), Slice("foo")};
  ColumnFamilyHandle* cfs[4] = {handles_[1], handles_[1], handles_[1],
                                handles_[1]};
  bool results[4] = {false, false, false, false};

  db_->MultiPrefixExists(ReadOptions(), 4, cfs, prefixes, results);

  EXPECT_TRUE(results[0]) << "foo in cf1 should exist";
  EXPECT_TRUE(results[1]) << "bar in cf1 should exist";
  EXPECT_FALSE(results[2]) << "baz in cf1 should not exist";
  EXPECT_TRUE(results[3]) << "foo (duplicate) in cf1 should exist";
}
