...
TEST_F(DBIteratorTest, DBIteratorPrevNext) {
  ...
  MutableCFOptions mutable_cf_options = MutableCFOptions(options);
  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 10,
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 7,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
  ...
  // and less that the iterator's sequence id)
  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 4,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
...

  {
    ...
    ReadOptions ro;
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 2,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
    ASSERT_EQ(db_iter->key().ToString(), "b");
...
TEST_F(DBIteratorTest, DBIteratorEmpty) {
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 0,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(!db_iter->Valid());
  }
...
TEST_F(DBIteratorTest, DBIteratorUseSkipCountSkips) {
  ...
  std::unique_ptr<Iterator> db_iter(
      NewDBIterator(env_, ro, ImmutableCFOptions(options), MutableCFOptions(options), BytewiseComparator(),
...
TEST_F(DBIteratorTest, DBIteratorUseSkip) {
...

  {
    for (size_t i = 0; i < 200; ++i) {
      ...
      options.statistics = rocksdb::CreateDBStatistics();
      std::unique_ptr<Iterator> db_iter(NewDBIterator(
          env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, i + 2,
...

  {
...

    {
      ...
      std::unique_ptr<Iterator> db_iter(NewDBIterator(
          env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 202,
          options.max_sequential_skip_in_iterations,
          nullptr /*read_callback*/));
      db_iter->SeekToLast();
      ASSERT_TRUE(db_iter->Valid());
...

  {
    for (size_t i = 0; i < 200; ++i) {
      ...
      internal_iter->Finish();
      std::unique_ptr<Iterator> db_iter(NewDBIterator(
          env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, i,
          options.max_sequential_skip_in_iterations,
          nullptr /*read_callback*/));
      db_iter->SeekToLast();
      ASSERT_TRUE(!db_iter->Valid());
    ...
    internal_iter->Finish();
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 200,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
    ASSERT_EQ(db_iter->key().ToString(), "c");
...
TEST_F(DBIteratorTest, DBIteratorSkipInternalKeys) {
  ...
  // max_sequential_skip_in_iterations.
  {
    for (size_t i = 1; i <= 200; ++i) {
      ...
      std::unique_ptr<Iterator> db_iter(NewDBIterator(
          env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 2 * i + 1,
          options.max_sequential_skip_in_iterations,
          nullptr /*read_callback*/));
...
TEST_F(DBIteratorTest, DBIterator5) {
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 1,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
    ASSERT_EQ(db_iter->key().ToString(), "a");
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 3,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
    ASSERT_EQ(db_iter->key().ToString(), "a");
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 5,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
    ASSERT_EQ(db_iter->key().ToString(), "a");
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 6,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
    ASSERT_EQ(db_iter->key().ToString(), "a");
...
TEST_F(DBIteratorTest, DBIterator7) {
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 9,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 13,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
...

  {
    ...
    std::unique_ptr<Iterator> db_iter(NewDBIterator(
        env_, ro, cf_options, mutable_cf_options, BytewiseComparator(), internal_iter, 14,
        options.max_sequential_skip_in_iterations, nullptr /*read_callback*/));
    db_iter->SeekToLast();
    ASSERT_TRUE(db_iter->Valid());
...
TEST_F(DBIteratorTest, DBIterator8) {
  ...
  std::unique_ptr<Iterator> db_iter(NewDBIterator(
      env_, ro, ImmutableCFOptions(options), MutableCFOptions(options), BytewiseComparator(),
 ...
 public:
  DBIterWithMergeIterTest()
    ...
    db_iter_.reset(NewDBIterator(
        env_, ro_, ImmutableCFOptions(options_), MutableCFOptions(options_), BytewiseComparator(),
