...
class FlushJobTimestampTest
 ...
 protected:
  ...
  void CheckFileMetaData(ColumnFamilyData* cfd,
    ...
    // Check the installed version of the FileMetaData has the same file
    // boundaries. When UserDefinedTimestampTestMode::kStripUserDefinedTimestamp
    // test mode is enabled, these file boundaries' UDT are stripped before
    // persisted to Manifest, and padded when installed to Version. This is to
    // cover that path.
