...
TEST_F(DBOptionsTest, DynamicOffpeakOptions) {
  Options options;
  ASSERT_EQ(0u, options.dynamic_offpeak_window_percent);
  ...
  options.dynamic_offpeak_window_percent = 51;
  ...
  options.dynamic_offpeak_window_percent = 0;
  ASSERT_OK(DBImpl::TEST_ValidateOptions(options));
}
...
TEST_F(DBOptionsTest, DynamicOffpeakSetDBOptionsValidation) {
  ...
  ASSERT_TRUE(dbfull()
                  ->SetDBOptions({{"dynamic_offpeak_window_percent", "51"}})
                  .IsInvalidArgument());
  ASSERT_EQ(25u, dbfull()->GetDBOptions().dynamic_offpeak_window_percent);
  ...
  ASSERT_EQ(0u, dbfull()->GetDBOptions().dynamic_offpeak_window_percent);
  ASSERT_OK(dbfull()->SetDBOptions({{"stats_persist_period_sec", "0"}}));
  ASSERT_TRUE(dbfull()
                  ->SetDBOptions({{"dynamic_offpeak_window_percent", "25"}})
                  .IsInvalidArgument());
  ASSERT_EQ(0u, dbfull()->GetDBOptions().dynamic_offpeak_window_percent);
}
...
TEST_F(DBOptionsTest, DynamicOffpeakModel) {
  ...
  constexpr uint32_t kWindowPercent = 25;
  ...
  ASSERT_EQ("00:45-06:44", model.LearnedWindow());
  ...
  ASSERT_OK(restored.Decode(model.Encode(), kWindowPercent));
  ...
  ASSERT_TRUE(disabled_restored.LearnedWindow().empty());
  ASSERT_EQ(model.TrainedDays(), disabled_restored.TrainedDays());
}

TEST_F(DBOptionsTest, FlatDynamicOffpeakModelUsesConfiguredDuration) {
  struct TestCase {
  ...
  for (const auto& test_case : test_cases) {
    DynamicOffpeakModel model;
    ASSERT_FALSE(model.AddSample(0, DynamicOffpeakModel::kSecondsPerDay, 100,
                                 100, test_case.window_percent));
    ASSERT_TRUE(model.AddSample(DynamicOffpeakModel::kSecondsPerDay,
                                DynamicOffpeakModel::kSecondsPerDay + 1, 1, 1,
                                test_case.window_percent));
    ASSERT_EQ(test_case.expected_window, model.LearnedWindow());
  }
}
