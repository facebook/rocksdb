...
char* rocksdb_get(rocksdb_t* db, const rocksdb_readoptions_t* options,
  ...
  Status s = db->rep->Get(options->rep, Slice(key, keylen), value_sink);
  if (s.ok()) {
    *vallen = tmp.size();
    result = CopyString(tmp);
  } else {
...
char* rocksdb_transaction_get(rocksdb_transaction_t* txn,
  ...
  std::string tmp;
  ROCKSDB_NAMESPACE::StringValueSink value_sink(&tmp);
  Status s = txn->rep->Get(options->rep, Slice(key, klen), value_sink);
  if (s.ok()) {
    *vlen = tmp.size();
    result = CopyString(tmp);
  } else {
...
char* rocksdb_transaction_get_for_update_cf(
  ...
  Status s = txn->rep->GetForUpdate(options->rep, column_family->rep,
                                    Slice(key, klen), value_sink, exclusive);
  if (s.ok()) {
    *vlen = tmp.size();
    result = CopyString(tmp);
  } else {
...
char* rocksdb_transactiondb_get(rocksdb_transactiondb_t* txn_db,
  ...
  std::string tmp;
  ROCKSDB_NAMESPACE::StringValueSink value_sink(&tmp);
  Status s = txn_db->rep->Get(options->rep, Slice(key, klen), value_sink);
  if (s.ok()) {
    *vlen = tmp.size();
    result = CopyString(tmp);
  } else {
