...
//   value_sizes_bytes = byte size of VALUE SIZES section
...
class WideColumnSerialization {
 public:
  ...
  static Status ResolveEntityBlobColumns(
      ...
      bool* resolved, uint64_t* total_bytes_read,
  ...
  //   output: Receives the serialized entity in V1 format. Uses std::string
  //           (not Slice) because the resolved blob values need owned storage
  //           that outlives the serialization call.
  //
 ...
 private:
  ...
  // into columns. Used by both Deserialize and DeserializeColumns to avoid
  // code duplication.
  ...
  // column types. Used by both Deserialize and DeserializeColumns to avoid
  // code duplication.
  static Status DeserializeV2(Slice& input, uint32_t num_columns,
                              std::vector<WideColumn>* columns,
                              std::vector<uint8_t>* column_types);

  // Writes V2 sections 1-7 to output using pre-computed metadata.
  // Shared by both SerializeWithBlobIndices overloads.
  // get_name_data(i): returns const char* to column i's name bytes
  // get_value_data(i): returns const char* to column i's value bytes
  //                    (resolved to serialized blob index when applicable)
  template <typename GetNameData, typename GetValueData>
  static void WriteV2Sections(std::string* output, size_t num_columns,
                              const std::string& column_types,
                              const std::vector<uint32_t>& name_sizes,
                              const std::vector<uint32_t>& value_sizes,
                              uint32_t name_sizes_bytes, uint32_t names_bytes,
                              uint32_t total_value_sizes_bytes,
                              uint32_t total_values_bytes,
                              GetNameData get_name_data,
