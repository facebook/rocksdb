...
TEST_F(ConfigurableTest, ConfigureFromMapTest) {
  ...
  ASSERT_OK(configurable->ConfigureFromMap({}, options_));
...
TEST_F(ConfigurableTest, ConfigureFromStringTest) {
  std::unique_ptr<Configurable> configurable(SimpleConfigurable::Create());
  auto* opts = configurable->GetOptions<TestOptions>("simple");
  ASSERT_OK(configurable->ConfigureFromString("", options_));
  ASSERT_NE(opts, nullptr);
 ...
 public:
  ...
  Status PrepareOptions(const ConfigOptions& opts) override {
    if (++prepared <= 0) {
      ...
      return SimpleConfigurable::PrepareOptions(opts);
    }
  }
...
TEST_F(ConfigurableTest, ValidateOptionsTest) {
  ...
  DBOptions db_opts;
  ASSERT_OK(configurable->ConfigureOption("validated", "false", options_));
  ASSERT_NOK(configurable->ValidateOptions(db_opts, cf_opts));
  ASSERT_OK(configurable->ConfigureOption("validated", "true", options_));
...
TEST_F(ConfigurableTest, PrepareOptionsTest) {
  ...
  ASSERT_EQ(*cp, 0);
  ASSERT_EQ(*up, 0);
  ASSERT_OK(c->ConfigureFromMap({}, opts));
  ASSERT_EQ(*cp, 0);
  ASSERT_EQ(*up, 0);
  ...
  ASSERT_EQ(*cp, 1);
  ASSERT_EQ(*up, 1);
  ASSERT_OK(c->ConfigureFromString("prepared=0", opts));
  ASSERT_EQ(*up, 2);
  ASSERT_EQ(*cp, 1);

  ASSERT_NOK(c->ConfigureFromString("prepared=-2", opts));
...
static std::unordered_map<std::string, OptionTypeInfo> noserialize_option_info =
    {
        {"int",
         {offsetof(struct TestOptions, i), OptionType::kInt,
          OptionVerificationType::kNormal, OptionTypeFlags::kStringNone}},
};
...
TEST_F(ConfigurableTest, TestNoSerialize) {
  ...
  ASSERT_OK(base->ConfigureFromString("int=10", options_));
  ASSERT_OK(base->GetOptionString(options_, &opts_str));
  ASSERT_EQ(opts_str, "");
  ASSERT_NOK(base->GetOption("int", options_, &value));
}
...
TEST_F(ConfigurableTest, TestNoCompare) {
  ...
  copy.reset(SimpleConfigurable::Create("c", TestConfigMode::kDefaultMode,
                                        &normal_option_info));
  ASSERT_OK(base->ConfigureFromString("int=10", options_));
  ASSERT_OK(copy->ConfigureFromString("int=20", options_));
  std::string bvalue, cvalue;
  ASSERT_OK(base->GetOption("int", options_, &bvalue));
  ASSERT_OK(copy->GetOption("int", options_, &cvalue));
  ASSERT_EQ(bvalue, "10");
  ASSERT_EQ(cvalue, "20");
  ASSERT_TRUE(base->Matches(copy.get(), options_));
...
void ConfigurableParamTest::TestConfigureOptions(const ConfigOptions& opts) {
  ...
  for (auto name : names) {
    std::string value;
    Status s = base->GetOption(name, opts, &value);
    if (s.ok()) {
      s = copy->ConfigureOption(name, value, opts);
