...
TEST_F(ConfigurableTest, MatchesTest) {
  ...
  std::string mismatch;
  ASSERT_FALSE(base->Matches(config_options_, copy.get(), &mismatch));
...
TEST_F(ConfigurableTest, TestNoCompare) {
  ...
  ASSERT_EQ(cvalue, "20");
  ASSERT_TRUE(base->Matches(config_options_, copy.get()));
  ASSERT_FALSE(copy->Matches(config_options_, base.get()));
}
#endif
...
void ConfigurableParamTest::TestConfigureOptions(
  ...
  std::string mismatch;
  ASSERT_TRUE(base->Matches(config_options, copy.get(), &mismatch));
}
...
TEST_P(ConfigurableParamTest, ConfigureFromPropsTest) {
  ...
  ASSERT_TRUE(object_->Matches(config_options_, copy.get()));
}
...
static Configurable* DBOptionsFactory() {
  auto config = DBOptionsAsConfigurable(ConfigOptions(), DBOptions());
