...
// to allow it to be processed first (higher precedence).  This is necessary
// because the registry might set up factories and such that are required by
// the later options.
static std::unordered_map<std::string, OptionTypeInfo>
    object_registry_type_info = {
        {"object_registry",
...
class DBOptionsConfigurable : public MutableDBConfigurable {
 public:
  DBOptionsConfigurable(const ConfigOptions& config_options,
                        const DBOptions& db_options)
      : MutableDBConfigurable(MutableDBOptions(db_options)) {
    // The ImmutableDBOptions currently requires the env to be non-null.  Make
    // sure it is
    if (db_options.env != nullptr) {
      immutable_ = ImmutableDBOptions(db_options);
    } else {
      DBOptions copy = db_options;
      copy.env = config_options.env;
      if (copy.env == nullptr) {
        copy.env = Env::Default();
      }
      immutable_ = ImmutableDBOptions(copy);
    }
    immutable_.object_registry = config_options.registry;

    RegisterOptions("", &immutable_, &object_registry_type_info);
...
std::unique_ptr<Configurable> DBOptionsAsConfigurable(
    const ConfigOptions& cfg_opts, const DBOptions& db_opts) {
  std::unique_ptr<Configurable> ptr(
      new DBOptionsConfigurable(cfg_opts, db_opts));
  return ptr;
}
#endif  // ROCKSDB_LITE
