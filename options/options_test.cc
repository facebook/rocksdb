...
TEST_F(OptionsOldApiTest, GetOptionsFromStringTest) {
  ...
  // Register an Env with default object library.
  ObjectLibrary::Default()->Register<Env>(
      "CustomEnvDefault",
      [](const std::string& /*name*/, std::unique_ptr<Env>* /*env_guard*/,
         std::string* /* errmsg */) {
...
static void TestAndCompareOption(const ConfigOptions& config_options,
  ...
  ASSERT_OK(
      opt_info.SerializeOption(config_options, opt_name, base_ptr, &result));
  ASSERT_OK(opt_info.ParseOption(config_options, opt_name, result, comp_ptr));
  ASSERT_TRUE(opt_info.MatchesOption(config_options, opt_name, base_ptr,
                                     comp_ptr, &mismatch));
}
...
static void TestAndCompareOption(const ConfigOptions& config_options,
                                 ...
                                 void* comp_ptr) {
  ASSERT_OK(
      opt_info.ParseOption(config_options, opt_name, opt_value, base_ptr));
...
void TestOptInfo(const ConfigOptions& config_options, OptionType opt_type,
  ...
  ASSERT_FALSE(
      opt_info.MatchesOption(config_options, "base", base, comp, &result));
...
TEST_F(OptionTypeInfoTest, TestInvalidArgs) {
  ...
  ASSERT_OK(func_info.ParseOption(config_options, "b", "1", &i));
  ASSERT_NOK(func_info.ParseOption(config_options, "b", "x", &i));
}
...
TEST_F(OptionTypeInfoTest, TestParseFunc) {
  ...
  ConfigOptions config_options;
  std::string base;
  ASSERT_OK(opt_info.ParseOption(config_options, "World", "Hello", &base));
  ASSERT_EQ(base, "Hello World");
  ASSERT_NOK(opt_info.ParseOption(config_options, "Oops", "Hello", &base));
}
...
TEST_F(OptionTypeInfoTest, TestSerializeFunc) {
  ...
  std::string base;
  std::string value;
  ASSERT_OK(opt_info.SerializeOption(config_options, "Hello", &base, &value));
  ASSERT_EQ(value, "Hello");
  ASSERT_NOK(opt_info.SerializeOption(config_options, "Oops", &base, &value));
}
...
TEST_F(OptionTypeInfoTest, TestEqualsFunc) {
  ...
  std::string mismatch;
  ASSERT_TRUE(
      opt_info.MatchesOption(config_options, "LT", &int1, &int2, &mismatch));
  ASSERT_EQ(mismatch, "");
  ASSERT_FALSE(
      opt_info.MatchesOption(config_options, "GT", &int1, &int2, &mismatch));
  ASSERT_EQ(mismatch, "GT");
  ASSERT_FALSE(
      opt_info.MatchesOption(config_options, "NO", &int1, &int2, &mismatch));
...
TEST_F(OptionTypeInfoTest, TestOptionFlags) {
  ...
  ASSERT_TRUE(opt_never.MatchesOption(config_options, "Never", &base, &comp,
                                      &mismatch));
  ASSERT_FALSE(
      opt_none.MatchesOption(config_options, "Never", &base, &comp, &mismatch));

  // An alias can change the value via parse, but does nothing on serialize on
  // match
  std::string result;
  ASSERT_OK(opt_alias.ParseOption(config_options, "Alias", "Alias", &base));
  ASSERT_OK(opt_alias.SerializeOption(config_options, "Alias", &base, &result));
  ASSERT_TRUE(
      opt_alias.MatchesOption(config_options, "Alias", &base, &comp, &result));
  ASSERT_EQ(base, "Alias");
  ASSERT_NE(base, comp);

  // Deprecated options do nothing on any of the commands
  ASSERT_OK(
      opt_deprecated.ParseOption(config_options, "Alias", "Deprecated", &base));
  ASSERT_OK(
      opt_deprecated.SerializeOption(config_options, "Alias", &base, &result));
  ASSERT_TRUE(opt_deprecated.MatchesOption(config_options, "Alias", &base,
                                           &comp, &result));
  ASSERT_EQ(base, "Alias");
  ASSERT_NE(base, comp);
}
...
TEST_F(OptionTypeInfoTest, TestCustomEnum) {
  ...
  ASSERT_OK(opt_info.ParseOption(config_options, "", "B", &e1));
  ASSERT_OK(opt_info.SerializeOption(config_options, "", &e1, &result));
  ASSERT_EQ(e1, TestEnum::kB);
  ASSERT_EQ(result, "B");

  ASSERT_FALSE(
      opt_info.MatchesOption(config_options, "Enum", &e1, &e2, &mismatch));
  ASSERT_EQ(mismatch, "Enum");

  TestAndCompareOption(config_options, opt_info, "", "C", &e1, &e2);
  ASSERT_EQ(e2, TestEnum::kC);

  ASSERT_NOK(opt_info.ParseOption(config_options, "", "D", &e1));
...
TEST_F(OptionTypeInfoTest, TestStruct) {
  ...
  ASSERT_FALSE(
      basic_info.MatchesOption(config_options, "b", &e1.b, &e2.b, &mismatch));
  ASSERT_EQ(mismatch, "b.i");
  mismatch.clear();
  ASSERT_FALSE(
      basic_info.MatchesOption(config_options, "b.i", &e1.b, &e2.b, &mismatch));
  ASSERT_EQ(mismatch, "b.i");
  mismatch.clear();
  ASSERT_FALSE(
      basic_info.MatchesOption(config_options, "i", &e1.b, &e2.b, &mismatch));
  ASSERT_EQ(mismatch, "b.i");
  mismatch.clear();

  e1 = e2;
  ASSERT_NOK(
      basic_info.ParseOption(config_options, "b", "{i=33;s=33;j=44}", &e1.b));
  ASSERT_TRUE(
      basic_info.MatchesOption(config_options, "b.i", &e1.b, &e2.b, &mismatch));
  ASSERT_NOK(basic_info.ParseOption(config_options, "b.j", "44", &e1.b));
  ASSERT_TRUE(
      basic_info.MatchesOption(config_options, "b.i", &e1.b, &e2.b, &mismatch));
  ASSERT_NOK(basic_info.ParseOption(config_options, "j", "44", &e1.b));
  ASSERT_TRUE(
      basic_info.MatchesOption(config_options, "b.i", &e1.b, &e2.b, &mismatch));
...
TEST_F(OptionTypeInfoTest, TestVectorType) {
  ...
  vec1[3] = "e";
  ASSERT_FALSE(
      vec_info.MatchesOption(config_options, "v", &vec1, &vec2, &mismatch));
...
TEST_F(ConfigOptionsTest, EnvFromConfigOptions) {
  ...
  config_options.registry->AddLocalLibrary(RegisterCustomEnv,
                                           "RegisterCustomEnv", kCustomEnvName);
  ...
  config_options.ignore_unknown_objects = true;
  ASSERT_OK(
...
TEST_F(ConfigOptionsTest, EnvFromDBOptions) {
  ...
  size_t envs = db_opts.object_registry->GetRegisteredNames(Env::Type());

  db_opts.object_registry->AddLocalLibrary(RegisterCustomEnv,
                                           "RegisterCustomEnv", kCustomEnvName);
  ASSERT_EQ(db_opts.object_registry->GetRegisteredNames(Env::Type()), envs + 1);
  config_options.env = mem_env;
  config_options.ignore_unknown_objects = false;

  // The library is not registered with the ConfigOptions, so
  // the Get should fail.
  ASSERT_NOK(GetDBOptionsFromString(config_options, db_opts, kCustomEnvProp,
                                    &db_opts));
  ASSERT_EQ(db_opts.object_registry->GetRegisteredNames(Env::Type()), envs + 1);
  ASSERT_NE(config_options.env, db_opts.env);

  config_options.registry = db_opts.object_registry;
  ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()), envs + 1);
  ASSERT_OK(GetDBOptionsFromString(config_options, DBOptions(), kCustomEnvProp,
                                   &db_opts));
  ASSERT_NE(config_options.env, db_opts.env);
...
TEST_F(ConfigOptionsTest, DBOptionsFromRegistryString) {
  ...
  if (db_opts.object_registry
          ->AddLocalLibrary(db_opts.env, "RegisterCustomEnv", kCustomEnvName)
          .ok()) {
    std::string opts_str;
    ConfigOptions config_options(db_opts);
    config_options.ignore_unknown_objects = false;
    ASSERT_OK(Env::CreateFromString(config_options, "Custom", &db_opts.env));
    ASSERT_OK(GetStringFromDBOptions(config_options, db_opts, &opts_str));
    ASSERT_OK(GetDBOptionsFromString(config_options, DBOptions(), opts_str,
                                     &db_opts));
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 1);
    ASSERT_EQ(db_opts.object_registry->GetRegisteredNames(Env::Type()),
              envs + 1);

    config_options.registry = db_opts.object_registry;
    config_options.registry->AddLocalLibrary(
        RegisterCustomEnv, "RegisterCustomEnvXXX", "customXX");
    ASSERT_OK(GetStringFromDBOptions(config_options, db_opts, &opts_str));
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 2);

    ASSERT_NOK(GetDBOptionsFromString(config_options, DBOptions(), opts_str,
                                      &db_opts));
    ASSERT_EQ(db_opts.object_registry->GetRegisteredNames(Env::Type()), envs);
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 2);
  }
}
...
TEST_F(ConfigOptionsTest, OptionsFromRegistryString) {
  ...
  size_t envs = opts.object_registry->GetRegisteredNames(Env::Type());
  if (opts.object_registry
          ->AddLocalLibrary(opts.env, "RegisterCustomEnv", kCustomEnvName)
          .ok()) {
    std::string opts_str;
    ConfigOptions config_options(opts);
    config_options.ignore_unknown_objects = false;
    ASSERT_OK(Env::CreateFromString(config_options, kCustomEnvName, &opts.env));
    ASSERT_OK(GetStringFromDBOptions(config_options, opts, &opts_str));
    ASSERT_OK(GetOptionsFromString(config_options, Options(), opts_str, &opts));
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 1);
    ASSERT_EQ(opts.object_registry->GetRegisteredNames(Env::Type()), envs + 1);
    ASSERT_NOK(GetOptionsFromString(config_options, opts,
                                    "table_factory=UnknownXXX", &opts));
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 1);
    ASSERT_EQ(opts.object_registry->GetRegisteredNames(Env::Type()), envs + 1);

    config_options.registry = opts.object_registry;
    config_options.registry->AddLocalLibrary(
        RegisterCustomEnv, "RegisterCustomEnvXXX", "customXX");
    ASSERT_OK(GetStringFromDBOptions(config_options, opts, &opts_str));
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 2);
    ASSERT_NOK(
        GetOptionsFromString(config_options, Options(), opts_str, &opts));
    ASSERT_EQ(opts.object_registry->GetRegisteredNames(Env::Type()), envs);
    ASSERT_EQ(config_options.registry->GetRegisteredNames(Env::Type()),
              envs + 2);
  }
}
