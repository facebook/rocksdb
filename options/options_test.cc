...
TEST_F(OptionsTest, GetColumnFamilyOptionsFromStringTest) {
  ...
  ASSERT_EQ(static_cast<ShardedCache32*>(new_cf_opt.blob_cache.get())
                ->GetNumShardBits(),
            4);
...
TEST_F(OptionsTest, GetBlockBasedTableOptionsFromString) {
  ...
  ASSERT_TRUE(new_opt.block_cache != nullptr);
  ASSERT_EQ(new_opt.block_cache->GetCapacity(), 1024UL*1024UL);
  ASSERT_EQ(std::dynamic_pointer_cast<ShardedCache32>(new_opt.block_cache)
                ->GetNumShardBits(),
            4);
  ...
  ASSERT_EQ(
      std::dynamic_pointer_cast<ShardedCache32>(new_opt.block_cache_compressed)
          ->GetNumShardBits(),
      4);
  ...
  ASSERT_EQ(std::dynamic_pointer_cast<ShardedCache32>(new_opt.block_cache)
                ->GetNumShardBits(),
            ShardedCache32::GetDefaultCacheShardBits(
                new_opt.block_cache->GetCapacity()));
