...
static Status GetMutableOptionsFromMap(
    const ConfigOptions& config_options, const MutableCFOptions& base_options,
  ...
  *new_options = base_options;
  ...
  Status s =
      ParseOptionsTypeFromMap(config_options, cf_mutable_options_type_info,
                              new_options, options_map, unused_opts);
  if (!s.ok()) {
    *new_options = base_options;
  }
  return s;
...
Status GetMutableOptionsFromStrings(
  ...
  *new_options = base_options;
  ...
  for (const auto& o : options_map) {
    std::string elem;
    const auto opt_info = OptionTypeInfo::FindOption(
        o.first, cf_mutable_options_type_info, &elem);
    if (opt_info == nullptr) {
      ...
      Status s =
          opt_info->ParseOption(config_options, elem, o.second, new_options);
      if (!s.ok()) {
        return s;
      }
    }
...
Status ParseOptionsTypeFromMap(
  ...
  std::unordered_map<std::string, std::string> unused_opts;
  return ParseOptionsTypeFromMap(config_options, opt_map, opt_ptr, options,
                                 &unused_opts);
}
...
Status ParseOptionsTypeFromMap(
  ...
  for (const auto& o : options) {
    ...
    if (opt_info == nullptr) {
      ...
      s = opt_info->ParseOption(config_options, opt_name, o.second, opt_ptr);
      if (s.ok()) {
        found_one = true;
  ...
  while (found_one && !unused_opts->empty()) {
    ...
    for (auto it = unused_opts->begin(); it != unused_opts->end();) {
      ...
      if (opt_info != nullptr) {
        s = opt_info->ParseOption(config_options, opt_name, it->second,
                                  opt_ptr);
        if (s.ok()) {
          found_one = true;
  ...
  if (config_options.ignore_unknown_options || (invalid.ok() && result.ok())) {
    return Status::OK();
  } else if (!invalid.ok()) {
    return invalid;
  } else {
  ...
  *new_options = base_options;
  ...
  for (const auto& o : options_map) {
    try {
      std::string elem;
      const auto opt_info = OptionTypeInfo::FindOption(
          o.first, db_mutable_options_type_info, &elem);
      if (opt_info == nullptr) {
        return Status::InvalidArgument("Unrecognized option: " + o.first);
...
Status GetStringFromStruct(
  ...
  for (const auto iter : type_info) {
    ...
    if (opt_info.ShouldSerialize()) {
      std::string value;
      Status s =
          opt_info.SerializeOption(config_options, iter.first, opt_ptr, &value);
      if (s.ok()) {
...
Status GetStringFromMutableDBOptions(const ConfigOptions& config_options,
                                     ...
                                     std::string* opt_string) {
  return GetStringFromStruct(config_options, &mutable_opts,
                             db_mutable_options_type_info, opt_string);
}
...
Status GetStringFromDBOptions(const ConfigOptions& config_options,
  ...
  Status s = GetStringFromMutableDBOptions(config_options, mdb, &mutable_opts);
  if (s.ok()) {
    s = GetStringFromStruct(config_options, &idb,
                            db_immutable_options_type_info, &immutable_opts);
  }
  if (s.ok()) {
...
Status GetStringFromMutableCFOptions(const ConfigOptions& config_options,
  ...
  opt_string->clear();
  return GetStringFromStruct(config_options, &mutable_opts,
                             cf_mutable_options_type_info, opt_string);
}
...
Status GetStringFromColumnFamilyOptions(const ConfigOptions& config_options,
  ...
  Status s =
      GetStringFromStruct(config_options, &cf_options,
                          cf_immutable_options_type_info, &immutable_string);
  if (s.ok()) {
    s = GetStringFromMutableCFOptions(
        config_options, MutableCFOptions(cf_options), &mutable_string);
  }
  if (s.ok()) {
...
Status GetColumnFamilyOptionsFromMap(
  ...
  *new_options = base_options;
  ...
  Status s = GetMutableOptionsFromMap(copy, MutableCFOptions(*new_options),
                                      opts_map, &mcf, &unused);
  if (s.ok() && !unused.empty()) {
    copy.ignore_unknown_options = config_options.ignore_unknown_options;
    s = ParseOptionsTypeFromMap(copy, cf_immutable_options_type_info,
                                new_options, unused);
  }
  if (s.ok()) {
...
Status GetDBOptionsFromMapInternal(
  ...
  *new_options = base_options;
  ...
  Status s = ParseOptionsTypeFromMap(copy, db_immutable_options_type_info, &idb,
                                     opts_map, &unused_idb);
  if (s.ok()) {
    copy.ignore_unknown_options = config_options.ignore_unknown_options;
    s = ParseOptionsTypeFromMap(copy, db_mutable_options_type_info, &mdb,
                                unused_idb, unused_opts);
  }
  if (!s.ok()) {
    // Restore "new_options" to the default "base_options".
    *new_options = base_options;
...
Status GetOptionsFromString(const ConfigOptions& config_options,
  ...
  s = GetDBOptionsFromMapInternal(copy, base_options, opts_map, &new_db_options,
                                  &unused_opts);
  if (s.ok()) {
    copy.ignore_unknown_options = config_options.ignore_unknown_options;
...
Status OptionTypeInfo::ParseStruct(
  ...
  if (opt_name == struct_name || EndsWith(opt_name, "." + struct_name)) {
    ...
    if (status.ok()) {
      for (const auto& map_iter : opt_map) {
        ...
        if (iter != struct_map->end()) {
          status = iter->second.ParseOption(config_options, map_iter.first,
    ...
    if (opt_info != nullptr) {
      status =
          opt_info->ParseOption(config_options, elem_name, opt_value, opt_addr);
    } else {
...
Status OptionTypeInfo::SerializeStruct(
  ...
  if (EndsWith(opt_name, struct_name)) {
    ...
    for (const auto& iter : *struct_map) {
      ...
      if (opt_info.ShouldSerialize()) {
        status =
            opt_info.SerializeOption(embedded, iter.first, opt_addr, &single);
        if (!status.ok()) {
          return status;
        } else {
    ...
    *value = "{" + result + "}";
    ...
    if (opt_info != nullptr) {
      status =
          opt_info->SerializeOption(config_options, elem_name, opt_addr, value);
    } else {
      status = Status::InvalidArgument("Unrecognized option: ", opt_name);
    }
  } else {
    ...
    if (opt_info == nullptr) {
      ...
      return opt_info->SerializeOption(
          config_options, opt_name + "." + elem_name, opt_addr, value);
    }
  }
  return Status::OK();
}
...
bool OptionTypeInfo::MatchesStruct(
  ...
  if (EndsWith(opt_name, struct_name)) {
    ...
    for (const auto& iter : *struct_map) {
      ...
      matches = opt_info.MatchesOption(config_options, iter.first, this_offset,
                                       that_offset, &result);
      if (!matches) {
    ...
    if (opt_info == nullptr) {
      *mismatch = opt_name;
      matches = false;
    } else if (!opt_info->MatchesOption(config_options, elem_name, this_offset,
                                        that_offset, &result)) {
      matches = false;
      *mismatch = struct_name + "." + result;
    }
...
bool MatchesOptionsTypeFromMap(
  ...
  for (auto& pair : type_map) {
    ...
    if (config_options.IsCheckEnabled(pair.second.GetSanityLevel())) {
      if (!pair.second.MatchesOption(config_options, pair.first, this_ptr,
                                     that_ptr, mismatch) &&
          !pair.second.CheckByName(config_options, pair.first, this_ptr,
                                   that_ptr)) {
        return false;
      }
    }
...
bool OptionTypeInfo::CheckByName(const ConfigOptions& config_options,
                                 const std::string& opt_name,
                                 const void* const this_ptr,
                                 const void* const that_ptr) const {
  if (IsByName()) {
    std::string that_value;
    if (SerializeOption(config_options, opt_name, that_ptr, &that_value).ok()) {
      return CheckByName(config_options, opt_name, this_ptr, that_value);
    }
  }
  return false;
}
...
bool OptionTypeInfo::CheckByName(const ConfigOptions& config_options,
  ...
  if (!IsByName()) {
    return false;
  } else if (!SerializeOption(config_options, opt_name, opt_ptr, &this_value)
                  .ok()) {
    return false;
