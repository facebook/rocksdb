  ...
  try {
    ...
    if (opt_addr == nullptr) {
      ...
      if (config_options.invoke_prepare_options &&
        ...
        // If the option is marked don't prepare, turn it off
        ConfigOptions copy = config_options;
        copy.invoke_prepare_options = false;
        return parser_func(copy, opt_name, opt_value, opt_addr);
      } else {
...
Status OptionTypeInfo::SerializeOption(const ConfigOptions& config_options,
  ...
  if (opt_addr == nullptr || IsDeprecated()) {
    ...
    if (config != nullptr) {
      *opt_value = config->ToString(config_options.Embedded());
    }
    return Status::OK();
  } else {
  ...
  if (this_addr == nullptr || that_addr == nullptr) {
    ...
    if (this_config == that_config) {
      ...
      if (level < config_options.sanity_level) {
        ...
        matches = this_config->Matches(copy, that_config, &bad_name);
      } else {
        matches = this_config->Matches(config_options, that_config, &bad_name);
      }
      if (!matches) {
