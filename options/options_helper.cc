...
Status OptionTypeInfo::Parse(const ConfigOptions& config_options,
  ...
  try {
    ...
    if (opt_addr == nullptr) {
    ...
    } else if (opt_name == "read_amp_bytes_per_bit") {
      ...
      // the fix, we need to manually parse read_amp_bytes_per_bit with this
      // special hack.
      assert(type_ == OptionType::kUInt32T);
      uint64_t read_amp_bytes_per_bit = ParseUint64(opt_value);
      EncodeFixed32(opt_addr, static_cast<uint32_t>(read_amp_bytes_per_bit));
      return Status::OK();
